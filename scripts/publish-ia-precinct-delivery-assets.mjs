...
import {
  inspectIowaPrecinctBlobPublicationPlan,
  validateIowaBlobPublicationAuthorization,
} from "./lib/ia-precinct-blob-publication.mjs";
...
function parseArguments(args) {
  ...
  for (const argument of args) {
    if (
      ...
      throw new Error("Unknown Iowa Blob publication option: " + argument);
    }
  }
  if (!packagePath || !packageSha256) {
    throw new Error(
      "usage: npm run precinct-gis:delivery-publish:ia -- "
      + "--package=<release-candidate.json> --package-sha256=<sha256> "
      + "[--concurrency=4] [--write]",
    );
  }
  if (!Number.isInteger(concurrency) || concurrency < 1 || concurrency > 8) {
    throw new Error("Iowa Blob publication concurrency must be 1 through 8");
  }
...
async function verifyPublicBlob(url, artifact) {
  ...
  if (!response.ok) {
    throw new Error(
      "Published Iowa geometry returned HTTP " + response.status,
    );
  }
  const bytes = Buffer.from(await response.arrayBuffer());
  if (bytes.length !== artifact.byteCount || sha256(bytes) !== artifact.sha256) {
    throw new Error("Published Iowa geometry failed byte/hash verification");
  }
}
...
async function publishArtifact(artifact, sdk) {
  ...
  if (existing) {
    if (existing.size !== artifact.byteCount) {
      throw new Error(
        "Existing immutable Iowa geometry has a different byte count",
      );
    }
  ...
  if (
    ...
    throw new Error("Published Iowa geometry URL/pathname drifted");
  }
...
export async function publishIowaPrecinctDeliveryAssets(options) {
  ...
  const authorization = validateIowaBlobPublicationAuthorization(plan);
  ...
  if (origins.size !== 1) {
    throw new Error("Iowa geometry assets were not published to one origin");
  }
...
async function main() {
  ...
  const result = await publishIowaPrecinctDeliveryAssets(args);
