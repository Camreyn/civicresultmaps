...
import {
  inspectPennsylvaniaPrecinctBlobPublicationPlan,
  validatePennsylvaniaBlobPublicationAuthorization,
} from "./lib/pa-precinct-blob-publication.mjs";
...
function parseArguments(args) {
  ...
  for (const argument of args) {
    if (
      ...
      throw new Error("Unknown Pennsylvania Blob publication option: " + argument);
    }
  }
  ...
  if (!Number.isInteger(concurrency) || concurrency < 1 || concurrency > 8) {
    throw new Error("Pennsylvania Blob publication concurrency must be 1 through 8");
  }
...
async function verifyPublicBlob(url, artifact) {
  ...
  if (!response.ok) {
    throw new Error(
      "Published Pennsylvania geometry returned HTTP " + response.status,
    );
  }
  const bytes = Buffer.from(await response.arrayBuffer());
  if (bytes.length !== artifact.byteCount || sha256(bytes) !== artifact.sha256) {
    throw new Error("Published Pennsylvania geometry failed byte/hash verification");
  }
}
...
async function publishArtifact(artifact, sdk) {
  ...
  if (existing) {
    if (existing.size !== artifact.byteCount) {
      throw new Error(
        "Existing immutable Pennsylvania geometry has a different byte count",
      );
    }
  ...
  if (
    ...
    throw new Error("Published Pennsylvania geometry URL/pathname drifted");
  }
...
export async function publishPennsylvaniaPrecinctDeliveryAssets(options) {
  ...
  const authorization = validatePennsylvaniaBlobPublicationAuthorization(plan);
  ...
  if (origins.size !== 1) {
    throw new Error("Pennsylvania geometry assets were not published to one origin");
  }
...
async function main() {
  ...
  const result = await publishPennsylvaniaPrecinctDeliveryAssets(args);
