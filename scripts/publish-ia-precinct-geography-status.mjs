...
import {
  buildIowaPrecinctExecutionContext,
...
function parseArguments(args) {
  ...
  for (const arg of args) {
    if (
      ...
      throw new Error("Unknown Iowa publication-status option: " + arg);
    }
  }
  const modes = [
    parsed.writePlan,
  ...
  if (modes > 1) throw new Error("Iowa publication-status modes are mutually exclusive");
  for (const field of [
    "packagePath",
    "packageSha256",
    "hiddenReceiptPath",
...
function safeJson(root, relativePath, expectedSha256, allowedRoot) {
  if (
    ...
    throw new Error("Iowa publication-status JSON path is unsafe");
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ...allowedRoot.split("/"));
  if (!absolute.startsWith(allowed + path.sep) || !existsSync(absolute)) {
    throw new Error("Iowa publication-status JSON is missing");
  }
  const bytes = readFileSync(absolute);
  if (sha256(bytes) !== expectedSha256) {
    throw new Error("Iowa publication-status JSON SHA-256 drifted");
  }
  return {
    path: relativePath,
    absolute,
...
function immutableJson(root, relativePath, value, allowedRoot) {
  if (
    ...
    throw new Error("Iowa publication-status output path is unsafe");
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ...allowedRoot.split("/"));
  if (!absolute.startsWith(allowed + path.sep)) {
    throw new Error("Iowa publication-status output escapes its fixed root");
  }
  const bytes = serializeIowaPublicationDocument(value);
  if (existsSync(absolute)) {
    if (!readFileSync(absolute).equals(bytes)) {
      throw new Error("Refusing to overwrite different Iowa publication evidence");
    }
...
async function verifyPostconditions(
  ...
  if (versions.length !== 3) {
    throw new Error("Iowa publication postcondition has an incomplete version set");
  }
  ...
  if (Number(results[0]?.total) !== 14_982) {
    throw new Error("Iowa publication result-row postcondition failed");
  }
...
export async function applyIowaGeographyPublicationTransaction(nv, context) {
  ...
  if (
    ...
    throw new Error("Iowa publication transaction database identity drifted");
  }
...
function defaultPlanPath(planSha256) {
  return ".etl/precinct-publication-plans/IA/ia-publication-plan-"
...
function reserveReceipt(
  ...
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ".etl", "production-publication-receipts", "IA");
  if (!absolute.startsWith(allowed + path.sep) || !relativePath.endsWith(".json")) {
    throw new Error("Iowa publication receipt path is unsafe");
  }
  if (existsSync(absolute)) throw new Error("Iowa publication receipt already exists");
  mkdirSync(path.dirname(absolute), { recursive: true });
...
function finishPublicationReceipt(reservation, receipt) {
  const receiptBytes = serializeIowaPublicationDocument(receipt);
  ...
  if (existsSync(temporary)) {
    if (!readFileSync(temporary).equals(receiptBytes)) {
      throw new Error("Iowa publication receipt temporary file drifted");
    }
  } else {
...
export async function runIowaGeographyPublication(options = {}) {
  ...
  if (parsed.writePlan) {
    ...
    const artifact = immutableJson(
      ...
      built.plan,
      ".etl/precinct-publication-plans/IA",
    );
    return {
      mode: "write_plan",
  ...
  if (endpointFingerprint !== built.plan.hiddenLoad.endpointFingerprint) {
    throw new Error("Iowa publication database endpoint drifted from the hidden load");
  }
  const gisPlan = await buildIowaPrecinctGisPlan({
    root,
  ...
  if (parsed.recoverReceipt) {
    ...
    try {
      sql = (options.postgresFactory ?? postgres)(databaseUrl, {
        ...
        idle_timeout: 20,
        connection: {
          application_name: "civicresultmaps-ia-precinct-publication-receipt-recovery",
  ...
  try {
    sql = (options.postgresFactory ?? postgres)(databaseUrl, {
      ...
      connect_timeout: 10,
      idle_timeout: 20,
      connection: { application_name: "civicresultmaps-ia-precinct-publication-status" },
    });
  } catch (error) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  runIowaGeographyPublication().then(
