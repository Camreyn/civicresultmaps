...
for (const arg of args) {
  if (
    ...
    throw new Error("Unknown Nevada GIS setup option: " + arg);
  }
}
...
function writeReport(relativePath, value) {
  ...
  if (target !== etlRoot && !target.startsWith(etlRoot + path.sep)) {
    throw new Error("Nevada GIS reports must remain under .etl");
  }
...
try {
  ...
  } = await import("./lib/nv-precinct-gis-plan.mjs");
  const plan = await buildNevadaPrecinctGisPlan({ years });
  ...
  if (!apply) {
    ...
    const {
      applyNevadaPrecinctGisPlan,
      validateNevadaPrecinctGisDatabase,
    } = await import("./lib/nv-precinct-gis-db.mjs");
    const applied = await applyNevadaPrecinctGisPlan(plan);
    const validation = await validateNevadaPrecinctGisDatabase(plan);
    const report = {
      schemaVersion: 1,
      generatedAtUtc: new Date().toISOString(),
      scope: "local-only Nevada precinct GIS setup",
      productionMutationPerformed: false,
      publicDeliveryAuthorized: false,
      plan: summary,
      applied,
    ...
    const reportPath = writeReport(
      reportArgument
        ...
        : ".etl/local-db/nv-precinct-gis-setup-report.json",
      report,
    );
