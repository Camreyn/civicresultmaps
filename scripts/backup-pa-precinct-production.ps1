...
$VerifyDatabase = 'crm_pa_precinct_restore_verify'
$BackupRoot = 'C:\tmp\crm-db-clone\pa-precinct-release-backups'
$ContainerBackupRoot = '/backups/pa-precinct-release-backups'
$ExpectedLegacyEndpointFingerprint = 'bf2bf2213814'

function Fail([string]$Message) {
  throw "Pennsylvania production backup aborted: $Message"
}
...
function Assert-ReleasePackage {
  $candidateRoot = [IO.Path]::GetFullPath((Join-Path $RepoRoot '.etl\precinct-release-candidates\PA'))
  ...
  if (
    ...
    Fail 'The release package must be an existing release-candidate.json under .etl/precinct-release-candidates/PA.'
  }
  ...
  if (
    ...
    $document.id -ne 'pa-precinct-gis-two-election-v1' -or
    $document.state -ne 'PA' -or
    ...
    $document.totals.elections -ne 2
  ) {
...
if (-not $Execute) {
  [ordered]@{
    ...
    acknowledgementRequired = @(
      'CRM_PA_PRECINCT_GEOGRAPHY_BACKUP_ACK=CREATE_FULL_PUBLIC_SCHEMA_ROLLBACK_BACKUP',
      'CRM_PA_PRECINCT_GEOGRAPHY_BACKUP_ENDPOINT_FINGERPRINT=<fresh 64-hex preflight endpoint fingerprint>'
    )
...
if (
  [Environment]::GetEnvironmentVariable('CRM_PA_PRECINCT_GEOGRAPHY_BACKUP_ACK') -ne 'CREATE_FULL_PUBLIC_SCHEMA_ROLLBACK_BACKUP' -or
  [Environment]::GetEnvironmentVariable('CRM_PA_PRECINCT_GEOGRAPHY_BACKUP_PACKAGE_SHA256') -ne $ReleasePackageSha256
) {
...
if (
  ...
  [Environment]::GetEnvironmentVariable('CRM_PA_PRECINCT_GEOGRAPHY_BACKUP_ENDPOINT_FINGERPRINT') -ne $sourceEndpointFingerprint
) {
...
$dumpFile = "pa-precinct-full-public-$stamp.dump"
$dumpPath = Join-Path $BackupRoot $dumpFile
$containerDumpPath = "$ContainerBackupRoot/$dumpFile"
$manifestPath = Join-Path $BackupRoot "pa-precinct-full-public-$stamp.manifest.json"
