...
export const WISCONSIN_PUBLIC_ACTIVATION_ROOT =
  ".etl/precinct-public-activations/WI";
export const WISCONSIN_PUBLIC_ACTIVATION_YEARS = Object.freeze([
  2016,
  2020,
  2024,
]);
...
export function serializeWisconsinPublicActivationDocument(value) {
...
function readRepositoryJson(root, relativePath) {
  ...
  if (!existsSync(absolutePath)) {
    throw new Error("Wisconsin public activation target is missing: " + relativePath);
  }
...
function loadReleasePackage(options) {
  if (!/^[a-f0-9]{64}$/.test(options.packageSha256 ?? "")) {
    throw new Error("Wisconsin public activation requires the exact package SHA-256");
  }
  ...
  if (
    ...
    || !semanticallyEqual(
      ...
      WISCONSIN_PUBLIC_ACTIVATION_YEARS,
    )
    || Number.isNaN(Date.parse(document?.preparedFromLocalValidationAt))
  ) {
    throw new Error("Wisconsin public-activation package contract is incompatible");
  }
  return {
    artifact,
    document,
...
function loadDraftManifests(root, loaded) {
  return loaded.document.years.map((year) => {
    ...
    if (
      ...
      throw new Error("Wisconsin " + year.year + " draft manifest is not public-eligible");
    }
    const preimage = inspectReleaseArtifact(root, year.canonicalManifest.path, {
      allowedRoots: ["data/precinct-geometry/WI/"],
    ...
    if (
      ...
      throw new Error("Wisconsin " + year.year + " canonical source preimage is not blocked");
    }
    return {
      year: year.year,
...
function updateRegistry(root, manifests, activatedAtUtc) {
  ...
  if (
    ...
    throw new Error("Wisconsin canonical manifest registry is invalid");
  }
  const existing = current.value.manifests.filter((row) => row?.state === "WI");
  ...
  if (existing.length === 0) {
    ...
    throw new Error("Wisconsin canonical registry is partially activated or drifted");
  }
  const value = {
    ...current.value,
  ...
  if (inspection.errors.length) {
    throw new Error(
      "Wisconsin activated registry is invalid: " + inspection.errors.join("; "),
    );
  }
  const eligible = inspection.manifests.filter((item, index) =>
    value.manifests[index]?.state === "WI"
    && item.publicEligibilityReasons.length === 0);
  if (eligible.length !== 3) {
    throw new Error("Wisconsin activated registry does not contain three eligible manifests");
  }
...
function wisconsinCoverageRow(manifest, activatedAtUtc) {
  return {
    state: "WI",
    stateName: "Wisconsin",
    ...
    generalOfficialSourceLeads: [
      ...
      manifest.election.year === 2016
        ? "https://elections.wi.gov/media/7123/download"
        : `https://elections.wi.gov/election-result/${manifest.election.year}-fall-general-election-results`,
    ],
    geometry: {
    ...
    notes: [
      ["official_export", "official_service"].includes(
        ...
        ? "An official Wisconsin local-boundary source supplies the reviewed presentation geometry."
        : manifest.election.year === 2024
          ? "The New York Times election-specific official-boundary compilation is used only with official Wisconsin Elections Commission result rows under retained non-commercial attribution terms."
          : "VEST election-specific geometry is explicitly attributed and used only with official Wisconsin Elections Commission local result rows and retained CC BY 4.0 evidence.",
      "The attributed election-specific geometry is joined to official Wisconsin Elections Commission local results through retained reviewed-name and spatial-review relationships; all secondary election values are excluded from geometry delivery.",
      manifest.election.year === 2024
      ...
      "The 2012 election remains separately blocked because the retained LTSB 2011 layer does not provide a reviewed vote-preserving crosswalk from the 3,525 official reporting rows.",
    ],
  };
}
...
function updateCoverage(root, manifest, relativePath, activatedAtUtc) {
  ...
  if (!Array.isArray(current.value?.states)) {
    throw new Error("Wisconsin coverage inventory is invalid: " + relativePath);
  }
  const existing = current.value.states.filter((row) => row?.state === "WI");
  let disposition;
  let states;
  if (existing.length === 0) {
    const expectedRow = wisconsinCoverageRow(manifest, activatedAtUtc);
    disposition = "activate";
    ...
    if (
      ...
      throw new Error(relativePath + " contains a drifted Wisconsin row");
    }
    const expectedRow = {
      ...row,
    ...
    if (currentEligible === 1) {
      if (!semanticallyEqual(row, expectedRow)) {
        throw new Error(relativePath + " contains a partial Wisconsin activation");
      }
      disposition = "verified_existing";
      ...
      states = current.value.states.map((candidate) =>
        candidate?.state === "WI" ? expectedRow : candidate);
    }
  } else {
    throw new Error(relativePath + " contains a partial or drifted Wisconsin row");
  }
  const value = {
    ...current.value,
    updatedAt: disposition === "activate" ? activatedAtUtc : current.value.updatedAt,
...
function outputFile(relativePath, built) {
  const bytes = serializeWisconsinPublicActivationDocument(built.value);
  return {
    path: relativePath,
...
export function inspectWisconsinPublicActivationPlan(options = {}) {
  ...
  for (const [year, relativePath] of COVERAGE_PATHS) {
    ...
    if (!manifest) throw new Error("Wisconsin activation is missing year " + year);
  ...
  const plan = {
    schemaVersion: 1,
    id: "wi-local-public-activation-three-election-v1",
    state: "WI",
    ...
    manifests: manifests.map((item) => ({
      ...
      draftManifest: {
        ...
        publicManifestSha256: sha256(
          serializeWisconsinPublicActivationDocument(item.draft.manifest),
        ),
  ...
  const bytes = serializeWisconsinPublicActivationDocument(plan);
  ...
  return {
    ...
    outputPath: path.posix.join(
      WISCONSIN_PUBLIC_ACTIVATION_ROOT,
