...
import { inspectAlaskaPrecinctBlobPublicationPlan } from "./ak-precinct-blob-publication.mjs";
import { inspectReleaseArtifact } from "./ak-precinct-release-candidate.mjs";
import {
  inspectPrecinctGeometryManifest,
  inspectPrecinctGeometryRegistry,
} from "../../src/lib/precinct-geography.ts";

export const ALASKA_PUBLICATION_YEARS = Object.freeze([2012, 2016, 2020, 2024]);
export const ALASKA_PUBLICATION_SCOPES = Object.freeze([
  "publish_ak_precinct_geography_versions",
  "authorize_ak_precinct_results",
  "increment_public_data_revision",
]);
export const ALASKA_PUBLIC_ROLLBACK_SCOPES = Object.freeze([
  "block_ak_precinct_geography_versions",
  "revoke_ak_precinct_results",
  "increment_public_data_revision",
]);

const EXPECTED_TOTALS = Object.freeze({
  reportingUnits: 2_205,
  candidateResultRows: 12_021,
  geometryFeatures: 1_722,
  reviewedRelationships: 2_205,
  zeroVoteUnits: 2,
  sourceDocuments: 8,
  importRuns: 4,
});
...
export function serializeAlaskaPublicationDocument(value) {
...
function safeEvidence(root, relativePath, allowedRoot, expectedSha256) {
  if (
    ...
    throw new Error("Alaska publication evidence path is unsafe");
  }
  ...
  if (!absolutePath.startsWith(allowed + path.sep) || !existsSync(absolutePath)) {
    throw new Error("Alaska publication evidence is missing");
  }
  const bytes = readFileSync(absolutePath);
  const digest = sha256(bytes);
  if (digest !== expectedSha256) {
    throw new Error("Alaska publication evidence SHA-256 drifted");
  }
  return {
    path: relativePath,
    absolutePath,
...
function credentialFreeHttpsOrigin(value) {
  ...
  try {
    ...
    throw new Error("Alaska publication delivery origin is invalid");
  }
  if (
    ...
    throw new Error("Alaska publication origin must be credential-free HTTPS");
  }
...
function loadPackage(root, packagePath, packageSha256) {
  if (!/^[a-f0-9]{64}$/.test(packageSha256 ?? "")) {
    throw new Error("Alaska publication requires the exact package SHA-256");
  }
  ...
  if (
    ...
    || document?.id !== "ak-precinct-gis-four-election-v1"
    || document?.state !== "AK"
    ...
    || !semanticallyEqual(
      ...
      ALASKA_PUBLICATION_YEARS,
    )
  ) {
    throw new Error("Alaska publication release package is incompatible");
  }
  return {
    artifact,
    document,
...
function publicManifests(root, loaded) {
  ...
  if (inspection.errors.length) {
    throw new Error("Alaska publication registry is invalid: " + inspection.errors.join("; "));
  }
  const rows = registry.manifests.filter((manifest) => manifest?.state === "AK");
  if (rows.length !== 4) {
    throw new Error("Alaska publication registry does not contain four manifests");
  }
  return loaded.document.years.map((year) => {
    ...
    if (
      ...
      || manifest.delivery?.parentCount !== 40
    ) {
      throw new Error("Alaska " + year.year + " public manifest drifted from the package");
    }
    return {
      year: year.year,
      electionId: year.electionId,
      manifestId: manifest.id,
      publicManifestSha256: sha256(serializeAlaskaPublicationDocument(manifest)),
    ...
    if (new Set(all.map((row) => row.manifestId)).size !== 4) {
      throw new Error("Alaska publication manifest IDs are duplicated");
    }
    return item;
  });
}
...
export function validateAlaskaHiddenLoadReceipt(value, context) {
  ...
  if (
    ...
    throw new Error("Alaska hidden-load receipt is incomplete or incompatible");
  }
  const totals = {
  ...
  for (const [key, expected] of Object.entries(EXPECTED_TOTALS)) {
    ...
    if (Number(totals[key]) !== expected) {
      throw new Error("Alaska hidden-load receipt total drifted: " + key);
    }
  }
  return {
...
export function validateAlaskaBlobPublicationEvidence(value, blobPlan, now = Date.now()) {
  const origin = credentialFreeHttpsOrigin(value?.deliveryOrigin);
  if (
    value?.schemaVersion !== 1
    || value?.state !== "AK"
    || value?.purpose !== "ak-precinct-parent-scoped-immutable-geometry-publication"
    ...
    || value?.assetCount !== 164
    || value?.canonicalManifestChanged !== false
    || value?.publicEligibilityChanged !== false
    || !Array.isArray(value?.artifacts)
    || value.artifacts.length !== 164
  ) {
    throw new Error("Alaska Blob publication evidence is incomplete or incompatible");
  }
  ...
  for (const artifact of value.artifacts) {
    ...
    try {
      ...
      throw new Error("Alaska Blob publication evidence contains an invalid URL");
    }
    if (
      !wanted
      ...
      throw new Error("Alaska Blob publication artifact set drifted");
    }
    seen.add(artifact.pathname);
  }
  if (seen.size !== expected.size) {
    throw new Error("Alaska Blob publication artifact set is incomplete");
  }
  return {
...
export function inspectAlaskaPublicationPlan(options = {}) {
  ...
  const hidden = validateAlaskaHiddenLoadReceipt(hiddenArtifact.value, {
    now,
  ...
  const blobArtifact = safeEvidence(
    ...
    ".etl/precinct-blob-publications/AK",
    options.blobEvidenceSha256,
  );
  const blobPlan = inspectAlaskaPrecinctBlobPublicationPlan({
    root,
    packagePath: loaded.identity.path,
    packageSha256: loaded.identity.sha256,
  });
  const blob = validateAlaskaBlobPublicationEvidence(blobArtifact.value, blobPlan, now);
  ...
  const bytes = serializeAlaskaPublicationDocument(plan);
  return {
    root,
...
export function buildAlaskaPublicationAuthorizationTemplate(plan, planSha256) {
  return {
    schemaVersion: 1,
    state: "AK",
    ...
    scopes: [...ALASKA_PUBLICATION_SCOPES],
...
export function validateAlaskaPublicationAuthorization(value, context) {
  ...
  if (
    ...
    || !semanticallyEqual(value?.scopes, ALASKA_PUBLICATION_SCOPES)
    ...
    throw new Error("Alaska public authorization is absent, expired, or incompatible");
  }
  return {
...
export function buildAlaskaPublicRollbackAuthorizationTemplate(
  plan,
  planSha256,
  publicationReceipt,
) {
  return {
    ...
    scopes: [...ALASKA_PUBLIC_ROLLBACK_SCOPES],
    rollbackWindow: {
      startsAtUtc: null,
      endsAtUtc: null,
    },
...
export function validateAlaskaPublicRollbackAuthorization(value, context) {
  const now = context.now ?? Date.now();
  const authorizedAt = Date.parse(value?.authorizedAtUtc);
  const expiresAt = Date.parse(value?.expiresAtUtc);
  ...
  if (
    ...
    || !semanticallyEqual(value?.scopes, ALASKA_PUBLIC_ROLLBACK_SCOPES)
    ...
    throw new Error("Alaska rollback authorization is absent, expired, or incompatible");
  }
  return {
