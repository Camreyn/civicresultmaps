...
const APPLICATION_NAME = "civicresultmaps-local-nv-precinct-gis";
...
function validatedProductionReleaseAudit(value) {
  if (!value || typeof value !== "object") {
    throw new Error("Production Nevada execution requires durable release-audit metadata");
  }
  const output = {};
  for (const key of PRODUCTION_RELEASE_AUDIT_ARTIFACT_KEYS) {
    ...
    if (
      ...
      throw new Error("Production Nevada execution release-audit metadata is invalid: " + key);
    }
  ...
  if (
    ...
    throw new Error("Production Nevada execution release-audit metadata is invalid");
  }
...
export function buildNevadaPrecinctExecutionContext(options = {}) {
  if (!options.mode || options.mode === "local") return LOCAL_EXECUTION_CONTEXT;
  if (options.mode !== "production_release") {
    throw new Error("Unknown Nevada precinct execution mode");
  }
  ...
  if (typeof options.databaseName !== "string" || !options.databaseName.trim()) {
    throw new Error("Production Nevada execution requires the preflight database name");
  }
  return Object.freeze({
    mode: "production_release",
    importParser: "nevadaPrecinctGisProductionRelease",
...
export function assertNevadaGeometryVersionPrecondition(versions, yearPlan) {
  if (
...
async function electionAndContest(tx, yearPlan) {
  ...
  const contests = await query(tx, [
    ...
    "returning id",
  ], [electionId, STATE, "Nevada " + yearPlan.year + " President"]);
...
async function reportingUnits(
  ...
  if (stored.length !== yearPlan.reportingUnits.length) {
    throw new Error("Nevada " + yearPlan.year + " reporting-unit count drifted");
  }
  const ids = new Map();
...
async function results(tx, yearPlan, contestId, importRunId, sourceDocumentId, unitIds) {
  for (const batch of chunks(yearPlan.resultRows)) {
    ...
    if (records.some((row) => !row.reporting_unit_id)) {
      throw new Error("Nevada " + yearPlan.year + " result lacks reporting unit");
    }
    await query(tx, [
  ...
  if (stored.length !== yearPlan.resultRows.length) {
    throw new Error("Nevada " + yearPlan.year + " precinct-result count drifted");
  }
  ...
  for (const row of stored) {
    ...
    if (
      ...
      throw new Error("Nevada " + yearPlan.year + " result drift at " + key);
    }
  }
}
...
async function geometry(
  ...
  if (yearPlan.geometry.disposition === "blocked") {
    ...
    if (Number(existing[0].count) !== 0) {
      throw new Error("Blocked Nevada " + yearPlan.year + " already has geometry");
    }
  ...
  assertNevadaGeometryVersionPrecondition(versions, yearPlan);
  ...
  if (storedFeatures.length !== yearPlan.geometry.features.length) {
    throw new Error("Nevada " + yearPlan.year + " geography-feature count drifted");
  }
  ...
  for (const batch of chunks(yearPlan.geometry.crosswalks)) {
    ...
    if (records.some((row) => !row.reporting_unit_id || !row.geography_feature_id)) {
      throw new Error("Nevada " + yearPlan.year + " cannot resolve crosswalk UUIDs");
    }
    await query(tx, [
  ...
  if (["total", "reviewed"]
    ...
    throw new Error("Nevada " + yearPlan.year + " crosswalk count drifted");
  }
...
async function applyYear(tx, yearPlan, context) {
  ...
  const resultSourceId = await sourceDocument(tx, {
    ...
    title: "Nevada " + yearPlan.year + " presidential precinct results",
    ...
    confidence: "The plan retains each Nevada source's official, reconstructed, privacy-suppression, and temporal caveats; these precinct rows never replace certified county/state totals.",
    status: "loaded",
    metadata: {
  ...
  const geometrySourceId = await sourceDocument(tx, {
    ...
    title: "Nevada " + yearPlan.year + " precinct geometry package",
...
export async function applyNevadaPrecinctGisTransaction(
  ...
  const context = buildNevadaPrecinctExecutionContext(
  ...
  if (
    ...
    throw new Error(
      "Nevada precinct release transaction database identity or write mode drifted",
    );
  }
  ...
  if (context.mode === "production_release" && context.productionReplacement) {
    ...
    const existing = await query(tx, [
      ...
      "  where ru.state_code='NV' and ru.reporting_grain='precinct'",
    ...
    if (
      ...
      throw new Error(
        "Nevada production hidden load refuses existing precinct release rows; "
  ...
  for (const yearPlan of plan.years) {
    ...
    if (
      ...
      throw new Error("Intentional Nevada GIS rollback after " + yearPlan.year);
    }
  }
...
export async function applyNevadaPrecinctGisPlan(plan, options = {}) {
  if (getDatabaseDriver() !== "postgres") {
    throw new Error("Nevada local GIS setup requires CRM_DATABASE_DRIVER=postgres");
  }
  ...
  try {
    return await sql.begin((tx) => applyNevadaPrecinctGisTransaction(
      tx,
      plan,
      {
...
async function validateSourceDocumentRecords(sql, yearPlan, context) {
  ...
  if (rows.length !== 2) {
    throw new Error("Nevada " + yearPlan.year + " source-document count drifted");
  }
  ...
  if (
    ...
    throw new Error("Nevada " + yearPlan.year + " result provenance drifted");
  }
  ...
  if (
    ...
    throw new Error("Nevada " + yearPlan.year + " geometry provenance drifted");
  }
  return {
...
async function validateStoredProductionAudit(
  ...
  if (
    ...
    throw new Error("Nevada " + yearPlan.year + " reporting-unit release audit drifted");
  }
  ...
  if (
    ...
    throw new Error("Nevada " + yearPlan.year + " import-run release audit drifted");
  }
}
...
async function validateStoredGeometryRecords(sql, yearPlan, electionId, context) {
  ...
  if (
    ...
    throw new Error("Nevada " + yearPlan.year + " geography-version provenance drifted");
  }
  ...
  if (features.length !== expectedFeatures.size) {
    throw new Error("Nevada " + yearPlan.year + " exact feature count drifted");
  }
  ...
  for (const row of features) {
    ...
    assertNoElectionValueProperties(
      row.properties,
      "Nevada " + yearPlan.year + " stored feature " + sourceFeatureId,
    );
    if (
      !expected
      ...
      throw new Error("Nevada " + yearPlan.year + " stored feature drifted at " + sourceFeatureId);
    }
  ...
  if (crosswalks.length !== expectedCrosswalks.size) {
    throw new Error("Nevada " + yearPlan.year + " exact crosswalk count drifted");
  }
  ...
  for (const row of crosswalks) {
    ...
    if (
      ...
      throw new Error("Nevada " + yearPlan.year + " stored crosswalk drifted at " + reportingCode);
    }
...
export async function readNevadaPersistedProductionReleaseAudit(
  sql,
  releaseCandidate,
) {
  if (
    ...
    throw new Error("Nevada hidden receipt recovery requires an exact release candidate");
  }
  const rows = await query(sql, [
  ...
  if (
    ...
    throw new Error("Nevada hidden receipt recovery found an incomplete year set");
  }
  ...
  for (const row of rows) {
    if (
      ...
      throw new Error("Nevada hidden receipt recovery audit drifted across years");
    }
  }
  return audit;
}

export async function validateNevadaPrecinctGisClient(
  sql,
  plan,
  options = {},
) {
    ...
    for (const yearPlan of plan.years) {
      ...
      if (
        ...
        throw new Error("Nevada " + yearPlan.year + " database counts drifted: " + JSON.stringify({
          actual: row,
          expected: {
            reportingUnits: yearPlan.reportingUnits.length,
            resultRows: yearPlan.resultRows.length,
      ...
      if (Number(zero[0].count) !== yearPlan.zeroVoteUnits) {
        throw new Error("Nevada " + yearPlan.year + " zero-vote count drifted");
      }
      output.push({
        year: yearPlan.year,
        electionId: yearPlan.electionId,
    ...
    if (Number(invalid[0].count) !== 0) {
      throw new Error("Nevada precinct database has unvalidated public constraints");
    }
...
export async function validateNevadaPrecinctGisDatabase(plan, options = {}) {
  if (getDatabaseDriver() !== "postgres") {
    throw new Error("Nevada local GIS validation requires CRM_DATABASE_DRIVER=postgres");
  }
  ...
  try {
    return await validateNevadaPrecinctGisClient(sql, plan, {
      executionContext: { mode: "local" },
