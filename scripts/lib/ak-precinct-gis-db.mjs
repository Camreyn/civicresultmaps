...
const STATE = "AK";
const SETUP_PARSER = "alaskaPrecinctReportingGisSetup";
const APPLICATION_NAME = "civicresultmaps-local-ak-precinct-gis";

const LOCAL_EXECUTION_CONTEXT = Object.freeze({
  mode: "local",
  importParser: SETUP_PARSER,
  importScope: "local-only",
  resultParser: "scripts/setup-ak-precinct-gis-local.mjs",
  reviewedBy: "repository-reviewed",
  revisionReason: "Local-only Alaska reviewed precinct GIS setup",
  database: {
    environment: "local",
    host: "loopback",
    port: 54329,
...
function validatedProductionReleaseAudit(value) {
  if (!value || typeof value !== "object") {
    throw new Error("Production Alaska execution requires durable release-audit metadata");
  }
  const output = {};
  for (const key of PRODUCTION_RELEASE_AUDIT_ARTIFACT_KEYS) {
    ...
    if (
      ...
      throw new Error("Production Alaska execution release-audit metadata is invalid: " + key);
    }
  ...
  if (
    ...
    throw new Error("Production Alaska execution release-audit metadata is invalid");
  }
  ...
  if (Object.keys(value).length !== PRODUCTION_RELEASE_AUDIT_KEYS.length) {
    throw new Error("Production Alaska execution release-audit metadata has extra fields");
  }
  return Object.freeze(output);
}

export function buildAlaskaPrecinctExecutionContext(options = {}) {
  if (!options.mode || options.mode === "local") return LOCAL_EXECUTION_CONTEXT;
  if (options.mode !== "production_release") {
    throw new Error("Unknown Alaska precinct unit execution mode");
  }
  if (!/^[a-f0-9]{64}$/.test(options.releasePackageSha256 ?? "")) {
    throw new Error("Production Alaska execution requires a release-package SHA-256");
  }
  if (!/^ak-precinct-gis-four-election-v\d+$/.test(options.releaseCandidateId ?? "")) {
    throw new Error("Production Alaska execution requires the reviewed release-candidate ID");
  }
  if (typeof options.databaseName !== "string" || !options.databaseName.trim()) {
    throw new Error("Production Alaska execution requires the preflight database name");
  }
  return Object.freeze({
    mode: "production_release",
    importParser: "alaskaPrecinctGisProductionRelease",
    importScope: "production-release-hidden-load",
    resultParser: "scripts/apply-ak-precinct-release.mjs",
    reviewedBy: "repository-reviewed-release",
    revisionReason:
      "Alaska four-election precinct GIS hidden load "
...
export function assertAlaskaGeometryVersionPrecondition(versions, yearPlan) {
  if (
...
async function electionAndContest(tx, yearPlan) {
  ...
  const contests = await query(tx, [
    ...
    "returning id",
  ], [electionId, STATE, "Alaska " + yearPlan.year + " President"]);
...
async function reportingUnits(
  ...
  if (stored.length !== yearPlan.reportingUnits.length) {
    throw new Error("Alaska " + yearPlan.year + " reporting-unit count drifted");
  }
  const ids = new Map();
...
async function results(tx, yearPlan, contestId, importRunId, sourceDocumentId, unitIds) {
  for (const batch of chunks(yearPlan.resultRows)) {
    ...
    if (records.some((row) => !row.reporting_unit_id)) {
      throw new Error("Alaska " + yearPlan.year + " result lacks reporting unit");
    }
    await query(tx, [
  ...
  if (stored.length !== yearPlan.resultRows.length) {
    throw new Error("Alaska " + yearPlan.year + " local-result count drifted");
  }
  ...
  for (const row of stored) {
    ...
    if (
      ...
      throw new Error("Alaska " + yearPlan.year + " result drift at " + key);
    }
  }
}
...
async function geometry(
  ...
  if (yearPlan.geometry.disposition === "blocked") {
    ...
    if (Number(existing[0].count) !== 0) {
      throw new Error("Blocked Alaska " + yearPlan.year + " already has geometry");
    }
  ...
  assertAlaskaGeometryVersionPrecondition(versions, yearPlan);
  ...
  if (storedFeatures.length !== yearPlan.geometry.features.length) {
    throw new Error("Alaska " + yearPlan.year + " geography-feature count drifted");
  }
  ...
  for (const batch of chunks(yearPlan.geometry.crosswalks)) {
    ...
    if (records.some((row) => (
      ...
      throw new Error("Alaska " + yearPlan.year + " cannot resolve crosswalk UUIDs");
    }
    await query(tx, [
  ...
  if (["total", "reviewed"]
    ...
    throw new Error("Alaska " + yearPlan.year + " crosswalk count drifted");
  }
...
async function applyYear(tx, yearPlan, context) {
  ...
  const resultSourceId = await sourceDocument(tx, {
    ...
    title: "Alaska " + yearPlan.year + " presidential precinct results",
    ...
    confidence: "Official Alaska Division of Elections rows. Administrative absentee, early, questioned, and federal-overseas buckets are retained for reconciliation but are never assigned to precinct polygons.",
    status: "loaded",
    metadata: {
  ...
  const geometrySourceId = await sourceDocument(tx, {
    ...
    title: "Alaska " + yearPlan.year + " precinct geometry package",
...
export async function applyAlaskaPrecinctGisTransaction(
  tx,
  plan,
  options = {},
) {
  const context = buildAlaskaPrecinctExecutionContext(
  ...
  if (
    ...
    throw new Error(
      "Alaska precinct unit release transaction database identity or write mode drifted",
    );
  }
  ...
  if (context.mode === "production_release") {
    ...
    if (
      ...
      throw new Error(
        "Alaska production hidden load refuses existing precinct release rows; "
  ...
  for (const yearPlan of plan.years) {
    ...
    if (
      ...
      throw new Error("Intentional Alaska GIS rollback after " + yearPlan.year);
    }
  }
...
export async function applyAlaskaPrecinctGisPlan(plan, options = {}) {
  if (getDatabaseDriver() !== "postgres") {
    throw new Error("Alaska precinct GIS setup requires CRM_DATABASE_DRIVER=postgres");
  }
  ...
  try {
    return await sql.begin((tx) => applyAlaskaPrecinctGisTransaction(
      tx,
      plan,
      {
...
async function validateSourceDocumentRecords(sql, yearPlan, context) {
  ...
  if (rows.length !== 2) {
    throw new Error("Alaska " + yearPlan.year + " source-document count drifted");
  }
  ...
  if (
    ...
    throw new Error("Alaska " + yearPlan.year + " result provenance drifted");
  }
  ...
  if (
    ...
    throw new Error("Alaska " + yearPlan.year + " geometry provenance drifted");
  }
  return {
...
async function validateStoredProductionAudit(
  ...
  if (
    ...
    throw new Error("Alaska " + yearPlan.year + " reporting-unit release audit drifted");
  }
  ...
  if (
    ...
    throw new Error("Alaska " + yearPlan.year + " import-run release audit drifted");
  }
}
...
async function validateStoredGeometryRecords(sql, yearPlan, electionId, context) {
  ...
  if (
    ...
    throw new Error("Alaska " + yearPlan.year + " geography-version provenance drifted");
  }
  ...
  if (features.length !== expectedFeatures.size) {
    throw new Error("Alaska " + yearPlan.year + " exact feature count drifted");
  }
  ...
  for (const row of features) {
    ...
    assertNoElectionValueProperties(
      row.properties,
      "Alaska " + yearPlan.year + " stored feature " + sourceFeatureId,
    );
    if (
      !expected
      ...
      throw new Error("Alaska " + yearPlan.year + " stored feature drifted at " + sourceFeatureId);
    }
  ...
  if (crosswalks.length !== expectedCrosswalks.size) {
    throw new Error("Alaska " + yearPlan.year + " exact crosswalk count drifted");
  }
  ...
  for (const row of crosswalks) {
    ...
    if (
      ...
      throw new Error("Alaska " + yearPlan.year + " stored crosswalk drifted at " + reportingCode);
    }
...
export async function readAlaskaPersistedProductionReleaseAudit(
  sql,
  releaseCandidate,
) {
  if (
    ...
    throw new Error("Alaska hidden receipt recovery requires an exact release candidate");
  }
  const rows = await query(sql, [
  ...
  if (
    ...
    throw new Error("Alaska hidden receipt recovery found an incomplete year set");
  }
  ...
  for (const row of rows) {
    if (
      ...
      throw new Error("Alaska hidden receipt recovery audit drifted across years");
    }
  }
  return audit;
}

export async function validateAlaskaPrecinctGisClient(
  sql,
  plan,
  options = {},
) {
    ...
    for (const yearPlan of plan.years) {
      ...
      if (
        ...
        throw new Error("Alaska " + yearPlan.year + " database counts drifted: " + JSON.stringify({
          actual: row,
          expected: {
            reportingUnits: yearPlan.reportingUnits.length,
            resultRows: yearPlan.resultRows.length,
      ...
      for (const [name, votes] of Object.entries(expectedTotals)) {
        if (actualTotals[name] !== votes) {
          throw new Error("Alaska " + yearPlan.year + " " + name + " total drifted");
        }
      }
      if (Object.keys(actualTotals).length !== Object.keys(expectedTotals).length) {
        throw new Error("Alaska " + yearPlan.year + " candidate grouping drifted");
      }
      ...
      if (Number(zero[0].count) !== yearPlan.zeroVoteUnits) {
        throw new Error("Alaska " + yearPlan.year + " zero-vote count drifted");
      }
      output.push({
        year: yearPlan.year,
        electionId: yearPlan.electionId,
    ...
    if (Number(invalid[0].count) !== 0) {
      throw new Error("Alaska precinct unit database has unvalidated public constraints");
    }
...
export async function validateAlaskaPrecinctGisDatabase(plan, options = {}) {
  if (getDatabaseDriver() !== "postgres") {
    throw new Error("Alaska precinct GIS validation requires CRM_DATABASE_DRIVER=postgres");
  }
  ...
  try {
    return await validateAlaskaPrecinctGisClient(sql, plan, {
      executionContext: { mode: "local" },
