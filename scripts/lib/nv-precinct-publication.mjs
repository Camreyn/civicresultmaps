...
import { inspectNevadaPrecinctBlobPublicationPlan } from "./nv-precinct-blob-publication.mjs";
...
export const NEVADA_PUBLICATION_YEARS = Object.freeze([2016, 2020, 2024]);
export const NEVADA_PUBLICATION_SCOPES = Object.freeze([
  "publish_nv_geography_versions",
  "authorize_nv_precinct_results",
...
export function serializeNevadaPublicationDocument(value) {
...
function safeEvidence(root, relativePath, allowedRoot, expectedSha256) {
  if (
    ...
    throw new Error("Nevada publication evidence path is unsafe");
  }
  ...
  if (!absolutePath.startsWith(allowed + path.sep) || !existsSync(absolutePath)) {
    throw new Error("Nevada publication evidence is missing");
  }
  const bytes = readFileSync(absolutePath);
  const digest = sha256(bytes);
  if (digest !== expectedSha256) {
    throw new Error("Nevada publication evidence SHA-256 drifted");
  }
  return {
    path: relativePath,
    absolutePath,
...
function credentialFreeHttpsOrigin(value) {
  ...
  try {
    ...
    throw new Error("Nevada publication delivery origin is invalid");
  }
  if (
    ...
    throw new Error("Nevada publication origin must be credential-free HTTPS");
  }
...
function loadPackage(root, packagePath, packageSha256) {
  if (!/^[a-f0-9]{64}$/.test(packageSha256 ?? "")) {
    throw new Error("Nevada publication requires the exact package SHA-256");
  }
  ...
  if (
    ...
    || !semanticallyEqual(
      ...
      NEVADA_PUBLICATION_YEARS,
    )
  ) {
    throw new Error("Nevada publication release package is incompatible");
  }
  return {
    artifact,
    document,
...
function publicManifests(root, loaded) {
  ...
  if (inspection.errors.length) {
    throw new Error("Nevada publication registry is invalid: " + inspection.errors.join("; "));
  }
  const rows = registry.manifests.filter((manifest) => manifest?.state === "NV");
  if (rows.length !== 3) {
    throw new Error("Nevada publication registry does not contain three manifests");
  }
  return loaded.document.years.map((year) => {
    ...
    if (
      ...
      throw new Error("Nevada " + year.year + " public manifest drifted from the package");
    }
    return {
      year: year.year,
      electionId: year.electionId,
      manifestId: manifest.id,
      publicManifestSha256: sha256(serializeNevadaPublicationDocument(manifest)),
    ...
    if (new Set(all.map((row) => row.manifestId)).size !== 3) {
      throw new Error("Nevada publication manifest IDs are duplicated");
    }
    return item;
  });
}
...
export function validateNevadaHiddenLoadReceipt(value, context) {
  ...
  if (
    ...
    throw new Error("Nevada hidden-load receipt is incomplete or incompatible");
  }
  const totals = {
  ...
  for (const [key, expected] of Object.entries(EXPECTED_TOTALS)) {
    ...
    if (Number(totals[key]) !== expected) {
      throw new Error("Nevada hidden-load receipt total drifted: " + key);
    }
  }
  return {
...
export function validateNevadaBlobPublicationEvidence(value, blobPlan, now = Date.now()) {
  const origin = credentialFreeHttpsOrigin(value?.deliveryOrigin);
  if (
    value?.schemaVersion !== 1
    || value?.state !== "NV"
    || value?.purpose !== "nv-precinct-parent-scoped-immutable-geometry-publication"
    ...
    || value.artifacts.length !== 54
  ) {
    throw new Error("Nevada Blob publication evidence is incomplete or incompatible");
  }
  ...
  for (const artifact of value.artifacts) {
    ...
    try {
      ...
      throw new Error("Nevada Blob publication evidence contains an invalid URL");
    }
    if (
      !wanted
      ...
      throw new Error("Nevada Blob publication artifact set drifted");
    }
    seen.add(artifact.pathname);
  }
  if (seen.size !== expected.size) {
    throw new Error("Nevada Blob publication artifact set is incomplete");
  }
  return {
...
export function inspectNevadaPublicationPlan(options = {}) {
  ...
  const hidden = validateNevadaHiddenLoadReceipt(hiddenArtifact.value, {
    now,
  ...
  const blobArtifact = safeEvidence(
    ...
    ".etl/precinct-blob-publications/NV",
    options.blobEvidenceSha256,
  );
  const blobPlan = inspectNevadaPrecinctBlobPublicationPlan({
    root,
    packagePath: loaded.identity.path,
    packageSha256: loaded.identity.sha256,
  });
  const blob = validateNevadaBlobPublicationEvidence(blobArtifact.value, blobPlan, now);
  ...
  const bytes = serializeNevadaPublicationDocument(plan);
  return {
    root,
...
export function buildNevadaPublicationAuthorizationTemplate(plan, planSha256) {
  return {
    schemaVersion: 1,
    state: "NV",
    ...
    scopes: [...NEVADA_PUBLICATION_SCOPES],
...
export function validateNevadaPublicationAuthorization(value, context) {
  ...
  if (
    ...
    || !semanticallyEqual(value?.scopes, NEVADA_PUBLICATION_SCOPES)
    ...
    throw new Error("Nevada public authorization is absent, expired, or incompatible");
  }
  return {
