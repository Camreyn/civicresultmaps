...
export const ALASKA_PUBLIC_ACTIVATION_ROOT =
  ".etl/precinct-public-activations/AK";
export const ALASKA_PUBLIC_ACTIVATION_YEARS = Object.freeze([
  2012,
  2016,
  2020,
  2024,
...
export function serializeAlaskaPublicActivationDocument(value) {
...
function readRepositoryJson(root, relativePath) {
  ...
  if (!existsSync(absolutePath)) {
    throw new Error("Alaska public activation target is missing: " + relativePath);
  }
...
function loadReleasePackage(options) {
  if (!/^[a-f0-9]{64}$/.test(options.packageSha256 ?? "")) {
    throw new Error("Alaska public activation requires the exact package SHA-256");
  }
  ...
  if (
    ...
    || !semanticallyEqual(
      ...
      ALASKA_PUBLIC_ACTIVATION_YEARS,
    )
    || Number.isNaN(Date.parse(document?.preparedFromLocalValidationAt))
  ) {
    throw new Error("Alaska public-activation package contract is incompatible");
  }
  return {
    artifact,
    document,
...
function loadDraftManifests(root, loaded) {
  return loaded.document.years.map((year) => {
    ...
    if (
      ...
      throw new Error("Alaska " + year.year + " draft manifest is not public-eligible");
    }
    ...
    if (
      ...
      throw new Error("Alaska " + year.year + " canonical source preimage is not blocked");
    }
    return {
      year: year.year,
...
function updateRegistry(root, manifests, activatedAtUtc) {
  ...
  if (
    ...
    throw new Error("Alaska canonical manifest registry is invalid");
  }
  const existing = current.value.manifests.filter((row) => row?.state === "AK");
  ...
  if (existing.length === 0) {
    ...
    throw new Error("Alaska canonical registry is partially activated or drifted");
  }
  const value = {
    ...current.value,
  ...
  if (inspection.errors.length) {
    throw new Error(
      "Alaska activated registry is invalid: " + inspection.errors.join("; "),
    );
  }
  const eligible = inspection.manifests.filter((item, index) =>
    value.manifests[index]?.state === "AK"
    && item.publicEligibilityReasons.length === 0);
  if (eligible.length !== 4) {
    throw new Error("Alaska activated registry does not contain four eligible manifests");
  }
...
function alaskaCoverageRow(manifest, activatedAtUtc) {
  return {
    state: "AK",
    stateName: "Alaska",
    ...
    generalOfficialSourceLeads: [
      manifest.source.url,
      "https://sos.alaska.gov/alaskans/election-results-statistics",
    ],
    geometry: {
    ...
    notes: [
      manifest.election.year === 2012
        ? "Official Alaska results are joined to a commit-pinned public mirror of the April 5, 2012 amended-proclamation polygons. The secondary custody and lack of a formal license remain explicit caveats."
        : "Official Alaska Division of Elections result and redistricting artifacts supply the reviewed election-specific presentation geometry.",
      "Geographic precincts use exact retained identifiers. Absentee, early-voting, questioned-ballot, and federal-overseas buckets remain non-geographic and are never assigned to polygons.",
      "The map is parent-scoped by Alaska House District rather than county or county equivalent; election values are excluded from geometry delivery.",
...
function updateCoverage(root, manifest, relativePath, activatedAtUtc) {
  ...
  if (!Array.isArray(current.value?.states)) {
    throw new Error("Alaska coverage inventory is invalid: " + relativePath);
  }
  const existing = current.value.states.filter((row) => row?.state === "AK");
  let disposition;
  let states;
  if (existing.length === 0) {
    const expectedRow = alaskaCoverageRow(manifest, activatedAtUtc);
    disposition = "activate";
    ...
    if (
      ...
      throw new Error(relativePath + " contains a drifted Alaska row");
    }
    const expectedRow = {
      ...row,
    ...
    if (currentEligible === 1) {
      if (!semanticallyEqual(row, expectedRow)) {
        throw new Error(relativePath + " contains a partial Alaska activation");
      }
      disposition = "verified_existing";
      ...
      states = current.value.states.map((candidate) =>
        candidate?.state === "AK" ? expectedRow : candidate);
    }
  } else {
    throw new Error(relativePath + " contains a partial or drifted Alaska row");
  }
  const value = {
    ...current.value,
    updatedAt: disposition === "activate" ? activatedAtUtc : current.value.updatedAt,
...
function outputFile(relativePath, built) {
  const bytes = serializeAlaskaPublicActivationDocument(built.value);
  return {
    path: relativePath,
...
export function inspectAlaskaPublicActivationPlan(options = {}) {
  ...
  for (const [year, relativePath] of COVERAGE_PATHS) {
    ...
    if (!manifest) throw new Error("Alaska activation is missing year " + year);
  ...
  const plan = {
    schemaVersion: 1,
    id: "ak-precinct-public-activation-four-election-v1",
    state: "AK",
    ...
    manifests: manifests.map((item) => ({
      ...
      draftManifest: {
        ...
        publicManifestSha256: sha256(
          serializeAlaskaPublicActivationDocument(item.draft.manifest),
        ),
  ...
  const bytes = serializeAlaskaPublicActivationDocument(plan);
