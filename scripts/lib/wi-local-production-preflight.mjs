...
export function productionEndpointFingerprint(databaseUrl) {
  ...
  try {
    ...
    throw new Error("Wisconsin production preflight requires a valid PostgreSQL URL");
  }
  if (!/^postgres(?:ql)?:$/.test(parsed.protocol)) {
    throw new Error("Wisconsin production preflight requires a PostgreSQL URL");
  }
  if (LOOPBACK_HOSTS.has(parsed.hostname.toLowerCase())) {
    throw new Error("Wisconsin production preflight refuses a loopback database URL");
  }
  const databaseName = decodeURIComponent(parsed.pathname).replace(/^\//, "");
  if (!databaseName) {
    throw new Error("Wisconsin production preflight requires a database name");
  }
...
export function assertWisconsinReleaseCandidateDocument(
  document,
  packageSha256,
) {
  if (!/^[a-f0-9]{64}$/.test(packageSha256 ?? "")) {
    throw new Error("Wisconsin release package SHA-256 is invalid");
  }
  if (
    ...
    throw new Error("Wisconsin release candidate does not match the guarded contract");
  }
  ...
  for (const [key, value] of Object.entries(expectedTotals)) {
    if (Number(document?.totals?.[key]) !== value) {
      throw new Error("Wisconsin release candidate total drifted: " + key);
    }
  }
  ...
  if (
    ...
    throw new Error("Wisconsin release candidate migration pins are invalid");
  }
  const years = (document.years ?? []).map((year) => Number(year.year));
  if (JSON.stringify(years) !== JSON.stringify([2016, 2020, 2024])) {
    throw new Error("Wisconsin release candidate year set drifted");
  }
  return {
    id: document.id,
...
export function buildWisconsinProductionPreflightReport(snapshot, context) {
  if (String(snapshot.identity?.transaction_read_only) !== "on") {
    throw new Error("Wisconsin production preflight did not use a read-only transaction");
  }
  const schema = migrationState(snapshot.precinctTables, snapshot.precinctColumns);
  if (schema.status === "partial_blocked") {
    throw new Error("Wisconsin production schema is partially migrated; release is blocked");
  }
  const invalidConstraints = Number(snapshot.invalidConstraints ?? NaN);
  if (!Number.isInteger(invalidConstraints)) {
    throw new Error("Wisconsin production preflight constraint count is invalid");
  }
  ...
  if (migration0009.status === "partial_blocked") {
    throw new Error("Wisconsin production derivation-method constraint is ambiguous");
  }
  const report = {
    schemaVersion: 1,
    state: "WI",
    scope: "read-only Wisconsin local reporting unit GIS production preflight",
    ...
    stopConditions: [
      ...
      "any Wisconsin year or row set differs before the write transaction",
...
export async function collectWisconsinProductionPreflight(sql, context) {
  ...
  if (identityRows.length !== 1) {
    throw new Error("Wisconsin production preflight database identity is missing");
  }
  ...
  const coreYears = await query(sql, [
    ...
    "from elections e",
    "left join contests c on c.election_id=e.id and c.state_code='WI'",
    "left join result_rows rr on rr.contest_id=c.id and rr.state_code='WI'",
  ...
  const sourceDocuments = await query(sql, [
    ...
    "where state_code='WI' order by election_year,slug",
  ]);
  ...
  if (state.status === "complete") {
    localYears = await query(sql, [
      ...
      "  where ru.state_code='WI' and ru.election_id=e.id",
      ...
      "  where rr.state_code='WI' and c.election_id=e.id",
      ...
      "  where gv.state_code='WI' and gv.election_id=e.id",
      ...
      "  where gv.state_code='WI' and gv.election_id=e.id",
      "   and ru.election_id=e.id and x.review_status='reviewed'",
      "   and x.confidence='high') reviewed_relationships",
      "from elections e where e.office='president'",
      " and e.year in (2016,2020,2024) order by e.year",
    ]);
  }

  return buildWisconsinProductionPreflightReport({
...
export const wisconsinLocalSchemaContract = Object.freeze({
