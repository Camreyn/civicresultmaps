...
export function productionEndpointFingerprint(databaseUrl) {
  ...
  try {
    ...
    throw new Error("Texas production preflight requires a valid PostgreSQL URL");
  }
  if (!/^postgres(?:ql)?:$/.test(parsed.protocol)) {
    throw new Error("Texas production preflight requires a PostgreSQL URL");
  }
  if (LOOPBACK_HOSTS.has(parsed.hostname.toLowerCase())) {
    throw new Error("Texas production preflight refuses a loopback database URL");
  }
  const databaseName = decodeURIComponent(parsed.pathname).replace(/^\//, "");
  if (!databaseName) {
    throw new Error("Texas production preflight requires a database name");
  }
...
export function assertTexasReleaseCandidateDocument(
  document,
  packageSha256,
) {
  if (!/^[a-f0-9]{64}$/.test(packageSha256 ?? "")) {
    throw new Error("Texas release package SHA-256 is invalid");
  }
  if (
    ...
    throw new Error("Texas release candidate does not match the guarded contract");
  }
  ...
  for (const [key, value] of Object.entries(expectedTotals)) {
    if (Number(document?.totals?.[key]) !== value) {
      throw new Error("Texas release candidate total drifted: " + key);
    }
  }
  if (
    !/^[a-f0-9]{64}$/.test(
    ...
    throw new Error("Texas release candidate migration pin is invalid");
  }
  const years = (document.years ?? []).map((year) => Number(year.year));
  if (JSON.stringify(years) !== JSON.stringify([2012, 2016, 2020, 2024])) {
    throw new Error("Texas release candidate year set drifted");
  }
  return {
    id: document.id,
...
export function buildTexasProductionPreflightReport(snapshot, context) {
  if (String(snapshot.identity?.transaction_read_only) !== "on") {
    throw new Error("Texas production preflight did not use a read-only transaction");
  }
  const schema = migrationState(snapshot.precinctTables, snapshot.precinctColumns);
  if (schema.status === "partial_blocked") {
    throw new Error("Texas production schema is partially migrated; release is blocked");
  }
  const invalidConstraints = Number(snapshot.invalidConstraints ?? NaN);
  if (!Number.isInteger(invalidConstraints)) {
    throw new Error("Texas production preflight constraint count is invalid");
  }
  const report = {
    schemaVersion: 1,
    state: "TX",
    scope: "read-only Texas precinct GIS production preflight",
    ...
    stopConditions: [
      ...
      "any Texas year or row set differs before the write transaction",
...
export async function collectTexasProductionPreflight(sql, context) {
  ...
  if (identityRows.length !== 1) {
    throw new Error("Texas production preflight database identity is missing");
  }
  ...
  const coreYears = await query(sql, [
    ...
    "from elections e",
    "left join contests c on c.election_id=e.id and c.state_code='TX'",
    "left join result_rows rr on rr.contest_id=c.id and rr.state_code='TX'",
  ...
  const sourceDocuments = await query(sql, [
    ...
    "where state_code='TX' order by election_year,slug",
  ]);
  ...
  if (state.status === "complete") {
    precinctYears = await query(sql, [
      ...
      "  where gv.state_code='TX' and gv.election_id=e.id",
  ...
  return buildTexasProductionPreflightReport({
...
export const texasPrecinctSchemaContract = Object.freeze({
