...
export function productionEndpointFingerprint(databaseUrl) {
  ...
  try {
    ...
    throw new Error("Alaska production preflight requires a valid PostgreSQL URL");
  }
  if (!/^postgres(?:ql)?:$/.test(parsed.protocol)) {
    throw new Error("Alaska production preflight requires a PostgreSQL URL");
  }
  if (LOOPBACK_HOSTS.has(parsed.hostname.toLowerCase())) {
    throw new Error("Alaska production preflight refuses a loopback database URL");
  }
  const databaseName = decodeURIComponent(parsed.pathname).replace(/^\//, "");
  if (!databaseName) {
    throw new Error("Alaska production preflight requires a database name");
  }
...
export function assertAlaskaReleaseCandidateDocument(
  document,
  packageSha256,
) {
  if (!/^[a-f0-9]{64}$/.test(packageSha256 ?? "")) {
    throw new Error("Alaska release package SHA-256 is invalid");
  }
  if (
    ...
    throw new Error("Alaska release candidate does not match the guarded contract");
  }
  ...
  for (const [key, value] of Object.entries(expectedTotals)) {
    if (Number(document?.totals?.[key]) !== value) {
      throw new Error("Alaska release candidate total drifted: " + key);
    }
  }
  ...
  if (
    ...
    throw new Error("Alaska release candidate migration pins are invalid");
  }
  const years = (document.years ?? []).map((year) => Number(year.year));
  if (JSON.stringify(years) !== JSON.stringify([2012, 2016, 2020, 2024])) {
    throw new Error("Alaska release candidate year set drifted");
  }
  return {
    id: document.id,
...
export function buildAlaskaProductionPreflightReport(snapshot, context) {
  if (String(snapshot.identity?.transaction_read_only) !== "on") {
    throw new Error("Alaska production preflight did not use a read-only transaction");
  }
  const schema = migrationState(snapshot.precinctTables, snapshot.precinctColumns);
  if (schema.status === "partial_blocked") {
    throw new Error("Alaska production schema is partially migrated; release is blocked");
  }
  const invalidConstraints = Number(snapshot.invalidConstraints ?? NaN);
  if (!Number.isInteger(invalidConstraints)) {
    throw new Error("Alaska production preflight constraint count is invalid");
  }
  ...
  if (migration0009.status === "partial_blocked") {
    throw new Error("Alaska production derivation-method constraint is ambiguous");
  }
  const report = {
    schemaVersion: 1,
    state: "AK",
    scope: "read-only Alaska precinct unit GIS production preflight",
    ...
    stopConditions: [
      ...
      "any Alaska year or row set differs before the write transaction",
...
export async function collectAlaskaProductionPreflight(sql, context) {
  ...
  if (identityRows.length !== 1) {
    throw new Error("Alaska production preflight database identity is missing");
  }
  ...
  const coreYears = await query(sql, [
    ...
    "from elections e",
    "left join contests c on c.election_id=e.id and c.state_code='AK'",
    "left join result_rows rr on rr.contest_id=c.id and rr.state_code='AK'",
  ...
  const sourceDocuments = await query(sql, [
    ...
    "where state_code='AK' order by election_year,slug",
  ]);
  ...
  if (state.status === "complete") {
    localYears = await query(sql, [
      ...
      "  where ru.state_code='AK' and ru.election_id=e.id",
    "   and ru.reporting_grain in ('precinct','administrative_reporting_unit')) reporting_units,",
      ...
      "  where rr.state_code='AK' and c.election_id=e.id",
      ...
      "  where gv.state_code='AK' and gv.election_id=e.id",
  ...
  return buildAlaskaProductionPreflightReport({
...
export const alaskaPrecinctSchemaContract = Object.freeze({
