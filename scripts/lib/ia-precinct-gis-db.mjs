...
const STATE = "IA";
const SETUP_PARSER = "iowaLocalPrecinctGisSetup";
const APPLICATION_NAME = "civicresultmaps-local-ia-precinct-gis";

const LOCAL_EXECUTION_CONTEXT = Object.freeze({
  mode: "local",
  importParser: SETUP_PARSER,
  importScope: "local-only",
  resultParser: "scripts/setup-ia-precinct-gis-local.mjs",
  reviewedBy: "repository-reviewed",
  revisionReason: "Local-only Iowa reviewed precinct GIS setup",
  database: {
    environment: "local",
    host: "loopback",
    port: 54329,
...
function validatedProductionReleaseAudit(value) {
  if (!value || typeof value !== "object") {
    throw new Error("Production Iowa execution requires durable release-audit metadata");
  }
  const output = {};
  for (const key of PRODUCTION_RELEASE_AUDIT_ARTIFACT_KEYS) {
    ...
    if (
      ...
      throw new Error("Production Iowa execution release-audit metadata is invalid: " + key);
    }
  ...
  if (
    ...
    throw new Error("Production Iowa execution release-audit metadata is invalid");
  }
  ...
  if (Object.keys(value).length !== PRODUCTION_RELEASE_AUDIT_KEYS.length) {
    throw new Error("Production Iowa execution release-audit metadata has extra fields");
  }
  return Object.freeze(output);
}

export function buildIowaPrecinctExecutionContext(options = {}) {
  if (!options.mode || options.mode === "local") return LOCAL_EXECUTION_CONTEXT;
  if (options.mode !== "production_release") {
    throw new Error("Unknown Iowa precinct execution mode");
  }
  if (!/^[a-f0-9]{64}$/.test(options.releasePackageSha256 ?? "")) {
    throw new Error("Production Iowa execution requires a release-package SHA-256");
  }
  if (!/^ia-precinct-gis-three-election-v\d+$/.test(options.releaseCandidateId ?? "")) {
    throw new Error("Production Iowa execution requires the reviewed release-candidate ID");
  }
  if (typeof options.databaseName !== "string" || !options.databaseName.trim()) {
    throw new Error("Production Iowa execution requires the preflight database name");
  }
  return Object.freeze({
    mode: "production_release",
    importParser: "iowaPrecinctGisProductionRelease",
    importScope: "production-release-hidden-load",
    resultParser: "scripts/apply-ia-precinct-release.mjs",
    reviewedBy: "repository-reviewed-release",
    revisionReason:
      "Iowa three-election precinct GIS hidden load "
...
export function assertIowaGeometryVersionPrecondition(versions, yearPlan) {
  if (
...
async function electionAndContest(tx, yearPlan) {
  ...
  const contests = await query(tx, [
    ...
    "returning id",
  ], [electionId, STATE, "Iowa " + yearPlan.year + " President"]);
...
async function reportingUnits(
  ...
  if (stored.length !== yearPlan.reportingUnits.length) {
    throw new Error("Iowa " + yearPlan.year + " reporting-unit count drifted");
  }
  const ids = new Map();
...
async function results(tx, yearPlan, contestId, importRunId, sourceDocumentId, unitIds) {
  for (const batch of chunks(yearPlan.resultRows)) {
    ...
    if (records.some((row) => !row.reporting_unit_id)) {
      throw new Error("Iowa " + yearPlan.year + " result lacks reporting unit");
    }
    await query(tx, [
  ...
  if (stored.length !== yearPlan.resultRows.length) {
    throw new Error("Iowa " + yearPlan.year + " precinct-result count drifted");
  }
  ...
  for (const row of stored) {
    ...
    if (
      ...
      throw new Error("Iowa " + yearPlan.year + " result drift at " + key);
    }
  }
}
...
async function geometry(
  ...
  if (yearPlan.geometry.disposition === "blocked") {
    ...
    if (Number(existing[0].count) !== 0) {
      throw new Error("Blocked Iowa " + yearPlan.year + " already has geometry");
    }
  ...
  assertIowaGeometryVersionPrecondition(versions, yearPlan);
  ...
  if (storedFeatures.length !== yearPlan.geometry.features.length) {
    throw new Error("Iowa " + yearPlan.year + " geography-feature count drifted");
  }
  ...
  for (const batch of chunks(yearPlan.geometry.crosswalks)) {
    ...
    if (records.some((row) => !row.reporting_unit_id || !row.geography_feature_id)) {
      throw new Error("Iowa " + yearPlan.year + " cannot resolve crosswalk UUIDs");
    }
    await query(tx, [
  ...
  if (["total", "reviewed"]
    ...
    throw new Error("Iowa " + yearPlan.year + " crosswalk count drifted");
  }
...
async function applyYear(tx, yearPlan, context) {
  ...
  const resultSourceId = await sourceDocument(tx, {
    ...
    year: yearPlan.year,
    category: "Reviewed source-specific presidential precinct results",
    title: "Iowa " + yearPlan.year + " presidential precinct results",
    ...
    confidence: "The plan retains each Iowa source's official, reconstructed, privacy-suppression, and temporal caveats; these precinct rows never replace certified county/state totals.",
    status: "loaded",
    metadata: {
  ...
  const geometrySourceId = await sourceDocument(tx, {
    ...
    title: "Iowa " + yearPlan.year + " precinct geometry package",
    ...
    confidence: yearPlan.geometry.disposition === "blocked"
      ...
      : "Reviewed election-specific precinct package with the exact source-local crosswalk and retained provenance caveats.",
...
export async function applyIowaPrecinctGisTransaction(
  tx,
  plan,
  options = {},
) {
  const context = buildIowaPrecinctExecutionContext(
  ...
  if (
    ...
    throw new Error(
      "Iowa precinct release transaction database identity or write mode drifted",
    );
  }
  ...
  await query(
    tx,
    "select pg_advisory_xact_lock(hashtextextended('crm-ia-precinct-gis-release-v1',0))",
  );
  ...
  if (context.mode === "production_release") {
    ...
    const existing = await query(tx, [
      ...
      "  where ru.state_code='IA' and ru.reporting_grain='precinct'",
      ...
      "  where rr.state_code='IA' and rr.level='precinct'",
    ...
    if (
      ...
      throw new Error(
        "Iowa production hidden load refuses existing precinct release rows; "
  ...
  for (const yearPlan of plan.years) {
    ...
    if (
      ...
      throw new Error("Intentional Iowa GIS rollback after " + yearPlan.year);
    }
  }
...
export async function applyIowaPrecinctGisPlan(plan, options = {}) {
  if (getDatabaseDriver() !== "postgres") {
    throw new Error("Iowa local GIS setup requires CRM_DATABASE_DRIVER=postgres");
  }
  ...
  try {
    return await sql.begin((tx) => applyIowaPrecinctGisTransaction(
      tx,
      plan,
      {
...
async function validateSourceDocumentRecords(sql, yearPlan, context) {
  ...
  if (rows.length !== 2) {
    throw new Error("Iowa " + yearPlan.year + " source-document count drifted");
  }
  ...
  if (
    ...
    throw new Error("Iowa " + yearPlan.year + " result provenance drifted");
  }
  ...
  if (
    ...
    throw new Error("Iowa " + yearPlan.year + " geometry provenance drifted");
  }
  return {
...
async function validateStoredProductionAudit(
  ...
  if (
    ...
    throw new Error("Iowa " + yearPlan.year + " reporting-unit release audit drifted");
  }
  ...
  if (
    ...
    throw new Error("Iowa " + yearPlan.year + " import-run release audit drifted");
  }
}
...
async function validateStoredGeometryRecords(sql, yearPlan, electionId, context) {
  ...
  if (
    ...
    throw new Error("Iowa " + yearPlan.year + " geography-version provenance drifted");
  }
  ...
  if (features.length !== expectedFeatures.size) {
    throw new Error("Iowa " + yearPlan.year + " exact feature count drifted");
  }
  ...
  for (const row of features) {
    ...
    assertNoElectionValueProperties(
      row.properties,
      "Iowa " + yearPlan.year + " stored feature " + sourceFeatureId,
    );
    if (
      !expected
      ...
      throw new Error("Iowa " + yearPlan.year + " stored feature drifted at " + sourceFeatureId);
    }
  ...
  if (crosswalks.length !== expectedCrosswalks.size) {
    throw new Error("Iowa " + yearPlan.year + " exact crosswalk count drifted");
  }
  ...
  for (const row of crosswalks) {
    ...
    if (
      ...
      throw new Error("Iowa " + yearPlan.year + " stored crosswalk drifted at " + reportingCode);
    }
...
export async function readIowaPersistedProductionReleaseAudit(
  sql,
  releaseCandidate,
) {
  if (
    ...
    throw new Error("Iowa hidden receipt recovery requires an exact release candidate");
  }
  const rows = await query(sql, [
  ...
  if (
    ...
    throw new Error("Iowa hidden receipt recovery found an incomplete year set");
  }
  ...
  for (const row of rows) {
    if (
      ...
      throw new Error("Iowa hidden receipt recovery audit drifted across years");
    }
  }
  return audit;
}

export async function validateIowaPrecinctGisClient(
  sql,
  plan,
  options = {},
) {
    ...
    for (const yearPlan of plan.years) {
      ...
      if (
        ...
        throw new Error("Iowa " + yearPlan.year + " database counts drifted: " + JSON.stringify({
          actual: row,
          expected: {
            reportingUnits: yearPlan.reportingUnits.length,
            resultRows: yearPlan.resultRows.length,
      ...
      if (!democraticName || !republicanName) {
        throw new Error("Iowa " + yearPlan.year + " major-party candidate grouping drifted");
      }
      ...
      for (const [name, votes] of Object.entries(expectedTotals)) {
        if (actualTotals[name] !== votes) {
          throw new Error("Iowa " + yearPlan.year + " " + name + " total drifted");
        }
      }
      if (Object.keys(actualTotals).length !== Object.keys(expectedTotals).length) {
        throw new Error("Iowa " + yearPlan.year + " candidate grouping drifted");
      }
      ...
      if (Number(zero[0].count) !== yearPlan.zeroVoteUnits) {
        throw new Error("Iowa " + yearPlan.year + " zero-vote count drifted");
      }
      output.push({
        year: yearPlan.year,
        electionId: yearPlan.electionId,
    ...
    if (Number(invalid[0].count) !== 0) {
      throw new Error("Iowa precinct database has unvalidated public constraints");
    }
...
export async function validateIowaPrecinctGisDatabase(plan, options = {}) {
  if (getDatabaseDriver() !== "postgres") {
    throw new Error("Iowa local GIS validation requires CRM_DATABASE_DRIVER=postgres");
  }
  ...
  try {
    return await validateIowaPrecinctGisClient(sql, plan, {
      executionContext: { mode: "local" },
