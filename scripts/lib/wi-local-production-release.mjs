...
export const WISCONSIN_PRODUCTION_RELEASE_SCOPES = Object.freeze([
  "apply_migration_0009",
  "load_wi_local_reporting_results_and_geometry_hidden",
  "increment_public_data_revision",
]);
export const WISCONSIN_MAX_RELEASE_EVIDENCE_AGE_MS = 4 * 60 * 60 * 1000;
...
function expectedScopes() {
  return [...WISCONSIN_PRODUCTION_RELEASE_SCOPES];
}

export function buildWisconsinProductionAuthorizationTemplate(context) {
  return {
    schemaVersion: 1,
    state: "WI",
    ...
    acknowledgement:
      "This template is not authorization. GO_PRODUCTION authorizes only migration 0009, the hidden Wisconsin load, and one public revision increment; public geometry and result visibility remain blocked.",
  };
}

export function validateWisconsinProductionPreflightEvidence(report, context) {
  const targetYears = [2016, 2020, 2024];
  const localYearRows = report?.wisconsin?.localYearRows;
  const coreYearRows = report?.wisconsin?.coreYearRows;
  ...
  if (
    ...
    throw new Error(
      "Wisconsin production preflight evidence is incompatible or already contains local reporting release rows",
    );
  }
  const age = context.now.getTime() - Date.parse(report.capturedAtUtc);
  if (age < 0 || age > WISCONSIN_MAX_RELEASE_EVIDENCE_AGE_MS) {
    throw new Error("Wisconsin production preflight is outside the four-hour release window");
  }
  if (typeof report.database.name !== "string" || !report.database.name.trim()) {
    throw new Error("Wisconsin production preflight database name is missing");
  }
  return {
...
export function validateWisconsinProductionBackupEvidence(manifest, context) {
  const restore = manifest?.restoreVerification;
  if (
    !Number.isInteger(manifest?.manifestVersion)
    || manifest.manifestVersion < 3
    || manifest?.backupPurpose !== "wi-local-production-release-rollback"
    ...
    throw new Error("Wisconsin production backup evidence is incomplete or incompatible");
  }
  ...
  if (
    ...
    || restored < created
    || now - created < 0
    || now - created > WISCONSIN_MAX_RELEASE_EVIDENCE_AGE_MS
    || now - restored < 0
    || now - restored > WISCONSIN_MAX_RELEASE_EVIDENCE_AGE_MS
  ) {
    throw new Error("Wisconsin backup must follow the fresh preflight and restore within four hours");
  }
  ...
  if (
    ...
    throw new Error("Wisconsin production backup exact table or row-count proof drifted");
  }
  return {
...
export function validateWisconsinProductionAuthorization(value, context) {
  if (
    ...
    throw new Error("Wisconsin production authorization is incomplete or incompatible");
  }
  ...
  if (authorized > now || expires <= now || expires <= authorized) {
    throw new Error("Wisconsin production authorization is not active");
  }
  return {
