...
import { inspectWisconsinLocalBlobPublicationPlan } from "./wi-local-blob-publication.mjs";
import { inspectReleaseArtifact } from "./wi-local-release-candidate.mjs";
import {
  inspectPrecinctGeometryManifest,
  inspectPrecinctGeometryRegistry,
} from "../../src/lib/precinct-geography.ts";

export const WISCONSIN_PUBLICATION_YEARS = Object.freeze([2016, 2020, 2024]);
export const WISCONSIN_PUBLICATION_SCOPES = Object.freeze([
  "publish_wi_local_geography_versions",
  "authorize_wi_local_results",
  "increment_public_data_revision",
]);
export const WISCONSIN_PUBLIC_ROLLBACK_SCOPES = Object.freeze([
  "block_wi_local_geography_versions",
  "revoke_wi_local_results",
  "increment_public_data_revision",
]);
...
export function serializeWisconsinPublicationDocument(value) {
...
function safeEvidence(root, relativePath, allowedRoot, expectedSha256) {
  if (
    ...
    throw new Error("Wisconsin publication evidence path is unsafe");
  }
  ...
  if (!absolutePath.startsWith(allowed + path.sep) || !existsSync(absolutePath)) {
    throw new Error("Wisconsin publication evidence is missing");
  }
  const bytes = readFileSync(absolutePath);
  const digest = sha256(bytes);
  if (digest !== expectedSha256) {
    throw new Error("Wisconsin publication evidence SHA-256 drifted");
  }
  return {
    path: relativePath,
    absolutePath,
...
function credentialFreeHttpsOrigin(value) {
  ...
  try {
    ...
    throw new Error("Wisconsin publication delivery origin is invalid");
  }
  if (
    ...
    throw new Error("Wisconsin publication origin must be credential-free HTTPS");
  }
...
function loadPackage(root, packagePath, packageSha256) {
  if (!/^[a-f0-9]{64}$/.test(packageSha256 ?? "")) {
    throw new Error("Wisconsin publication requires the exact package SHA-256");
  }
  ...
  if (
    ...
    || document?.id !== "wi-local-reporting-gis-three-election-v1"
    || document?.state !== "WI"
    ...
    || !semanticallyEqual(
      ...
      WISCONSIN_PUBLICATION_YEARS,
    )
  ) {
    throw new Error("Wisconsin publication release package is incompatible");
  }
  return {
    artifact,
    document,
...
function publicManifests(root, loaded) {
  ...
  if (inspection.errors.length) {
    throw new Error("Wisconsin publication registry is invalid: " + inspection.errors.join("; "));
  }
  const rows = registry.manifests.filter((manifest) => manifest?.state === "WI");
  if (rows.length !== 3) {
    throw new Error("Wisconsin publication registry does not contain three manifests");
  }
  return loaded.document.years.map((year) => {
    ...
    if (
      ...
      || manifest.delivery?.parentCount !== 72
    ) {
      throw new Error("Wisconsin " + year.year + " public manifest drifted from the package");
    }
    return {
      year: year.year,
      electionId: year.electionId,
      manifestId: manifest.id,
      publicManifestSha256: sha256(serializeWisconsinPublicationDocument(manifest)),
    ...
    if (new Set(all.map((row) => row.manifestId)).size !== 3) {
      throw new Error("Wisconsin publication manifest IDs are duplicated");
    }
    return item;
  });
}
...
export function validateWisconsinHiddenLoadReceipt(value, context) {
  ...
  if (
    ...
    throw new Error("Wisconsin hidden-load receipt is incomplete or incompatible");
  }
  const totals = {
  ...
  for (const [key, expected] of Object.entries(EXPECTED_TOTALS)) {
    ...
    if (Number(totals[key]) !== expected) {
      throw new Error("Wisconsin hidden-load receipt total drifted: " + key);
    }
  }
  return {
...
export function validateWisconsinBlobPublicationEvidence(value, blobPlan, now = Date.now()) {
  const origin = credentialFreeHttpsOrigin(value?.deliveryOrigin);
  if (
    value?.schemaVersion !== 1
    || value?.state !== "WI"
    || value?.purpose !== "wi-local-reporting-parent-scoped-immutable-geometry-publication"
    ...
    || value?.assetCount !== 219
    || value?.canonicalManifestChanged !== false
    || value?.publicEligibilityChanged !== false
    || !Array.isArray(value?.artifacts)
    || value.artifacts.length !== 219
  ) {
    throw new Error("Wisconsin Blob publication evidence is incomplete or incompatible");
  }
  ...
  for (const artifact of value.artifacts) {
    ...
    try {
      ...
      throw new Error("Wisconsin Blob publication evidence contains an invalid URL");
    }
    if (
      !wanted
      ...
      throw new Error("Wisconsin Blob publication artifact set drifted");
    }
    seen.add(artifact.pathname);
  }
  if (seen.size !== expected.size) {
    throw new Error("Wisconsin Blob publication artifact set is incomplete");
  }
  return {
...
export function inspectWisconsinPublicationPlan(options = {}) {
  ...
  const hidden = validateWisconsinHiddenLoadReceipt(hiddenArtifact.value, {
    now,
  ...
  const blobArtifact = safeEvidence(
    ...
    ".etl/precinct-blob-publications/WI",
    options.blobEvidenceSha256,
  );
  const blobPlan = inspectWisconsinLocalBlobPublicationPlan({
    root,
    packagePath: loaded.identity.path,
    packageSha256: loaded.identity.sha256,
  });
  const blob = validateWisconsinBlobPublicationEvidence(blobArtifact.value, blobPlan, now);
  ...
  const bytes = serializeWisconsinPublicationDocument(plan);
  return {
    root,
...
export function buildWisconsinPublicationAuthorizationTemplate(plan, planSha256) {
  return {
    schemaVersion: 1,
    state: "WI",
    ...
    scopes: [...WISCONSIN_PUBLICATION_SCOPES],
...
export function validateWisconsinPublicationAuthorization(value, context) {
  ...
  if (
    ...
    || !semanticallyEqual(value?.scopes, WISCONSIN_PUBLICATION_SCOPES)
    ...
    throw new Error("Wisconsin public authorization is absent, expired, or incompatible");
  }
  return {
...
export function buildWisconsinPublicRollbackAuthorizationTemplate(
  plan,
  planSha256,
  publicationReceipt,
) {
  return {
    ...
    scopes: [...WISCONSIN_PUBLIC_ROLLBACK_SCOPES],
    rollbackWindow: {
      startsAtUtc: null,
      endsAtUtc: null,
    },
...
export function validateWisconsinPublicRollbackAuthorization(value, context) {
  const now = context.now ?? Date.now();
  const authorizedAt = Date.parse(value?.authorizedAtUtc);
  const expiresAt = Date.parse(value?.expiresAtUtc);
  ...
  if (
    ...
    || !semanticallyEqual(value?.scopes, WISCONSIN_PUBLIC_ROLLBACK_SCOPES)
    ...
    throw new Error("Wisconsin rollback authorization is absent, expired, or incompatible");
  }
  return {
