...
const STATE = "MN";
const SETUP_PARSER = "minnesotaLocalPrecinctGisSetup";
const APPLICATION_NAME = "civicresultmaps-local-mn-precinct-gis";

const LOCAL_EXECUTION_CONTEXT = Object.freeze({
  mode: "local",
  importParser: SETUP_PARSER,
  importScope: "local-only",
  resultParser: "scripts/setup-mn-precinct-gis-local.mjs",
  reviewedBy: "repository-reviewed",
  revisionReason: "Local-only Minnesota four-election precinct GIS setup",
  database: {
    environment: "local",
    host: "loopback",
    port: 54329,
...
export function buildMinnesotaPrecinctExecutionContext(options = {}) {
  ...
  if (options.mode !== "production_release") {
    throw new Error("Unknown Minnesota precinct execution mode");
  }
  if (!/^[a-f0-9]{64}$/.test(options.releasePackageSha256 ?? "")) {
    throw new Error("Production Minnesota execution requires a release-package SHA-256");
  }
  if (!/^mn-precinct-gis-four-election-v\d+$/.test(options.releaseCandidateId ?? "")) {
    throw new Error("Production Minnesota execution requires the reviewed release-candidate ID");
  }
  if (typeof options.databaseName !== "string" || !options.databaseName.trim()) {
    throw new Error("Production Minnesota execution requires the preflight database name");
  }
  return Object.freeze({
    mode: "production_release",
    importParser: "minnesotaPrecinctGisProductionRelease",
    importScope: "production-release-hidden-load",
    resultParser: "scripts/apply-mn-precinct-release.mjs",
    reviewedBy: "repository-reviewed-release",
    revisionReason:
      "Minnesota four-election precinct GIS hidden load "
...
async function electionAndContest(tx, yearPlan) {
  ...
  const contests = await query(tx, [
    ...
    "returning id",
  ], [electionId, STATE, "Minnesota " + yearPlan.year + " President"]);
...
async function reportingUnits(
  ...
  if (stored.length !== yearPlan.reportingUnits.length) {
    throw new Error("Minnesota " + yearPlan.year + " reporting-unit count drifted");
  }
  const ids = new Map();
...
async function results(tx, yearPlan, contestId, importRunId, sourceDocumentId, unitIds) {
  for (const batch of chunks(yearPlan.resultRows)) {
    ...
    if (records.some((row) => !row.reporting_unit_id)) {
      throw new Error("Minnesota " + yearPlan.year + " result lacks reporting unit");
    }
    await query(tx, [
  ...
  if (stored.length !== yearPlan.resultRows.length) {
    throw new Error("Minnesota " + yearPlan.year + " precinct-result count drifted");
  }
  ...
  for (const row of stored) {
    ...
    if (
      ...
      throw new Error("Minnesota " + yearPlan.year + " result drift at " + key);
    }
  }
}
...
async function geometry(
  ...
  if (yearPlan.geometry.disposition === "blocked") {
    ...
    if (Number(existing[0].count) !== 0) {
      throw new Error("Blocked Minnesota " + yearPlan.year + " already has geometry");
    }
  ...
  if (versions.some((row) => row.manifest_id !== yearPlan.manifest.id)) {
    throw new Error("Minnesota " + yearPlan.year + " has a different geometry manifest");
  }
  ...
  if (storedFeatures.length !== yearPlan.geometry.features.length) {
    throw new Error("Minnesota " + yearPlan.year + " geography-feature count drifted");
  }
  ...
  for (const batch of chunks(yearPlan.geometry.crosswalks)) {
    ...
    if (records.some((row) => !row.reporting_unit_id || !row.geography_feature_id)) {
      throw new Error("Minnesota " + yearPlan.year + " cannot resolve crosswalk UUIDs");
    }
    await query(tx, [
  ...
  const counts = await query(tx, [
    ...
    " count(*) filter (where match_method='exact_official_id')::int exact_id,",
  ...
  if (["total", "one_to_one", "exact_id", "reviewed", "high"]
    ...
    throw new Error("Minnesota " + yearPlan.year + " crosswalk count drifted");
  }
...
async function applyYear(tx, yearPlan, context) {
  ...
  const resultSourceId = await sourceDocument(tx, {
    ...
    year: yearPlan.year,
    category: "Official certified presidential precinct results",
    title: "Minnesota " + yearPlan.year + " certified presidential precinct results",
    ...
    confidence: "Official certified/recount-inclusive VTDID-keyed Minnesota SOS workbook.",
    status: "loaded",
    metadata: {
  ...
  const geometrySourceId = await sourceDocument(tx, {
    ...
    title: "Minnesota " + yearPlan.year + " precinct geometry package",
    ...
    confidence: yearPlan.geometry.disposition === "blocked"
      ...
      : "Reviewed election-date-confirmed official export with exact VTDID crosswalk.",
...
export async function applyMinnesotaPrecinctGisTransaction(
  tx,
  plan,
  options = {},
) {
  ...
  if (
    ...
    throw new Error(
      "Minnesota precinct release transaction database identity or write mode drifted",
    );
  }
  ...
  for (const yearPlan of plan.years) {
    ...
    if (
      ...
      throw new Error("Intentional Minnesota GIS rollback after " + yearPlan.year);
    }
  }
...
export async function applyMinnesotaPrecinctGisPlan(plan, options = {}) {
  if (getDatabaseDriver() !== "postgres") {
    throw new Error("Minnesota local GIS setup requires CRM_DATABASE_DRIVER=postgres");
  }
  ...
  try {
    return await sql.begin((tx) => applyMinnesotaPrecinctGisTransaction(
      tx,
      plan,
      {
...
async function validateSourceDocumentRecords(sql, yearPlan, context) {
  ...
  if (rows.length !== 2) {
    throw new Error("Minnesota " + yearPlan.year + " source-document count drifted");
  }
...
async function validateStoredGeometryRecords(sql, yearPlan, electionId, context) {
  ...
  if (
    ...
    throw new Error("Minnesota " + yearPlan.year + " geography-version provenance drifted");
  }
  ...
  if (features.length !== expectedFeatures.size) {
    throw new Error("Minnesota " + yearPlan.year + " exact feature count drifted");
  }
  ...
  for (const row of features) {
    ...
    assertNoElectionValueProperties(
      row.properties,
      "Minnesota " + yearPlan.year + " stored feature " + sourceFeatureId,
    );
    if (
      !expected
      ...
      throw new Error("Minnesota " + yearPlan.year + " stored feature drifted at " + sourceFeatureId);
    }
  ...
  if (crosswalks.length !== expectedCrosswalks.size) {
    throw new Error("Minnesota " + yearPlan.year + " exact crosswalk count drifted");
  }
  ...
  options = {},
    ...
    for (const yearPlan of plan.years) {
      const rows = await query(sql, [
        ...
        "   and rr.jurisdiction_code=concat('reporting:',$1::text,':',$4::text,':precinct:',",
        "    ru.parent_geoid,':',ru.source_unit_id)) same_year_result_rows,",
        ...
        "   and gv.metadata->>'manifestId'=$5::text and gv.metadata->>'manifestSha256'=$6::text",
      ...
      if (
        ...
        throw new Error("Minnesota " + yearPlan.year + " database counts drifted");
      }
      ...
      for (const [name, votes] of Object.entries(expectedTotals)) {
        if (actualTotals[name] !== votes) {
          throw new Error("Minnesota " + yearPlan.year + " " + name + " total drifted");
        }
      }
      if (Object.keys(actualTotals).length !== 3) {
        throw new Error("Minnesota " + yearPlan.year + " candidate grouping drifted");
      }
      ...
      if (Number(zero[0].count) !== yearPlan.zeroVoteUnits) {
        throw new Error("Minnesota " + yearPlan.year + " zero-vote count drifted");
      }
      output.push({
        year: yearPlan.year,
        electionId: yearPlan.electionId,
    ...
    if (Number(invalid[0].count) !== 0) {
      throw new Error("Minnesota precinct database has unvalidated public constraints");
    }
...
export async function validateMinnesotaPrecinctGisDatabase(plan, options = {}) {
  if (getDatabaseDriver() !== "postgres") {
    throw new Error("Minnesota local GIS validation requires CRM_DATABASE_DRIVER=postgres");
  }
  ...
  try {
    return await validateMinnesotaPrecinctGisClient(sql, plan, {
      executionContext: { mode: "local" },
