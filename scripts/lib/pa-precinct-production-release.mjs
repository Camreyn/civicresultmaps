...
export const PENNSYLVANIA_PRODUCTION_RELEASE_SCOPES = Object.freeze([
  "apply_migration_0009",
  "load_pa_precinct_results_and_geometry_hidden",
  "increment_public_data_revision",
]);
export const PENNSYLVANIA_MAX_RELEASE_EVIDENCE_AGE_MS = 4 * 60 * 60 * 1000;
...
function expectedScopes() {
  return [...PENNSYLVANIA_PRODUCTION_RELEASE_SCOPES];
}

export function buildPennsylvaniaProductionAuthorizationTemplate(context) {
  return {
    schemaVersion: 1,
    state: "PA",
    ...
    acknowledgement:
      "This template is not authorization. GO_PRODUCTION authorizes only migration 0009, the hidden Pennsylvania load, and one public revision increment; public geometry and result visibility remain blocked.",
  };
}

export function validatePennsylvaniaProductionPreflightEvidence(report, context) {
  const targetYears = [2016, 2020];
  const localYearRows = report?.pennsylvania?.localYearRows;
  const coreYearRows = report?.pennsylvania?.coreYearRows;
  ...
  if (
    ...
    throw new Error(
      "Pennsylvania production preflight evidence is incompatible or already contains precinct release rows",
    );
  }
  const age = context.now.getTime() - Date.parse(report.capturedAtUtc);
  if (age < 0 || age > PENNSYLVANIA_MAX_RELEASE_EVIDENCE_AGE_MS) {
    throw new Error("Pennsylvania production preflight is outside the four-hour release window");
  }
  if (typeof report.database.name !== "string" || !report.database.name.trim()) {
    throw new Error("Pennsylvania production preflight database name is missing");
  }
  return {
...
export function validatePennsylvaniaProductionBackupEvidence(manifest, context) {
  const restore = manifest?.restoreVerification;
  if (
    !Number.isInteger(manifest?.manifestVersion)
    || manifest.manifestVersion < 3
    || manifest?.backupPurpose !== "pa-precinct-production-release-rollback"
    ...
    throw new Error("Pennsylvania production backup evidence is incomplete or incompatible");
  }
  ...
  if (
    ...
    || restored < created
    || now - created < 0
    || now - created > PENNSYLVANIA_MAX_RELEASE_EVIDENCE_AGE_MS
    || now - restored < 0
    || now - restored > PENNSYLVANIA_MAX_RELEASE_EVIDENCE_AGE_MS
  ) {
    throw new Error("Pennsylvania backup must follow the fresh preflight and restore within four hours");
  }
  ...
  if (
    ...
    throw new Error("Pennsylvania production backup exact table or row-count proof drifted");
  }
  return {
...
export function validatePennsylvaniaProductionAuthorization(value, context) {
  if (
    ...
    throw new Error("Pennsylvania production authorization is incomplete or incompatible");
  }
  ...
  if (authorized > now || expires <= now || expires <= authorized) {
    throw new Error("Pennsylvania production authorization is not active");
  }
  return {
