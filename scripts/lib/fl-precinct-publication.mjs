...
import { inspectFloridaPrecinctBlobPublicationPlan } from "./fl-precinct-blob-publication.mjs";
...
export const FLORIDA_PUBLICATION_YEARS = Object.freeze([2016, 2020, 2024]);
export const FLORIDA_PUBLICATION_SCOPES = Object.freeze([
  "publish_fl_precinct_geography_versions",
  "authorize_fl_precinct_results",
  "increment_public_data_revision",
]);
export const FLORIDA_PUBLIC_ROLLBACK_SCOPES = Object.freeze([
  "block_fl_precinct_geography_versions",
  "revoke_fl_precinct_results",
  "increment_public_data_revision",
]);

const EXPECTED_TOTALS = Object.freeze({
  reportingUnits: 17_424,
  candidateResultRows: 52_272,
  geometryFeatures: 17_555,
  reviewedRelationships: 17_424,
  zeroVoteUnits: 140,
  sourceDocuments: 6,
  importRuns: 3,
});
...
export function serializeFloridaPublicationDocument(value) {
...
function safeEvidence(root, relativePath, allowedRoot, expectedSha256) {
  if (
    ...
    throw new Error("Florida publication evidence path is unsafe");
  }
  ...
  if (!absolutePath.startsWith(allowed + path.sep) || !existsSync(absolutePath)) {
    throw new Error("Florida publication evidence is missing");
  }
  const bytes = readFileSync(absolutePath);
  const digest = sha256(bytes);
  if (digest !== expectedSha256) {
    throw new Error("Florida publication evidence SHA-256 drifted");
  }
  return {
    path: relativePath,
    absolutePath,
...
function credentialFreeHttpsOrigin(value) {
  ...
  try {
    ...
    throw new Error("Florida publication delivery origin is invalid");
  }
  if (
    ...
    throw new Error("Florida publication origin must be credential-free HTTPS");
  }
...
function loadPackage(root, packagePath, packageSha256) {
  if (!/^[a-f0-9]{64}$/.test(packageSha256 ?? "")) {
    throw new Error("Florida publication requires the exact package SHA-256");
  }
  ...
  if (
    ...
    || !semanticallyEqual(
      ...
      FLORIDA_PUBLICATION_YEARS,
    )
  ) {
    throw new Error("Florida publication release package is incompatible");
  }
  return {
    artifact,
    document,
...
function publicManifests(root, loaded) {
  ...
  if (inspection.errors.length) {
    throw new Error("Florida publication registry is invalid: " + inspection.errors.join("; "));
  }
  const rows = registry.manifests.filter((manifest) => manifest?.state === "FL");
  if (rows.length !== 3) {
    throw new Error("Florida publication registry does not contain three manifests");
  }
  return loaded.document.years.map((year) => {
    ...
    if (
      ...
      throw new Error("Florida " + year.year + " public manifest drifted from the package");
    }
    return {
      year: year.year,
      electionId: year.electionId,
      manifestId: manifest.id,
      publicManifestSha256: sha256(serializeFloridaPublicationDocument(manifest)),
    ...
    if (new Set(all.map((row) => row.manifestId)).size !== 3) {
      throw new Error("Florida publication manifest IDs are duplicated");
    }
    return item;
  });
}
...
export function validateFloridaHiddenLoadReceipt(value, context) {
  ...
  if (
    ...
    throw new Error("Florida hidden-load receipt is incomplete or incompatible");
  }
  const totals = {
  ...
  for (const [key, expected] of Object.entries(EXPECTED_TOTALS)) {
    ...
    if (Number(totals[key]) !== expected) {
      throw new Error("Florida hidden-load receipt total drifted: " + key);
    }
  }
  return {
...
export function validateFloridaBlobPublicationEvidence(value, blobPlan, now = Date.now()) {
  const origin = credentialFreeHttpsOrigin(value?.deliveryOrigin);
  if (
    value?.schemaVersion !== 1
    || value?.state !== "FL"
    || value?.purpose !== "fl-precinct-parent-scoped-immutable-geometry-publication"
    ...
    || value?.assetCount !== 204
    || value?.canonicalManifestChanged !== false
    || value?.publicEligibilityChanged !== false
    || !Array.isArray(value?.artifacts)
    || value.artifacts.length !== 204
  ) {
    throw new Error("Florida Blob publication evidence is incomplete or incompatible");
  }
  ...
  for (const artifact of value.artifacts) {
    ...
    try {
      ...
      throw new Error("Florida Blob publication evidence contains an invalid URL");
    }
    if (
      !wanted
      ...
      throw new Error("Florida Blob publication artifact set drifted");
    }
    seen.add(artifact.pathname);
  }
  if (seen.size !== expected.size) {
    throw new Error("Florida Blob publication artifact set is incomplete");
  }
  return {
...
export function inspectFloridaPublicationPlan(options = {}) {
  ...
  const hidden = validateFloridaHiddenLoadReceipt(hiddenArtifact.value, {
    now,
  ...
  const blobArtifact = safeEvidence(
    ...
    ".etl/precinct-blob-publications/FL",
    options.blobEvidenceSha256,
  );
  const blobPlan = inspectFloridaPrecinctBlobPublicationPlan({
    root,
    packagePath: loaded.identity.path,
    packageSha256: loaded.identity.sha256,
  });
  const blob = validateFloridaBlobPublicationEvidence(blobArtifact.value, blobPlan, now);
  ...
  const bytes = serializeFloridaPublicationDocument(plan);
  return {
    root,
...
export function buildFloridaPublicationAuthorizationTemplate(plan, planSha256) {
  return {
    schemaVersion: 1,
    state: "FL",
    ...
    scopes: [...FLORIDA_PUBLICATION_SCOPES],
...
export function validateFloridaPublicationAuthorization(value, context) {
  ...
  if (
    ...
    || !semanticallyEqual(value?.scopes, FLORIDA_PUBLICATION_SCOPES)
    ...
    throw new Error("Florida public authorization is absent, expired, or incompatible");
  }
  return {
...
export function buildFloridaPublicRollbackAuthorizationTemplate(
  plan,
  planSha256,
  publicationReceipt,
) {
  return {
    ...
    scopes: [...FLORIDA_PUBLIC_ROLLBACK_SCOPES],
    rollbackWindow: {
      startsAtUtc: null,
      endsAtUtc: null,
    },
...
export function validateFloridaPublicRollbackAuthorization(value, context) {
  const now = context.now ?? Date.now();
  const authorizedAt = Date.parse(value?.authorizedAtUtc);
  const expiresAt = Date.parse(value?.expiresAtUtc);
  ...
  if (
    ...
    || !semanticallyEqual(value?.scopes, FLORIDA_PUBLIC_ROLLBACK_SCOPES)
    ...
    throw new Error("Florida rollback authorization is absent, expired, or incompatible");
  }
  return {
