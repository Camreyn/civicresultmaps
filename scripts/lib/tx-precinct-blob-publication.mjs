...
function safeReleasePackage(root, relativePath) {
  if (
    ...
    || !/^\.etl\/precinct-release-candidates\/TX\/[^/]+\/release-candidate\.json$/.test(relativePath)
    || relativePath.includes("\\")
    || relativePath.split("/").includes("..")
  ) {
    throw new Error("Texas Blob publication package path is unsafe");
  }
  const resolvedRoot = path.resolve(root);
  const absolutePath = path.resolve(root, ...relativePath.split("/"));
  if (!absolutePath.startsWith(resolvedRoot + path.sep)) {
    throw new Error("Texas Blob publication package escapes the repository");
  }
  return absolutePath;
}
...
function safePackageAsset(releaseRoot, relativePath) {
  if (
    ...
    throw new Error("Texas Blob publication asset path is unsafe");
  }
  ...
  if (!absolutePath.startsWith(path.resolve(releaseRoot) + path.sep)) {
    throw new Error("Texas Blob publication asset escapes the package");
  }
  return absolutePath;
}
...
function publicPathname(publicUrl) {
  if (
    ...
    || !publicUrl.startsWith("/data/geography/tx/")
    || publicUrl.includes("\\")
    || publicUrl.includes("?")
    || publicUrl.includes("#")
  ) {
    throw new Error("Texas Blob publication URL is not a safe geography path");
  }
  ...
  if (segments.some((segment) => {
    ...
    throw new Error("Texas Blob publication URL has an unsafe segment");
  }
...
function inspectAsset(releaseRoot, declaration, kind, year) {
  ...
  if (!existsSync(absolutePath)) {
    throw new Error(
      "Texas " + year + " " + kind + " asset is missing",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Texas " + year + " " + kind + " asset hash or byte count drifted",
    );
  }
  return {
    kind,
...
export function inspectTexasPrecinctBlobPublicationPlan(options) {
  const root = path.resolve(options.root ?? process.cwd());
  if (!/^[a-f0-9]{64}$/.test(options.packageSha256 ?? "")) {
    throw new Error("Texas Blob publication requires the exact package SHA-256");
  }
  const packageAbsolutePath = safeReleasePackage(root, options.packagePath);
  if (!existsSync(packageAbsolutePath)) {
    throw new Error("Texas Blob publication package is missing");
  }
  const packageBytes = readFileSync(packageAbsolutePath);
  if (sha256(packageBytes) !== options.packageSha256) {
    throw new Error("Texas Blob publication package SHA-256 does not match");
  }
  ...
  if (
    ...
    throw new Error("Texas Blob publication package contract is incompatible");
  }
  ...
  for (const year of document.years) {
    ...
    if (
      ...
      || delivery.parentArtifacts.length !== 254
      ...
      throw new Error(
        "Texas " + year.year + " parent-scoped publication contract drifted",
      );
    }
    for (const parent of delivery.parentArtifacts) {
      if (!/^48\d{3}$/.test(parent.parentGeoid ?? "")) {
        throw new Error(
          "Texas " + year.year + " publication has an invalid parent GEOID",
        );
      }
  ...
  if (artifacts.length !== 1020 || pathnames.size !== artifacts.length) {
    throw new Error("Texas Blob publication asset set is incomplete or duplicated");
  }
...
export function validateTexasBlobPublicationAuthorization(
  plan,
  environment = process.env,
) {
  if (
    environment.CRM_TX_PRECINCT_PUBLIC_FILE_WRITES
      !== "I_ACKNOWLEDGE_PUBLIC_IMMUTABLE_GEOMETRY_UPLOAD"
    || environment.CRM_TX_PRECINCT_PUBLIC_FILE_PACKAGE_SHA256
      !== plan.releaseCandidate.sha256
    || typeof environment.CRM_TX_PRECINCT_PUBLIC_FILE_AUTHORIZATION_ID
      !== "string"
    || !environment.CRM_TX_PRECINCT_PUBLIC_FILE_AUTHORIZATION_ID.trim()
  ) {
    throw new Error(
      "Texas public immutable geometry upload is not explicitly authorized",
    );
  }
  return {
    authorizationId:
