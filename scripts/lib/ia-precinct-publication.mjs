...
import { inspectIowaPrecinctBlobPublicationPlan } from "./ia-precinct-blob-publication.mjs";
...
export const IOWA_PUBLICATION_YEARS = Object.freeze([2016, 2020, 2024]);
export const IOWA_PUBLICATION_SCOPES = Object.freeze([
...
export function serializeIowaPublicationDocument(value) {
...
function safeEvidence(root, relativePath, allowedRoot, expectedSha256) {
  if (
    ...
    throw new Error("Iowa publication evidence path is unsafe");
  }
  ...
  if (!absolutePath.startsWith(allowed + path.sep) || !existsSync(absolutePath)) {
    throw new Error("Iowa publication evidence is missing");
  }
  const bytes = readFileSync(absolutePath);
  const digest = sha256(bytes);
  if (digest !== expectedSha256) {
    throw new Error("Iowa publication evidence SHA-256 drifted");
  }
  return {
    path: relativePath,
    absolutePath,
...
function credentialFreeHttpsOrigin(value) {
  ...
  try {
    ...
    throw new Error("Iowa publication delivery origin is invalid");
  }
  if (
    ...
    throw new Error("Iowa publication origin must be credential-free HTTPS");
  }
...
function loadPackage(root, packagePath, packageSha256) {
  if (!/^[a-f0-9]{64}$/.test(packageSha256 ?? "")) {
    throw new Error("Iowa publication requires the exact package SHA-256");
  }
  ...
  if (
    ...
    || !semanticallyEqual(
      ...
      IOWA_PUBLICATION_YEARS,
    )
  ) {
    throw new Error("Iowa publication release package is incompatible");
  }
  return {
    artifact,
    document,
...
function publicManifests(root, loaded) {
  ...
  if (inspection.errors.length) {
    throw new Error("Iowa publication registry is invalid: " + inspection.errors.join("; "));
  }
  const rows = registry.manifests.filter((manifest) => manifest?.state === "IA");
  if (rows.length !== 3) {
    throw new Error("Iowa publication registry does not contain three manifests");
  }
  return loaded.document.years.map((year) => {
    ...
    if (
      ...
      throw new Error("Iowa " + year.year + " public manifest drifted from the package");
    }
    return {
      year: year.year,
      electionId: year.electionId,
      manifestId: manifest.id,
      publicManifestSha256: sha256(serializeIowaPublicationDocument(manifest)),
    ...
    if (new Set(all.map((row) => row.manifestId)).size !== 3) {
      throw new Error("Iowa publication manifest IDs are duplicated");
    }
    return item;
  });
}
...
export function validateIowaHiddenLoadReceipt(value, context) {
  ...
  if (
    ...
    throw new Error("Iowa hidden-load receipt is incomplete or incompatible");
  }
  const totals = {
  ...
  for (const [key, expected] of Object.entries(EXPECTED_TOTALS)) {
    ...
    if (Number(totals[key]) !== expected) {
      throw new Error("Iowa hidden-load receipt total drifted: " + key);
    }
  }
  return {
...
export function validateIowaBlobPublicationEvidence(value, blobPlan, now = Date.now()) {
  const origin = credentialFreeHttpsOrigin(value?.deliveryOrigin);
  if (
    value?.schemaVersion !== 1
    || value?.state !== "IA"
    || value?.purpose !== "ia-precinct-parent-scoped-immutable-geometry-publication"
    ...
    || value?.assetCount !== 300
    || value?.canonicalManifestChanged !== false
    || value?.publicEligibilityChanged !== false
    || !Array.isArray(value?.artifacts)
    || value.artifacts.length !== 300
  ) {
    throw new Error("Iowa Blob publication evidence is incomplete or incompatible");
  }
  ...
  for (const artifact of value.artifacts) {
    ...
    try {
      ...
      throw new Error("Iowa Blob publication evidence contains an invalid URL");
    }
    if (
      !wanted
      ...
      throw new Error("Iowa Blob publication artifact set drifted");
    }
    seen.add(artifact.pathname);
  }
  if (seen.size !== expected.size) {
    throw new Error("Iowa Blob publication artifact set is incomplete");
  }
  return {
...
export function inspectIowaPublicationPlan(options = {}) {
  ...
  const hidden = validateIowaHiddenLoadReceipt(hiddenArtifact.value, {
    now,
  ...
  const blobArtifact = safeEvidence(
    ...
    ".etl/precinct-blob-publications/IA",
  ...
  const blob = validateIowaBlobPublicationEvidence(blobArtifact.value, blobPlan, now);
  ...
  const bytes = serializeIowaPublicationDocument(plan);
  return {
    root,
...
export function buildIowaPublicationAuthorizationTemplate(plan, planSha256) {
  return {
    schemaVersion: 1,
    state: "IA",
    ...
    scopes: [...IOWA_PUBLICATION_SCOPES],
...
export function validateIowaPublicationAuthorization(value, context) {
  ...
  if (
    ...
    throw new Error("Iowa public authorization is absent, expired, or incompatible");
  }
  return {
