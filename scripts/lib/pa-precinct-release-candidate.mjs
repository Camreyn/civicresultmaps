...
import { buildPennsylvaniaPrecinctGisPlan } from "./pa-precinct-gis-plan.mjs";
import { inspectPrecinctGeometryManifest } from "../../src/lib/precinct-geography.ts";

export const PENNSYLVANIA_RELEASE_CANDIDATE_ID = "pa-precinct-gis-two-election-v1";
export const PENNSYLVANIA_RELEASE_CANDIDATE_ROOT = ".etl/precinct-release-candidates/PA";
export const PENNSYLVANIA_PRECINCT_VALIDATION_REPORT = ".etl/local-db/pa-precinct-gis-validation.json";
export const PENNSYLVANIA_PUBLIC_RELEASE_YEARS = Object.freeze([
  2016,
  2020,
]);
export const PENNSYLVANIA_DELIVERY_COORDINATE_DECIMALS = 7;
export const PENNSYLVANIA_DELIVERY_SIMPLIFICATION_TOLERANCE_DEGREES = 0.000005;
export const PENNSYLVANIA_MAX_PARENT_DELIVERY_BYTES = 8_000_000;

function sha256(bytes) {
  return createHash("sha256").update(bytes).digest("hex");
}

export function serializePennsylvaniaReleaseDocument(value) {
...
function safePath(root, relativePath, allowedRoots) {
  if (
    ...
    throw new Error("Unsafe Pennsylvania release artifact path: " + relativePath);
  }
  const resolvedRoot = path.resolve(root);
  const absolutePath = path.resolve(resolvedRoot, ...relativePath.split("/"));
  if (!absolutePath.startsWith(resolvedRoot + path.sep)) {
    throw new Error("Pennsylvania release artifact escapes repository root: " + relativePath);
  }
  return absolutePath;
}
...
function readArtifact(root, declaration, allowedRoots = ["data/", ".etl/"]) {
  ...
  if (!existsSync(absolutePath)) {
    throw new Error("Pennsylvania release artifact is missing: " + declaration.path);
  }
  ...
  if (
    ...
    throw new Error("Pennsylvania release artifact byte count drifted: " + declaration.path);
  }
  if (declaration.sha256 && digest !== declaration.sha256) {
    throw new Error("Pennsylvania release artifact SHA-256 drifted: " + declaration.path);
  }
...
function validateLocalReport(report, plan) {
  if (
    ...
    throw new Error("Pennsylvania precinct database validation report is not release-safe");
  }
  const rows = new Map(
  ...
  for (const year of plan.years) {
    ...
    if (!row) throw new Error("Pennsylvania precinct validation is missing " + year.year);
    for (const [key, value] of Object.entries(expected)) {
      if (Number(row[key]) !== value) {
        throw new Error("Pennsylvania " + year.year + " local validation " + key + " drifted");
      }
    }
  }
  if (rows.size !== plan.years.length) {
    throw new Error("Pennsylvania precinct validation year set drifted");
  }
  return {
...
function simplifyGeometry(geometry, scale, tolerance) {
  if (geometry.type === "Polygon") {
    ...
    if (!polygon) {
      throw new Error("Pennsylvania presentation simplification collapsed a polygon");
    }
    return {
      type: "Polygon",
      coordinates: polygon,
  ...
  if (geometry.type === "MultiPolygon") {
    ...
    if (polygons.length === 0) {
      throw new Error("Pennsylvania presentation simplification collapsed a multipolygon");
    }
    return {
      type: "MultiPolygon",
  ...
  throw new Error("Pennsylvania delivery has an unsupported geometry type");
}

export function quantizePennsylvaniaDeliveryCollection(collection) {
  const scale = 10 ** PENNSYLVANIA_DELIVERY_COORDINATE_DECIMALS;
  const tolerance = PENNSYLVANIA_DELIVERY_SIMPLIFICATION_TOLERANCE_DEGREES;
  return {
    ...collection,
    metadata: {
      ...collection.metadata,
      coordinatePrecisionDecimals: PENNSYLVANIA_DELIVERY_COORDINATE_DECIMALS,
...
function buildDraftManifest(year, delivery) {
  ...
  const mappedParentCount = new Set(
    year.reportingUnits.map((unit) => unit.parentGeoid),
  ).size;
  ...
  const warnings = [
    ...
    "All " + geographicReportingUnits
      + " displayable precinct relationships are reviewed one-to-one across "
      + mappedParentCount + " mapped Pennsylvania county scopes.",
  ...
  if (inspection.errors.length || inspection.publicEligibilityReasons.length) {
    throw new Error(
      "Pennsylvania " + year.year + " public draft is invalid: "
...
function buildYear(root, year) {
  ...
  const statewide = quantizePennsylvaniaDeliveryCollection(
  ...
  if (
    ...
    throw new Error("Pennsylvania " + year.year + " parent-scoped delivery drifted");
  }
  const largestParentByteCount = Math.max(
    ...delivery.parentArtifacts.map((artifact) => artifact.byteCount),
  );
  if (largestParentByteCount > PENNSYLVANIA_MAX_PARENT_DELIVERY_BYTES) {
    throw new Error(
      "Pennsylvania " + year.year + " parent delivery exceeds the web response safety limit",
    );
  }
  const draft = buildDraftManifest(year, delivery);
  const draftBytes = serializePennsylvaniaReleaseDocument(draft);
    ...
    summary: {
      ...
      reviewedGeometry: {
        ...
        publicGeographyLabel:
          "VEST election-specific geometry joined only to official Pennsylvania Department of State results under retained CC BY attribution terms",
      ...
      parentScopedDelivery: {
        ...
        simplificationToleranceDegrees:
          PENNSYLVANIA_DELIVERY_SIMPLIFICATION_TOLERANCE_DEGREES,
        maximumParentByteCount: PENNSYLVANIA_MAX_PARENT_DELIVERY_BYTES,
        parentArtifacts,
        electionValuesInDelivery: false,
...
export async function buildPennsylvaniaPrecinctReleaseCandidate(options = {}) {
  const root = path.resolve(options.root ?? process.cwd());
  const plan = await buildPennsylvaniaPrecinctGisPlan({
    root,
    years: [...PENNSYLVANIA_PUBLIC_RELEASE_YEARS],
  });
  const validationPath = options.validationReportPath
    ?? PENNSYLVANIA_PRECINCT_VALIDATION_REPORT;
  ...
  const packageDocument = {
    schemaVersion: 1,
    id: PENNSYLVANIA_RELEASE_CANDIDATE_ID,
    state: "PA",
    stateName: "Pennsylvania",
    scope: "reviewed partial 2016 and 2020 Pennsylvania presidential precinct GIS release preparation; 2012 and 2024 remain blocked and excluded",
    ...
    databaseActivationContract: {
      ...
      expectedPostLoad: {
        reportingUnits: sum(years, (year) => year.certifiedResults.reportingUnits),
        candidateResultRows: sum(years, (year) => year.certifiedResults.resultRows),
        geographyVersions: years.length,
        geometryFeatures: sum(years, (year) => year.reviewedGeometry.featureCount),
        reviewedRelationships: sum(years, (year) => year.reviewedGeometry.reviewedRelationships),
    ...
    releasePolicy: {
      officialResultsScope: "Official Pennsylvania Department of State presidential rows supply every displayed value. Only the 14,819 normalized result units with reviewed geometry are loaded; all excluded source units and totals remain exactly reconciled in the retained source packages.",
      certifiedCanvassBoundary: "The 3,518 official source units without reviewed geometry, totaling 2,328,386 candidate votes across the two elections, are not assigned or estimated onto precinct polygons. Each map is an explicitly partial reviewed subset and discloses the difference between displayed and full official source totals.",
      publicLabel: "Partial Pennsylvania precinct results with reviewed election-specific geometry",
    ...
    goNoGoGates: [
      ...
      { id: "two_year_exact_partial_result_geometry_join", status: "passed" },
      { id: "2012_and_2024_excluded_until_safe_geometry_joins_exist", status: "passed" },
  ...
  return {
    packageDocument,
    packageBytes: serializePennsylvaniaReleaseDocument(packageDocument),
    draftManifests: yearBuilds.map((year) => year.draft),
    deliveryAssets: yearBuilds.flatMap((year) => year.assets),
  };
}

export function pennsylvaniaReleaseCandidateOutputRoot(packageSha256) {
  if (!/^[a-f0-9]{64}$/.test(packageSha256)) {
    throw new Error("Pennsylvania release package hash must be SHA-256");
  }
  return path.posix.join(
    PENNSYLVANIA_RELEASE_CANDIDATE_ROOT,
    PENNSYLVANIA_RELEASE_CANDIDATE_ID + "-" + packageSha256.slice(0, 12),
  );
}
