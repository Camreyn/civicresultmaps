...
export const ALASKA_PRODUCTION_RELEASE_SCOPES = Object.freeze([
  "apply_migration_0009",
  "load_ak_precinct_results_and_geometry_hidden",
  "increment_public_data_revision",
]);
export const ALASKA_MAX_RELEASE_EVIDENCE_AGE_MS = 4 * 60 * 60 * 1000;
...
function expectedScopes() {
  return [...ALASKA_PRODUCTION_RELEASE_SCOPES];
}

export function buildAlaskaProductionAuthorizationTemplate(context) {
  return {
    schemaVersion: 1,
    state: "AK",
    ...
    acknowledgement:
      "This template is not authorization. GO_PRODUCTION authorizes only migration 0009, the hidden Alaska load, and one public revision increment; public geometry and result visibility remain blocked.",
  };
}

export function validateAlaskaProductionPreflightEvidence(report, context) {
  const targetYears = [2012, 2016, 2020, 2024];
  const localYearRows = report?.alaska?.localYearRows;
  const coreYearRows = report?.alaska?.coreYearRows;
  ...
  if (
    ...
    throw new Error(
      "Alaska production preflight evidence is incompatible or already contains precinct release rows",
    );
  }
  const age = context.now.getTime() - Date.parse(report.capturedAtUtc);
  if (age < 0 || age > ALASKA_MAX_RELEASE_EVIDENCE_AGE_MS) {
    throw new Error("Alaska production preflight is outside the four-hour release window");
  }
  if (typeof report.database.name !== "string" || !report.database.name.trim()) {
    throw new Error("Alaska production preflight database name is missing");
  }
  return {
...
export function validateAlaskaProductionBackupEvidence(manifest, context) {
  const restore = manifest?.restoreVerification;
  if (
    !Number.isInteger(manifest?.manifestVersion)
    || manifest.manifestVersion < 3
    || manifest?.backupPurpose !== "ak-precinct-production-release-rollback"
    ...
    throw new Error("Alaska production backup evidence is incomplete or incompatible");
  }
  ...
  if (
    ...
    || restored < created
    || now - created < 0
    || now - created > ALASKA_MAX_RELEASE_EVIDENCE_AGE_MS
    || now - restored < 0
    || now - restored > ALASKA_MAX_RELEASE_EVIDENCE_AGE_MS
  ) {
    throw new Error("Alaska backup must follow the fresh preflight and restore within four hours");
  }
  ...
  if (
    ...
    throw new Error("Alaska production backup exact table or row-count proof drifted");
  }
  return {
...
export function validateAlaskaProductionAuthorization(value, context) {
  if (
    ...
    throw new Error("Alaska production authorization is incomplete or incompatible");
  }
  ...
  if (authorized > now || expires <= now || expires <= authorized) {
    throw new Error("Alaska production authorization is not active");
  }
  return {
