...
const STATE = "WI";
const SETUP_PARSER = "wisconsinLocalReportingGisSetup";
const APPLICATION_NAME = "civicresultmaps-local-wi-local-gis";

const LOCAL_EXECUTION_CONTEXT = Object.freeze({
  mode: "local",
  importParser: SETUP_PARSER,
  importScope: "local-only",
  resultParser: "scripts/setup-wi-local-gis-local.mjs",
  reviewedBy: "repository-reviewed",
  revisionReason: "Local-only Wisconsin reviewed local reporting unit GIS setup",
  database: {
    environment: "local",
    host: "loopback",
    port: 54329,
...
function validatedProductionReleaseAudit(value) {
  if (!value || typeof value !== "object") {
    throw new Error("Production Wisconsin execution requires durable release-audit metadata");
  }
  const output = {};
  for (const key of PRODUCTION_RELEASE_AUDIT_ARTIFACT_KEYS) {
    ...
    if (
      ...
      throw new Error("Production Wisconsin execution release-audit metadata is invalid: " + key);
    }
  ...
  if (
    ...
    throw new Error("Production Wisconsin execution release-audit metadata is invalid");
  }
  ...
  if (Object.keys(value).length !== PRODUCTION_RELEASE_AUDIT_KEYS.length) {
    throw new Error("Production Wisconsin execution release-audit metadata has extra fields");
  }
  return Object.freeze(output);
}

export function buildWisconsinLocalExecutionContext(options = {}) {
  if (!options.mode || options.mode === "local") return LOCAL_EXECUTION_CONTEXT;
  if (options.mode !== "production_release") {
    throw new Error("Unknown Wisconsin local reporting unit execution mode");
  }
  if (!/^[a-f0-9]{64}$/.test(options.releasePackageSha256 ?? "")) {
    throw new Error("Production Wisconsin execution requires a release-package SHA-256");
  }
  if (!/^wi-local-reporting-gis-three-election-v\d+$/.test(options.releaseCandidateId ?? "")) {
    throw new Error("Production Wisconsin execution requires the reviewed release-candidate ID");
  }
  if (typeof options.databaseName !== "string" || !options.databaseName.trim()) {
    throw new Error("Production Wisconsin execution requires the preflight database name");
  }
  return Object.freeze({
    mode: "production_release",
    importParser: "wisconsinLocalGisProductionRelease",
    importScope: "production-release-hidden-load",
    resultParser: "scripts/apply-wi-local-release.mjs",
    reviewedBy: "repository-reviewed-release",
    revisionReason:
      "Wisconsin three-election local reporting unit GIS hidden load "
...
export function assertWisconsinGeometryVersionPrecondition(versions, yearPlan) {
  if (
...
async function electionAndContest(tx, yearPlan) {
  ...
  const contests = await query(tx, [
    ...
    "returning id",
  ], [electionId, STATE, "Wisconsin " + yearPlan.year + " President"]);
...
async function reportingUnits(
  ...
  if (stored.length !== yearPlan.reportingUnits.length) {
    throw new Error("Wisconsin " + yearPlan.year + " reporting-unit count drifted");
  }
  const ids = new Map();
...
async function results(tx, yearPlan, contestId, importRunId, sourceDocumentId, unitIds) {
  for (const batch of chunks(yearPlan.resultRows)) {
    ...
    if (records.some((row) => !row.reporting_unit_id)) {
      throw new Error("Wisconsin " + yearPlan.year + " result lacks reporting unit");
    }
    await query(tx, [
  ...
  if (stored.length !== yearPlan.resultRows.length) {
    throw new Error("Wisconsin " + yearPlan.year + " local-result count drifted");
  }
  ...
  for (const row of stored) {
    ...
    if (
      ...
      throw new Error("Wisconsin " + yearPlan.year + " result drift at " + key);
    }
  }
}
...
async function geometry(
  ...
  if (yearPlan.geometry.disposition === "blocked") {
    ...
    if (Number(existing[0].count) !== 0) {
      throw new Error("Blocked Wisconsin " + yearPlan.year + " already has geometry");
    }
  ...
  assertWisconsinGeometryVersionPrecondition(versions, yearPlan);
  ...
  if (storedFeatures.length !== yearPlan.geometry.features.length) {
    throw new Error("Wisconsin " + yearPlan.year + " geography-feature count drifted");
  }
  ...
  for (const batch of chunks(yearPlan.geometry.crosswalks)) {
    ...
    if (records.some((row) => (
      ...
      throw new Error("Wisconsin " + yearPlan.year + " cannot resolve crosswalk UUIDs");
    }
    await query(tx, [
  ...
  if (["total", "reviewed"]
    ...
    throw new Error("Wisconsin " + yearPlan.year + " crosswalk count drifted");
  }
...
async function applyYear(tx, yearPlan, context) {
  ...
  const resultSourceId = await sourceDocument(tx, {
    ...
    title: "Wisconsin " + yearPlan.year + " presidential local reporting results",
    ...
    confidence: "The plan retains each Wisconsin source's official, reconstructed, and temporal caveats; these local reporting rows never replace certified county/state totals.",
    status: "loaded",
    metadata: {
  ...
  const geometrySourceId = await sourceDocument(tx, {
    ...
    title: "Wisconsin " + yearPlan.year + " local reporting geometry package",
...
export async function applyWisconsinLocalGisTransaction(
  tx,
  plan,
  options = {},
) {
  const context = buildWisconsinLocalExecutionContext(
  ...
  if (
    ...
    throw new Error(
      "Wisconsin local reporting unit release transaction database identity or write mode drifted",
    );
  }
  ...
  if (context.mode === "production_release") {
    ...
    const existing = await query(tx, [
      ...
      "  where ru.state_code='WI'",
      "   and ru.reporting_grain in ('local_reporting_unit','administrative_reporting_unit')",
    ...
    if (
      ...
      throw new Error(
        "Wisconsin production hidden load refuses existing local reporting release rows; "
  ...
  for (const yearPlan of plan.years) {
    ...
    if (
      ...
      throw new Error("Intentional Wisconsin GIS rollback after " + yearPlan.year);
    }
  }
...
export async function applyWisconsinLocalGisPlan(plan, options = {}) {
  if (getDatabaseDriver() !== "postgres") {
    throw new Error("Wisconsin local GIS setup requires CRM_DATABASE_DRIVER=postgres");
  }
  ...
  try {
    return await sql.begin((tx) => applyWisconsinLocalGisTransaction(
      tx,
      plan,
      {
...
async function validateSourceDocumentRecords(sql, yearPlan, context) {
  ...
  if (rows.length !== 2) {
    throw new Error("Wisconsin " + yearPlan.year + " source-document count drifted");
  }
  ...
  if (
    ...
    throw new Error("Wisconsin " + yearPlan.year + " result provenance drifted");
  }
  ...
  if (
    ...
    throw new Error("Wisconsin " + yearPlan.year + " geometry provenance drifted");
  }
  return {
...
async function validateStoredProductionAudit(
  ...
  if (
    ...
    throw new Error("Wisconsin " + yearPlan.year + " reporting-unit release audit drifted");
  }
  ...
  if (
    ...
    throw new Error("Wisconsin " + yearPlan.year + " import-run release audit drifted");
  }
}
...
async function validateStoredGeometryRecords(sql, yearPlan, electionId, context) {
  ...
  if (
    ...
    throw new Error("Wisconsin " + yearPlan.year + " geography-version provenance drifted");
  }
  ...
  if (features.length !== expectedFeatures.size) {
    throw new Error("Wisconsin " + yearPlan.year + " exact feature count drifted");
  }
  ...
  for (const row of features) {
    ...
    assertNoElectionValueProperties(
      row.properties,
      "Wisconsin " + yearPlan.year + " stored feature " + sourceFeatureId,
    );
    if (
      !expected
      ...
      throw new Error("Wisconsin " + yearPlan.year + " stored feature drifted at " + sourceFeatureId);
    }
  ...
  if (crosswalks.length !== expectedCrosswalks.size) {
    throw new Error("Wisconsin " + yearPlan.year + " exact crosswalk count drifted");
  }
  ...
  for (const row of crosswalks) {
    ...
    if (
      ...
      throw new Error("Wisconsin " + yearPlan.year + " stored crosswalk drifted at " + reportingCode);
    }
...
export async function readWisconsinPersistedProductionReleaseAudit(
  sql,
  releaseCandidate,
) {
  if (
    ...
    throw new Error("Wisconsin hidden receipt recovery requires an exact release candidate");
  }
  const rows = await query(sql, [
  ...
  if (
    ...
    throw new Error("Wisconsin hidden receipt recovery found an incomplete year set");
  }
  ...
  for (const row of rows) {
    if (
      ...
      throw new Error("Wisconsin hidden receipt recovery audit drifted across years");
    }
  }
  return audit;
}

export async function validateWisconsinLocalGisClient(
  sql,
  plan,
  options = {},
) {
    ...
    for (const yearPlan of plan.years) {
      const rows = await query(sql, [
        ...
        "   and ru.reporting_grain in ('local_reporting_unit','administrative_reporting_unit')) reporting_units,",
      ...
      if (
        ...
        throw new Error("Wisconsin " + yearPlan.year + " database counts drifted: " + JSON.stringify({
          actual: row,
          expected: {
            reportingUnits: yearPlan.reportingUnits.length,
            resultRows: yearPlan.resultRows.length,
      ...
      if (!democraticName || !republicanName) {
        throw new Error("Wisconsin " + yearPlan.year + " major-party candidate grouping drifted");
      }
      ...
      for (const [name, votes] of Object.entries(expectedTotals)) {
        if (actualTotals[name] !== votes) {
          throw new Error("Wisconsin " + yearPlan.year + " " + name + " total drifted");
        }
      }
      if (Object.keys(actualTotals).length !== Object.keys(expectedTotals).length) {
        throw new Error("Wisconsin " + yearPlan.year + " candidate grouping drifted");
      }
      ...
      if (Number(zero[0].count) !== yearPlan.zeroVoteUnits) {
        throw new Error("Wisconsin " + yearPlan.year + " zero-vote count drifted");
      }
      output.push({
        year: yearPlan.year,
        electionId: yearPlan.electionId,
    ...
    if (Number(invalid[0].count) !== 0) {
      throw new Error("Wisconsin local reporting unit database has unvalidated public constraints");
    }
...
export async function validateWisconsinLocalGisDatabase(plan, options = {}) {
  if (getDatabaseDriver() !== "postgres") {
    throw new Error("Wisconsin local GIS validation requires CRM_DATABASE_DRIVER=postgres");
  }
  ...
  try {
    return await validateWisconsinLocalGisClient(sql, plan, {
      executionContext: { mode: "local" },
