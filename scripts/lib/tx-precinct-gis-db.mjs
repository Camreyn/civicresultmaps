...
const STATE = "TX";
const SETUP_PARSER = "texasLocalPrecinctGisSetup";
const APPLICATION_NAME = "civicresultmaps-local-tx-precinct-gis";

const LOCAL_EXECUTION_CONTEXT = Object.freeze({
  mode: "local",
  importParser: SETUP_PARSER,
  importScope: "local-only",
  resultParser: "scripts/setup-tx-precinct-gis-local.mjs",
  reviewedBy: "repository-reviewed",
  revisionReason: "Local-only Texas four-election precinct GIS setup",
  database: {
    environment: "local",
    host: "loopback",
    port: 54329,
...
function validatedProductionReleaseAudit(value) {
  if (!value || typeof value !== "object") {
    throw new Error("Production Texas execution requires durable release-audit metadata");
  }
  const output = {};
  for (const key of PRODUCTION_RELEASE_AUDIT_ARTIFACT_KEYS) {
    ...
    if (
      ...
      throw new Error("Production Texas execution release-audit metadata is invalid: " + key);
    }
  ...
  if (
    ...
    throw new Error("Production Texas execution release-audit metadata is invalid");
  }
  ...
  if (Object.keys(value).length !== PRODUCTION_RELEASE_AUDIT_KEYS.length) {
    throw new Error("Production Texas execution release-audit metadata has extra fields");
  }
  return Object.freeze(output);
}

export function buildTexasPrecinctExecutionContext(options = {}) {
  if (!options.mode || options.mode === "local") return LOCAL_EXECUTION_CONTEXT;
  if (options.mode !== "production_release") {
    throw new Error("Unknown Texas precinct execution mode");
  }
  if (!/^[a-f0-9]{64}$/.test(options.releasePackageSha256 ?? "")) {
    throw new Error("Production Texas execution requires a release-package SHA-256");
  }
  if (!/^tx-precinct-gis-four-election-v\d+$/.test(options.releaseCandidateId ?? "")) {
    throw new Error("Production Texas execution requires the reviewed release-candidate ID");
  }
  if (typeof options.databaseName !== "string" || !options.databaseName.trim()) {
    throw new Error("Production Texas execution requires the preflight database name");
  }
  return Object.freeze({
    mode: "production_release",
    importParser: "texasPrecinctGisProductionRelease",
    importScope: "production-release-hidden-load",
    resultParser: "scripts/apply-tx-precinct-release.mjs",
    reviewedBy: "repository-reviewed-release",
    revisionReason:
      "Texas four-election precinct GIS hidden load "
...
async function electionAndContest(tx, yearPlan) {
  ...
  const contests = await query(tx, [
    ...
    "returning id",
  ], [electionId, STATE, "Texas " + yearPlan.year + " President"]);
...
async function reportingUnits(
  ...
  if (stored.length !== yearPlan.reportingUnits.length) {
    throw new Error("Texas " + yearPlan.year + " reporting-unit count drifted");
  }
  const ids = new Map();
...
async function results(tx, yearPlan, contestId, importRunId, sourceDocumentId, unitIds) {
  for (const batch of chunks(yearPlan.resultRows)) {
    ...
    if (records.some((row) => !row.reporting_unit_id)) {
      throw new Error("Texas " + yearPlan.year + " result lacks reporting unit");
    }
    await query(tx, [
  ...
  if (stored.length !== yearPlan.resultRows.length) {
    throw new Error("Texas " + yearPlan.year + " precinct-result count drifted");
  }
  ...
  for (const row of stored) {
    ...
    if (
      ...
      throw new Error("Texas " + yearPlan.year + " result drift at " + key);
    }
  }
}
...
async function geometry(
  ...
  if (yearPlan.geometry.disposition === "blocked") {
    ...
    if (Number(existing[0].count) !== 0) {
      throw new Error("Blocked Texas " + yearPlan.year + " already has geometry");
    }
    return { geographyVersions: 0, features: 0, crosswalks: 0 };
  }

  const versions = await query(tx, [
    "select id,metadata->>'manifestId' manifest_id from geography_versions",
    "where state_code=$1 and election_id=$2::uuid and geography_type='precinct'",
  ], [STATE, electionId]);
  if (versions.some((row) => row.manifest_id !== yearPlan.manifest.id)) {
    throw new Error("Texas " + yearPlan.year + " has a different geometry manifest");
  }
  ...
  if (storedFeatures.length !== yearPlan.geometry.features.length) {
    throw new Error("Texas " + yearPlan.year + " geography-feature count drifted");
  }
  ...
  for (const batch of chunks(yearPlan.geometry.crosswalks)) {
    ...
    if (records.some((row) => !row.reporting_unit_id || !row.geography_feature_id)) {
      throw new Error("Texas " + yearPlan.year + " cannot resolve crosswalk UUIDs");
    }
    await query(tx, [
  ...
  const counts = await query(tx, [
    ...
    " count(*) filter (where relationship_type='one_to_one')::int one_to_one,",
    " count(*) filter (where match_method='official_crosswalk')::int exact_id,",
    " count(*) filter (where review_status='reviewed')::int reviewed,",
    " count(*) filter (where confidence='high')::int high",
    "from reporting_unit_geometry_crosswalks",
    "where geometry_version_id=$1::uuid",
  ], [versionId]);
  const expected = yearPlan.geometry.crosswalks.length;
  if (["total", "one_to_one", "exact_id", "reviewed", "high"]
    .some((key) => Number(counts[0][key]) !== expected)) {
    throw new Error("Texas " + yearPlan.year + " crosswalk count drifted");
  }
...
async function applyYear(tx, yearPlan, context) {
  ...
  const resultSourceId = await sourceDocument(tx, {
    ...
    year: yearPlan.year,
    category: "Official source-specific presidential VTD results",
    title: "Texas " + yearPlan.year + " TLC presidential VTD results",
    ...
    confidence: "Official TLC VTD result product derived from county and SOS returns; source-scope caveats are retained and certified SOS county/state totals remain separate.",
    status: "loaded",
    metadata: {
  ...
  const geometrySourceId = await sourceDocument(tx, {
    ...
    category: yearPlan.geometry.disposition === "blocked"
      ...
      : "Official election-applicable statewide precinct boundaries",
    title: "Texas " + yearPlan.year + " precinct geometry package",
    ...
    confidence: yearPlan.geometry.disposition === "blocked"
      ...
      : "Reviewed election-date-confirmed official VTD export with the exact TLC-documented crosswalk.",
...
export async function applyTexasPrecinctGisTransaction(
  tx,
  plan,
  options = {},
) {
  const context = buildTexasPrecinctExecutionContext(
  ...
  if (
    ...
    throw new Error(
      "Texas precinct release transaction database identity or write mode drifted",
    );
  }
  ...
  if (context.mode === "production_release") {
    ...
    const existing = await query(tx, [
      ...
      "  where ru.state_code='TX' and ru.reporting_grain='precinct'",
    ...
    if (
      ...
      throw new Error(
        "Texas production hidden load refuses existing precinct release rows; "
  ...
  for (const yearPlan of plan.years) {
    ...
    if (
      ...
      throw new Error("Intentional Texas GIS rollback after " + yearPlan.year);
    }
  }
...
export async function applyTexasPrecinctGisPlan(plan, options = {}) {
  if (getDatabaseDriver() !== "postgres") {
    throw new Error("Texas local GIS setup requires CRM_DATABASE_DRIVER=postgres");
  }
  ...
  try {
    return await sql.begin((tx) => applyTexasPrecinctGisTransaction(
      tx,
      plan,
      {
...
async function validateSourceDocumentRecords(sql, yearPlan, context) {
  ...
  if (rows.length !== 2) {
    throw new Error("Texas " + yearPlan.year + " source-document count drifted");
  }
  ...
  if (
    ...
    throw new Error("Texas " + yearPlan.year + " result provenance drifted");
  }
  ...
  if (
    ...
    throw new Error("Texas " + yearPlan.year + " geometry provenance drifted");
  }
  return {
...
async function validateStoredProductionAudit(
  ...
  if (
    ...
    throw new Error("Texas " + yearPlan.year + " reporting-unit release audit drifted");
  }
  ...
  if (
    ...
    throw new Error("Texas " + yearPlan.year + " import-run release audit drifted");
  }
}
...
async function validateStoredGeometryRecords(sql, yearPlan, electionId, context) {
  ...
  if (
    ...
    throw new Error("Texas " + yearPlan.year + " geography-version provenance drifted");
  }
  ...
  if (features.length !== expectedFeatures.size) {
    throw new Error("Texas " + yearPlan.year + " exact feature count drifted");
  }
  ...
  for (const row of features) {
    ...
    assertNoElectionValueProperties(
      row.properties,
      "Texas " + yearPlan.year + " stored feature " + sourceFeatureId,
    );
    if (
      !expected
      ...
      throw new Error("Texas " + yearPlan.year + " stored feature drifted at " + sourceFeatureId);
    }
  ...
  const expectedCrosswalks = new Map(
    yearPlan.geometry.crosswalks.map((crosswalk) => [crosswalk.reportingUnitCode, crosswalk]),
  );
  if (crosswalks.length !== expectedCrosswalks.size) {
    throw new Error("Texas " + yearPlan.year + " exact crosswalk count drifted");
  }
  ...
  for (const row of crosswalks) {
    ...
    const expected = expectedCrosswalks.get(reportingCode);
    const metadata = row.metadata ?? {};
    if (
      !expected
      || seenCrosswalks.has(reportingCode)
      ...
      throw new Error("Texas " + yearPlan.year + " stored crosswalk drifted at " + reportingCode);
    }
...
export async function readTexasPersistedProductionReleaseAudit(
  sql,
  releaseCandidate,
) {
  if (
    ...
    throw new Error("Texas hidden receipt recovery requires an exact release candidate");
  }
  const rows = await query(sql, [
  ...
  if (
    ...
    throw new Error("Texas hidden receipt recovery found an incomplete year set");
  }
  ...
  for (const row of rows) {
    if (
      ...
      throw new Error("Texas hidden receipt recovery audit drifted across years");
    }
  }
  return audit;
}

export async function validateTexasPrecinctGisClient(
  sql,
  plan,
  options = {},
) {
    ...
    for (const yearPlan of plan.years) {
      const rows = await query(sql, [
        ...
        "   and x.review_status='reviewed' and x.confidence='high') crosswalks",
      ...
      if (
        ...
        throw new Error("Texas " + yearPlan.year + " database counts drifted: " + JSON.stringify({
          actual: row,
          expected: {
            reportingUnits: yearPlan.reportingUnits.length,
            resultRows: yearPlan.resultRows.length,
      ...
      for (const [name, votes] of Object.entries(expectedTotals)) {
        if (actualTotals[name] !== votes) {
          throw new Error("Texas " + yearPlan.year + " " + name + " total drifted");
        }
      }
      if (Object.keys(actualTotals).length !== 3) {
        throw new Error("Texas " + yearPlan.year + " candidate grouping drifted");
      }
      ...
      if (Number(zero[0].count) !== yearPlan.zeroVoteUnits) {
        throw new Error("Texas " + yearPlan.year + " zero-vote count drifted");
      }
      output.push({
        year: yearPlan.year,
        electionId: yearPlan.electionId,
    ...
    if (Number(invalid[0].count) !== 0) {
      throw new Error("Texas precinct database has unvalidated public constraints");
    }
...
export async function validateTexasPrecinctGisDatabase(plan, options = {}) {
  if (getDatabaseDriver() !== "postgres") {
    throw new Error("Texas local GIS validation requires CRM_DATABASE_DRIVER=postgres");
  }
  ...
  try {
    return await validateTexasPrecinctGisClient(sql, plan, {
      executionContext: { mode: "local" },
