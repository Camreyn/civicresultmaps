...
export const IOWA_PRODUCTION_RELEASE_SCOPES = Object.freeze([
  "apply_migration_0009",
  "load_ia_precinct_results_and_geometry_hidden",
  "increment_public_data_revision",
]);
export const IOWA_MAX_RELEASE_EVIDENCE_AGE_MS = 4 * 60 * 60 * 1000;
...
function expectedScopes() {
  return [...IOWA_PRODUCTION_RELEASE_SCOPES];
}

export function buildIowaProductionAuthorizationTemplate(context) {
  return {
    schemaVersion: 1,
    state: "IA",
    ...
    acknowledgement:
      "This template is not authorization. GO_PRODUCTION authorizes only migration 0009, the hidden Iowa load, and one public revision increment; public geometry and result visibility remain blocked.",
  };
}

export function validateIowaProductionPreflightEvidence(report, context) {
  const targetYears = [2016, 2020, 2024];
  const precinctYearRows = report?.iowa?.precinctYearRows;
  const coreYearRows = report?.iowa?.coreYearRows;
  ...
  if (
    ...
    throw new Error(
      "Iowa production preflight evidence is incompatible or already contains precinct release rows",
    );
  }
  const age = context.now.getTime() - Date.parse(report.capturedAtUtc);
  if (age < 0 || age > IOWA_MAX_RELEASE_EVIDENCE_AGE_MS) {
    throw new Error("Iowa production preflight is outside the four-hour release window");
  }
  if (typeof report.database.name !== "string" || !report.database.name.trim()) {
    throw new Error("Iowa production preflight database name is missing");
  }
  return {
...
export function validateIowaProductionBackupEvidence(manifest, context) {
  const restore = manifest?.restoreVerification;
  if (
    !Number.isInteger(manifest?.manifestVersion)
    || manifest.manifestVersion < 3
    || manifest?.backupPurpose !== "ia-precinct-production-release-rollback"
    ...
    throw new Error("Iowa production backup evidence is incomplete or incompatible");
  }
  ...
  if (
    ...
    || restored < created
    || now - created < 0
    || now - created > IOWA_MAX_RELEASE_EVIDENCE_AGE_MS
    || now - restored < 0
    || now - restored > IOWA_MAX_RELEASE_EVIDENCE_AGE_MS
  ) {
    throw new Error("Iowa backup must follow the fresh preflight and restore within four hours");
  }
  ...
  if (
    ...
    throw new Error("Iowa production backup exact table or row-count proof drifted");
  }
  return {
...
export function validateIowaProductionAuthorization(value, context) {
  if (
    ...
    throw new Error("Iowa production authorization is incomplete or incompatible");
  }
  ...
  if (authorized > now || expires <= now || expires <= authorized) {
    throw new Error("Iowa production authorization is not active");
  }
  return {
