...
import { buildWisconsinLocalGisPlan } from "./wi-local-gis-plan.mjs";
import { inspectPrecinctGeometryManifest } from "../../src/lib/precinct-geography.ts";

export const WISCONSIN_RELEASE_CANDIDATE_ID = "wi-local-reporting-gis-three-election-v1";
export const WISCONSIN_RELEASE_CANDIDATE_ROOT = ".etl/precinct-release-candidates/WI";
export const WISCONSIN_LOCAL_VALIDATION_REPORT = ".etl/local-db/wi-public-local-gis-validation.json";
export const WISCONSIN_PUBLIC_RELEASE_YEARS = Object.freeze([2016, 2020, 2024]);
export const WISCONSIN_DELIVERY_COORDINATE_DECIMALS = 7;
export const WISCONSIN_DELIVERY_SIMPLIFICATION_TOLERANCE_DEGREES = 0.000005;
export const WISCONSIN_MAX_PARENT_DELIVERY_BYTES = 8_000_000;

function sha256(bytes) {
  return createHash("sha256").update(bytes).digest("hex");
}

export function serializeWisconsinReleaseDocument(value) {
...
function safePath(root, relativePath, allowedRoots) {
  if (
    ...
    throw new Error("Unsafe Wisconsin release artifact path: " + relativePath);
  }
  const resolvedRoot = path.resolve(root);
  const absolutePath = path.resolve(resolvedRoot, ...relativePath.split("/"));
  if (!absolutePath.startsWith(resolvedRoot + path.sep)) {
    throw new Error("Wisconsin release artifact escapes repository root: " + relativePath);
  }
  return absolutePath;
}
...
function readArtifact(root, declaration, allowedRoots = ["data/", ".etl/"]) {
  ...
  if (!existsSync(absolutePath)) {
    throw new Error("Wisconsin release artifact is missing: " + declaration.path);
  }
  ...
  if (
    ...
    throw new Error("Wisconsin release artifact byte count drifted: " + declaration.path);
  }
  if (declaration.sha256 && digest !== declaration.sha256) {
    throw new Error("Wisconsin release artifact SHA-256 drifted: " + declaration.path);
  }
...
function validateLocalReport(report, plan) {
  if (
    ...
    throw new Error("Wisconsin local database validation report is not release-safe");
  }
  const rows = new Map(
  ...
  for (const year of plan.years) {
    ...
    if (!row) throw new Error("Wisconsin local validation is missing " + year.year);
    for (const [key, value] of Object.entries(expected)) {
      if (Number(row[key]) !== value) {
        throw new Error("Wisconsin " + year.year + " local validation " + key + " drifted");
      }
    }
  }
  if (rows.size !== plan.years.length) {
    throw new Error("Wisconsin local validation year set drifted");
  }
  return {
...
function simplifyGeometry(geometry, scale, tolerance) {
  if (geometry.type === "Polygon") {
    ...
    if (!polygon) {
      throw new Error("Wisconsin presentation simplification collapsed a polygon");
    }
    return {
      type: "Polygon",
      coordinates: polygon,
  ...
  if (geometry.type === "MultiPolygon") {
    ...
    if (polygons.length === 0) {
      throw new Error("Wisconsin presentation simplification collapsed a multipolygon");
    }
    return {
      type: "MultiPolygon",
  ...
  throw new Error("Wisconsin delivery has an unsupported geometry type");
}

export function quantizeWisconsinDeliveryCollection(collection) {
  const scale = 10 ** WISCONSIN_DELIVERY_COORDINATE_DECIMALS;
  const tolerance = WISCONSIN_DELIVERY_SIMPLIFICATION_TOLERANCE_DEGREES;
  return {
    ...collection,
    metadata: {
      ...collection.metadata,
      coordinatePrecisionDecimals: WISCONSIN_DELIVERY_COORDINATE_DECIMALS,
...
function buildDraftManifest(year, delivery) {
  ...
  const warnings = [
    ...
    "All " + geographicReportingUnits
      + " displayable local-result relationships are reviewed one-to-one across all 72 Wisconsin counties.",
    "All " + nonGeographicReportingUnits
      + " official zero-vote units without reviewed geometry remain explicitly non-geographic and are never assigned to a polygon.",
  ...
  if (inspection.errors.length || inspection.publicEligibilityReasons.length) {
    throw new Error(
      "Wisconsin " + year.year + " public draft is invalid: "
...
function buildYear(root, year) {
  ...
  const statewide = quantizeWisconsinDeliveryCollection(
  ...
  if (
    delivery.parentCount !== 72
    || delivery.featureCount !== year.geometry.features.length
    || delivery.resultUnitCount !== year.geometry.features.length
  ) {
    throw new Error("Wisconsin " + year.year + " parent-scoped delivery drifted");
  }
  const largestParentByteCount = Math.max(
    ...delivery.parentArtifacts.map((artifact) => artifact.byteCount),
  );
  if (largestParentByteCount > WISCONSIN_MAX_PARENT_DELIVERY_BYTES) {
    throw new Error(
      "Wisconsin " + year.year + " parent delivery exceeds the web response safety limit",
    );
  }
  const draft = buildDraftManifest(year, delivery);
  const draftBytes = serializeWisconsinReleaseDocument(draft);
    ...
    summary: {
      ...
      reviewedGeometry: {
        ...
        publicGeographyLabel: year.year === 2016
          ? "attributed VEST election-specific local-boundary reconstruction joined only to official Wisconsin results"
          : year.year === 2020
            ? "attributed VEST election-specific local-boundary reconstruction joined only to official Wisconsin results"
            : "attributed New York Times official-boundary compilation joined only to official Wisconsin Elections Commission results",
      ...
      parentScopedDelivery: {
        ...
        simplificationToleranceDegrees:
          WISCONSIN_DELIVERY_SIMPLIFICATION_TOLERANCE_DEGREES,
        maximumParentByteCount: WISCONSIN_MAX_PARENT_DELIVERY_BYTES,
        parentArtifacts,
        electionValuesInDelivery: false,
...
export async function buildWisconsinLocalReleaseCandidate(options = {}) {
  const root = path.resolve(options.root ?? process.cwd());
  const plan = await buildWisconsinLocalGisPlan({
    root,
    years: [...WISCONSIN_PUBLIC_RELEASE_YEARS],
  });
  const validationPath = options.validationReportPath
    ?? WISCONSIN_LOCAL_VALIDATION_REPORT;
  ...
  const packageDocument = {
    schemaVersion: 1,
    id: WISCONSIN_RELEASE_CANDIDATE_ID,
    state: "WI",
    stateName: "Wisconsin",
    scope: "reviewed 2016, 2020, and 2024 Wisconsin presidential local-reporting GIS release preparation; 2012 remains separately blocked",
    ...
    totals: {
      ...
      parentDeliveryArtifacts: years.length * 72,
    },
    years,
    localValidation: {
      path: validationPath,
    ...
    releasePolicy: {
      officialResultsScope: "Official Wisconsin Elections Commission presidential local reporting rows from all 72 counties. Every displayed vote is grouped to Democratic, Republican, and Other and reconciled to the retained official workbook.",
      certifiedCanvassBoundary: "The 112 official zero-vote units without reviewed geometry remain reconciliation-only; they are never assigned or estimated onto a map polygon. Secondary geometry vote fields are discarded.",
      publicLabel: "Wisconsin local reporting unit results with reviewed election-specific geometry",
      blockedYear: {
        year: 2012,
        issue: "docs/developer/precinct-gis-implementation.md#wisconsin",
        reason: "Wisconsin 2012 retains all 3,525 official reporting rows and 3,047,999 presidential votes, but the LTSB 2011 ward layer exposes population-disaggregated election context rather than a reviewed vote-preserving 2012 result-to-boundary crosswalk.",
      },
    ...
    goNoGoGates: [
      ...
      { id: "2012_remains_separately_blocked", status: "passed", evidence: "docs/developer/precinct-gis-implementation.md#wisconsin" },
  ...
  return {
    packageDocument,
    packageBytes: serializeWisconsinReleaseDocument(packageDocument),
    draftManifests: yearBuilds.map((year) => year.draft),
    deliveryAssets: yearBuilds.flatMap((year) => year.assets),
  };
}

export function wisconsinReleaseCandidateOutputRoot(packageSha256) {
  if (!/^[a-f0-9]{64}$/.test(packageSha256)) {
    throw new Error("Wisconsin release package hash must be SHA-256");
  }
  return path.posix.join(
    WISCONSIN_RELEASE_CANDIDATE_ROOT,
    WISCONSIN_RELEASE_CANDIDATE_ID + "-" + packageSha256.slice(0, 12),
  );
}
