...
const STATE = "FL";
const SETUP_PARSER = "floridaPrecinctReportingGisSetup";
const APPLICATION_NAME = "civicresultmaps-local-fl-precinct-gis";

const LOCAL_EXECUTION_CONTEXT = Object.freeze({
  mode: "local",
  importParser: SETUP_PARSER,
  importScope: "local-only",
  resultParser: "scripts/setup-fl-precinct-gis-local.mjs",
  reviewedBy: "repository-reviewed",
  revisionReason: "Local-only Florida reviewed precinct GIS setup",
  database: {
    environment: "local",
    host: "loopback",
    port: 54329,
...
function validatedProductionReleaseAudit(value) {
  if (!value || typeof value !== "object") {
    throw new Error("Production Florida execution requires durable release-audit metadata");
  }
  const output = {};
  for (const key of PRODUCTION_RELEASE_AUDIT_ARTIFACT_KEYS) {
    ...
    if (
      ...
      throw new Error("Production Florida execution release-audit metadata is invalid: " + key);
    }
  ...
  if (
    ...
    throw new Error("Production Florida execution release-audit metadata is invalid");
  }
  ...
  if (Object.keys(value).length !== PRODUCTION_RELEASE_AUDIT_KEYS.length) {
    throw new Error("Production Florida execution release-audit metadata has extra fields");
  }
  return Object.freeze(output);
}

export function buildFloridaPrecinctExecutionContext(options = {}) {
  if (!options.mode || options.mode === "local") return LOCAL_EXECUTION_CONTEXT;
  if (options.mode !== "production_release") {
    throw new Error("Unknown Florida precinct unit execution mode");
  }
  if (!/^[a-f0-9]{64}$/.test(options.releasePackageSha256 ?? "")) {
    throw new Error("Production Florida execution requires a release-package SHA-256");
  }
  if (!/^fl-precinct-gis-three-election-v\d+$/.test(options.releaseCandidateId ?? "")) {
    throw new Error("Production Florida execution requires the reviewed release-candidate ID");
  }
  if (typeof options.databaseName !== "string" || !options.databaseName.trim()) {
    throw new Error("Production Florida execution requires the preflight database name");
  }
  return Object.freeze({
    mode: "production_release",
    importParser: "floridaPrecinctGisProductionRelease",
    importScope: "production-release-hidden-load",
    resultParser: "scripts/apply-fl-precinct-release.mjs",
    reviewedBy: "repository-reviewed-release",
    revisionReason:
      "Florida three-election precinct GIS hidden load "
...
export function assertFloridaGeometryVersionPrecondition(versions, yearPlan) {
  if (
...
async function electionAndContest(tx, yearPlan) {
  ...
  const contests = await query(tx, [
    ...
    "returning id",
  ], [electionId, STATE, "Florida " + yearPlan.year + " President"]);
...
async function reportingUnits(
  ...
  if (stored.length !== yearPlan.reportingUnits.length) {
    throw new Error("Florida " + yearPlan.year + " reporting-unit count drifted");
  }
  const ids = new Map();
...
async function results(tx, yearPlan, contestId, importRunId, sourceDocumentId, unitIds) {
  for (const batch of chunks(yearPlan.resultRows)) {
    ...
    if (records.some((row) => !row.reporting_unit_id)) {
      throw new Error("Florida " + yearPlan.year + " result lacks reporting unit");
    }
    await query(tx, [
  ...
  if (stored.length !== yearPlan.resultRows.length) {
    throw new Error("Florida " + yearPlan.year + " local-result count drifted");
  }
  ...
  for (const row of stored) {
    ...
    if (
      ...
      throw new Error("Florida " + yearPlan.year + " result drift at " + key);
    }
  }
}
...
async function geometry(
  ...
  if (yearPlan.geometry.disposition === "blocked") {
    ...
    if (Number(existing[0].count) !== 0) {
      throw new Error("Blocked Florida " + yearPlan.year + " already has geometry");
    }
  ...
  assertFloridaGeometryVersionPrecondition(versions, yearPlan);
  ...
  if (storedFeatures.length !== yearPlan.geometry.features.length) {
    throw new Error("Florida " + yearPlan.year + " geography-feature count drifted");
  }
  ...
  for (const batch of chunks(yearPlan.geometry.crosswalks)) {
    ...
    if (records.some((row) => (
      ...
      throw new Error("Florida " + yearPlan.year + " cannot resolve crosswalk UUIDs");
    }
    await query(tx, [
  ...
  if (["total", "reviewed"]
    ...
    throw new Error("Florida " + yearPlan.year + " crosswalk count drifted");
  }
...
async function applyYear(tx, yearPlan, context) {
  ...
  const resultSourceId = await sourceDocument(tx, {
    ...
    title: "Florida " + yearPlan.year + " presidential precinct results",
    ...
    confidence: "Official Florida Department of State rows with reviewed geometry. Source units without reviewed geometry remain reconciled in retained source evidence and are never spatially allocated.",
    status: "loaded",
    metadata: {
  ...
  const geometrySourceId = await sourceDocument(tx, {
    ...
    title: "Florida " + yearPlan.year + " precinct geometry package",
...
export async function applyFloridaPrecinctGisTransaction(
  tx,
  plan,
  options = {},
) {
  const context = buildFloridaPrecinctExecutionContext(
  ...
  if (
    ...
    throw new Error(
      "Florida precinct unit release transaction database identity or write mode drifted",
    );
  }
  ...
  if (context.mode === "production_release") {
    ...
    if (
      ...
      throw new Error(
        "Florida production hidden load refuses existing precinct release rows; "
  ...
  for (const yearPlan of plan.years) {
    ...
    if (
      ...
      throw new Error("Intentional Florida GIS rollback after " + yearPlan.year);
    }
  }
...
export async function applyFloridaPrecinctGisPlan(plan, options = {}) {
  if (getDatabaseDriver() !== "postgres") {
    throw new Error("Florida precinct GIS setup requires CRM_DATABASE_DRIVER=postgres");
  }
  ...
  try {
    return await sql.begin((tx) => applyFloridaPrecinctGisTransaction(
      tx,
      plan,
      {
...
async function validateSourceDocumentRecords(sql, yearPlan, context) {
  ...
  if (rows.length !== 2) {
    throw new Error("Florida " + yearPlan.year + " source-document count drifted");
  }
  ...
  if (
    ...
    throw new Error("Florida " + yearPlan.year + " result provenance drifted");
  }
  ...
  if (
    ...
    throw new Error("Florida " + yearPlan.year + " geometry provenance drifted");
  }
  return {
...
async function validateStoredProductionAudit(
  ...
  if (
    ...
    throw new Error("Florida " + yearPlan.year + " reporting-unit release audit drifted");
  }
  ...
  if (
    ...
    throw new Error("Florida " + yearPlan.year + " import-run release audit drifted");
  }
}
...
async function validateStoredGeometryRecords(sql, yearPlan, electionId, context) {
  ...
  if (
    ...
    throw new Error("Florida " + yearPlan.year + " geography-version provenance drifted");
  }
  ...
  if (features.length !== expectedFeatures.size) {
    throw new Error("Florida " + yearPlan.year + " exact feature count drifted");
  }
  ...
  for (const row of features) {
    ...
    assertNoElectionValueProperties(
      row.properties,
      "Florida " + yearPlan.year + " stored feature " + sourceFeatureId,
    );
    if (
      !expected
      ...
      throw new Error("Florida " + yearPlan.year + " stored feature drifted at " + sourceFeatureId);
    }
  ...
  if (crosswalks.length !== expectedCrosswalks.size) {
    throw new Error("Florida " + yearPlan.year + " exact crosswalk count drifted");
  }
  ...
  for (const row of crosswalks) {
    ...
    if (
      ...
      throw new Error("Florida " + yearPlan.year + " stored crosswalk drifted at " + reportingCode);
    }
...
export async function readFloridaPersistedProductionReleaseAudit(
  sql,
  releaseCandidate,
) {
  if (
    ...
    throw new Error("Florida hidden receipt recovery requires an exact release candidate");
  }
  const rows = await query(sql, [
  ...
  if (
    ...
    throw new Error("Florida hidden receipt recovery found an incomplete year set");
  }
  ...
  for (const row of rows) {
    if (
      ...
      throw new Error("Florida hidden receipt recovery audit drifted across years");
    }
  }
  return audit;
}

export async function validateFloridaPrecinctGisClient(
  sql,
  plan,
  options = {},
) {
    ...
    for (const yearPlan of plan.years) {
      ...
      if (
        ...
        throw new Error("Florida " + yearPlan.year + " database counts drifted: " + JSON.stringify({
          actual: row,
          expected: {
            reportingUnits: yearPlan.reportingUnits.length,
            resultRows: yearPlan.resultRows.length,
      ...
      for (const [name, votes] of Object.entries(expectedTotals)) {
        if (actualTotals[name] !== votes) {
          throw new Error("Florida " + yearPlan.year + " " + name + " total drifted");
        }
      }
      if (Object.keys(actualTotals).length !== Object.keys(expectedTotals).length) {
        throw new Error("Florida " + yearPlan.year + " candidate grouping drifted");
      }
      ...
      if (Number(zero[0].count) !== yearPlan.zeroVoteUnits) {
        throw new Error("Florida " + yearPlan.year + " zero-vote count drifted");
      }
      output.push({
        year: yearPlan.year,
        electionId: yearPlan.electionId,
    ...
    if (Number(invalid[0].count) !== 0) {
      throw new Error("Florida precinct unit database has unvalidated public constraints");
    }
...
export async function validateFloridaPrecinctGisDatabase(plan, options = {}) {
  if (getDatabaseDriver() !== "postgres") {
    throw new Error("Florida precinct GIS validation requires CRM_DATABASE_DRIVER=postgres");
  }
  ...
  try {
    return await validateFloridaPrecinctGisClient(sql, plan, {
      executionContext: { mode: "local" },
