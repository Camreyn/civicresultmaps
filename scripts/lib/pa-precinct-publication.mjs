...
import { inspectPennsylvaniaPrecinctBlobPublicationPlan } from "./pa-precinct-blob-publication.mjs";
...
export const PENNSYLVANIA_PUBLICATION_YEARS = Object.freeze([2016, 2020]);
export const PENNSYLVANIA_PUBLICATION_SCOPES = Object.freeze([
  "publish_pa_precinct_geography_versions",
  "authorize_pa_precinct_results",
  "increment_public_data_revision",
]);
export const PENNSYLVANIA_PUBLIC_ROLLBACK_SCOPES = Object.freeze([
  "block_pa_precinct_geography_versions",
  "revoke_pa_precinct_results",
  "increment_public_data_revision",
]);
...
export function serializePennsylvaniaPublicationDocument(value) {
...
function safeEvidence(root, relativePath, allowedRoot, expectedSha256) {
  if (
    ...
    throw new Error("Pennsylvania publication evidence path is unsafe");
  }
  ...
  if (!absolutePath.startsWith(allowed + path.sep) || !existsSync(absolutePath)) {
    throw new Error("Pennsylvania publication evidence is missing");
  }
  const bytes = readFileSync(absolutePath);
  const digest = sha256(bytes);
  if (digest !== expectedSha256) {
    throw new Error("Pennsylvania publication evidence SHA-256 drifted");
  }
  return {
    path: relativePath,
    absolutePath,
...
function credentialFreeHttpsOrigin(value) {
  ...
  try {
    ...
    throw new Error("Pennsylvania publication delivery origin is invalid");
  }
  if (
    ...
    throw new Error("Pennsylvania publication origin must be credential-free HTTPS");
  }
...
function loadPackage(root, packagePath, packageSha256) {
  if (!/^[a-f0-9]{64}$/.test(packageSha256 ?? "")) {
    throw new Error("Pennsylvania publication requires the exact package SHA-256");
  }
  ...
  if (
    ...
    || !semanticallyEqual(
      ...
      PENNSYLVANIA_PUBLICATION_YEARS,
    )
  ) {
    throw new Error("Pennsylvania publication release package is incompatible");
  }
  return {
    artifact,
    document,
...
function publicManifests(root, loaded) {
  ...
  if (inspection.errors.length) {
    throw new Error("Pennsylvania publication registry is invalid: " + inspection.errors.join("; "));
  }
  const rows = registry.manifests.filter((manifest) => manifest?.state === "PA");
  if (rows.length !== 2) {
    throw new Error("Pennsylvania publication registry does not contain two manifests");
  }
  return loaded.document.years.map((year) => {
    ...
    if (
      ...
      throw new Error("Pennsylvania " + year.year + " public manifest drifted from the package");
    }
    return {
      year: year.year,
      electionId: year.electionId,
      manifestId: manifest.id,
      publicManifestSha256: sha256(serializePennsylvaniaPublicationDocument(manifest)),
    ...
    if (new Set(all.map((row) => row.manifestId)).size !== 2) {
      throw new Error("Pennsylvania publication manifest IDs are duplicated");
    }
    return item;
  });
}
...
export function validatePennsylvaniaHiddenLoadReceipt(value, context) {
  ...
  if (
    ...
    || validation.years.length !== 2
    || !semanticallyEqual(
      validation.years.map((year) => Number(year.year)),
      PENNSYLVANIA_PUBLICATION_YEARS,
    )
  ) {
    throw new Error("Pennsylvania hidden-load receipt is incomplete or incompatible");
  }
  const totals = {
  ...
  for (const [key, expected] of Object.entries(EXPECTED_TOTALS)) {
    ...
    if (Number(totals[key]) !== expected) {
      throw new Error("Pennsylvania hidden-load receipt total drifted: " + key);
    }
  }
  return {
...
export function validatePennsylvaniaBlobPublicationEvidence(value, blobPlan, now = Date.now()) {
  const origin = credentialFreeHttpsOrigin(value?.deliveryOrigin);
  if (
    value?.schemaVersion !== 1
    || value?.state !== "PA"
    || value?.purpose !== "pa-precinct-parent-scoped-immutable-geometry-publication"
    ...
    || value?.assetCount !== 136
    || value?.canonicalManifestChanged !== false
    || value?.publicEligibilityChanged !== false
    || !Array.isArray(value?.artifacts)
    || value.artifacts.length !== 136
  ) {
    throw new Error("Pennsylvania Blob publication evidence is incomplete or incompatible");
  }
  ...
  for (const artifact of value.artifacts) {
    ...
    try {
      ...
      throw new Error("Pennsylvania Blob publication evidence contains an invalid URL");
    }
    if (
      !wanted
      ...
      throw new Error("Pennsylvania Blob publication artifact set drifted");
    }
    seen.add(artifact.pathname);
  }
  if (seen.size !== expected.size) {
    throw new Error("Pennsylvania Blob publication artifact set is incomplete");
  }
  return {
...
export function inspectPennsylvaniaPublicationPlan(options = {}) {
  ...
  const hidden = validatePennsylvaniaHiddenLoadReceipt(hiddenArtifact.value, {
    now,
  ...
  const blobArtifact = safeEvidence(
    ...
    ".etl/precinct-blob-publications/PA",
    options.blobEvidenceSha256,
  );
  const blobPlan = inspectPennsylvaniaPrecinctBlobPublicationPlan({
    root,
    packagePath: loaded.identity.path,
    packageSha256: loaded.identity.sha256,
  });
  const blob = validatePennsylvaniaBlobPublicationEvidence(blobArtifact.value, blobPlan, now);
  ...
  const bytes = serializePennsylvaniaPublicationDocument(plan);
  return {
    root,
...
export function buildPennsylvaniaPublicationAuthorizationTemplate(plan, planSha256) {
  return {
    schemaVersion: 1,
    state: "PA",
    ...
    scopes: [...PENNSYLVANIA_PUBLICATION_SCOPES],
...
export function validatePennsylvaniaPublicationAuthorization(value, context) {
  ...
  if (
    ...
    || !semanticallyEqual(value?.scopes, PENNSYLVANIA_PUBLICATION_SCOPES)
    ...
    throw new Error("Pennsylvania public authorization is absent, expired, or incompatible");
  }
  return {
...
export function buildPennsylvaniaPublicRollbackAuthorizationTemplate(
  plan,
  planSha256,
  publicationReceipt,
) {
  return {
    ...
    scopes: [...PENNSYLVANIA_PUBLIC_ROLLBACK_SCOPES],
    rollbackWindow: {
      startsAtUtc: null,
      endsAtUtc: null,
    },
...
export function validatePennsylvaniaPublicRollbackAuthorization(value, context) {
  const now = context.now ?? Date.now();
  const authorizedAt = Date.parse(value?.authorizedAtUtc);
  const expiresAt = Date.parse(value?.expiresAtUtc);
  ...
  if (
    ...
    || !semanticallyEqual(value?.scopes, PENNSYLVANIA_PUBLIC_ROLLBACK_SCOPES)
    ...
    throw new Error("Pennsylvania rollback authorization is absent, expired, or incompatible");
  }
  return {
