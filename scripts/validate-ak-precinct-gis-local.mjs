...
for (const arg of args) {
  if (!arg.startsWith("--years=") && !arg.startsWith("--report=")) {
    throw new Error("Unknown Alaska GIS validation option: " + arg);
  }
}
...
try {
  ...
  const { validateAlaskaPrecinctGisDatabase } =
  ...
  const report = {
    ...
    scope: "local-only Alaska precinct GIS validation",
    productionMutationPerformed: false,
    publicDeliveryAuthorized: false,
    plan: summarizeAlaskaPrecinctGisPlan(plan),
    validation,
  };
  const reportPath = writeReport(
    reportArgument
      ? reportArgument.slice("--report=".length)
      : ".etl/local-db/ak-precinct-gis-validation.json",
    report,
  );
