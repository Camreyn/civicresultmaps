...
for (const arg of args) {
  if (!arg.startsWith("--years=") && !arg.startsWith("--report=")) {
    throw new Error("Unknown Minnesota GIS validation option: " + arg);
  }
}
...
try {
  ...
  const { validateMinnesotaPrecinctGisDatabase } =
  ...
  const report = {
    ...
    scope: "local-only Minnesota precinct GIS validation",
    productionMutationPerformed: false,
    publicDeliveryAuthorized: false,
    plan: summarizeMinnesotaPrecinctGisPlan(plan),
    validation,
  };
