...
import {
  buildPennsylvaniaPrecinctReleaseCandidate,
  pennsylvaniaReleaseCandidateOutputRoot,
} from "./lib/pa-precinct-release-candidate.mjs";
...
function immutableWrite(root, relativePath, bytes) {
  ...
  const expectedRoot = path.resolve(root, ".etl", "precinct-release-candidates", "PA");
  if (!absolutePath.startsWith(expectedRoot + path.sep)) {
    throw new Error("Pennsylvania release output escapes its content-addressed root");
  }
  if (existsSync(absolutePath)) {
    if (!readFileSync(absolutePath).equals(bytes)) {
      throw new Error("Refusing to overwrite different Pennsylvania release bytes: " + relativePath);
    }
...
export async function preparePennsylvaniaPrecinctReleaseCandidate(options = {}) {
  const root = path.resolve(options.root ?? process.cwd());
  const built = await buildPennsylvaniaPrecinctReleaseCandidate({
    root,
    validationReportPath: options.validationReportPath,
  });
  const packageSha256 = sha256(built.packageBytes);
  const outputRoot = pennsylvaniaReleaseCandidateOutputRoot(packageSha256);
  const files = [
...
async function main() {
  ...
  if (unknown.length) throw new Error("Unknown Pennsylvania release-candidate option: " + unknown[0]);
  const validationReportPath = args.find((arg) => arg.startsWith("--validation-report="))
    ?.slice("--validation-report=".length);
  console.log(JSON.stringify(await preparePennsylvaniaPrecinctReleaseCandidate({
