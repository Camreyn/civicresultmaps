...
import {
  inspectAlaskaPrecinctBlobPublicationPlan,
  validateAlaskaBlobPublicationAuthorization,
} from "./lib/ak-precinct-blob-publication.mjs";
...
function parseArguments(args) {
  ...
  for (const argument of args) {
    if (
      ...
      throw new Error("Unknown Alaska Blob publication option: " + argument);
    }
  }
  ...
  if (!Number.isInteger(concurrency) || concurrency < 1 || concurrency > 8) {
    throw new Error("Alaska Blob publication concurrency must be 1 through 8");
  }
...
async function verifyPublicBlob(url, artifact) {
  ...
  if (!response.ok) {
    throw new Error(
      "Published Alaska geometry returned HTTP " + response.status,
    );
  }
  const bytes = Buffer.from(await response.arrayBuffer());
  if (bytes.length !== artifact.byteCount || sha256(bytes) !== artifact.sha256) {
    throw new Error("Published Alaska geometry failed byte/hash verification");
  }
}
...
async function publishArtifact(artifact, sdk) {
  ...
  if (existing) {
    if (existing.size !== artifact.byteCount) {
      throw new Error(
        "Existing immutable Alaska geometry has a different byte count",
      );
    }
  ...
  if (
    ...
    throw new Error("Published Alaska geometry URL/pathname drifted");
  }
...
export async function publishAlaskaPrecinctDeliveryAssets(options) {
  ...
  const authorization = validateAlaskaBlobPublicationAuthorization(plan);
  ...
  if (origins.size !== 1) {
    throw new Error("Alaska geometry assets were not published to one origin");
  }
  const evidenceDocument = {
    ...
    nextRequiredStep:
      "Configure CRM_PRECINCT_GEOGRAPHY_ORIGIN to deliveryOrigin in a protected preview and verify every election and House District before canonical manifest activation.",
...
async function main() {
  ...
  const result = await publishAlaskaPrecinctDeliveryAssets(args);
