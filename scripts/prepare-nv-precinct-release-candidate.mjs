...
import {
  buildNevadaPrecinctReleaseCandidate,
  nevadaReleaseCandidateOutputRoot,
} from "./lib/nv-precinct-release-candidate.mjs";
...
function immutableWrite(root, relativePath, bytes) {
  ...
  const expectedRoot = path.resolve(root, ".etl", "precinct-release-candidates", "NV");
  if (!absolutePath.startsWith(expectedRoot + path.sep)) {
    throw new Error("Nevada release output escapes its content-addressed root");
  }
  if (existsSync(absolutePath)) {
    if (!readFileSync(absolutePath).equals(bytes)) {
      throw new Error("Refusing to overwrite different Nevada release bytes: " + relativePath);
    }
...
export async function prepareNevadaPrecinctReleaseCandidate(options = {}) {
  ...
  const outputRoot = nevadaReleaseCandidateOutputRoot(packageSha256);
  const files = [
...
async function main() {
  ...
  if (unknown.length) throw new Error("Unknown Nevada release-candidate option: " + unknown[0]);
  const validationReportPath = args.find((arg) => arg.startsWith("--validation-report="))
    ?.slice("--validation-report=".length);
  console.log(JSON.stringify(await prepareNevadaPrecinctReleaseCandidate({
