...
import {
  applyFloridaPrecinctGisTransaction,
  readFloridaPersistedProductionReleaseAudit,
...
import { inspectReleaseArtifact } from "./lib/fl-precinct-release-candidate.mjs";
import {
  assertFloridaReleaseCandidateDocument,
  productionEndpointFingerprint,
} from "./lib/fl-precinct-production-preflight.mjs";
import {
...
} from "./lib/fl-precinct-production-release.mjs";
...
function parseArguments(args) {
  ...
  for (const arg of args) {
    ...
    if (!known.has(key)) throw new Error("Unknown Florida production-release option: " + arg);
  }
  if (!parsed.packagePath || !parsed.packageSha256) {
    throw new Error("Florida production release requires --package and --package-sha256");
  }
  if ([parsed.apply, parsed.recoverReceipt, parsed.writeAuthorizationTemplate]
    .filter(Boolean).length > 1) {
    throw new Error("Florida production release modes are mutually exclusive");
  }
  return parsed;
}
...
function safeEtlPath(root, relativePath, requiredPrefix) {
  if (
    ...
    throw new Error("Unsafe Florida release evidence path: " + relativePath);
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ...requiredPrefix.replace(/\/$/, "").split("/"));
  if (!absolute.startsWith(allowed + path.sep)) {
    throw new Error("Florida release evidence escapes its .etl root");
  }
  return absolute;
}
...
function readEtlJson(root, relativePath, expectedSha256, prefix) {
  if (!/^[a-f0-9]{64}$/.test(expectedSha256 ?? "")) {
    throw new Error("Florida release evidence requires an exact SHA-256");
  }
  const absolute = safeEtlPath(root, relativePath, prefix);
  const bytes = readFileSync(absolute);
  if (sha256(bytes) !== expectedSha256) {
    throw new Error("Florida release evidence SHA-256 drifted: " + relativePath);
  }
...
function readBackupManifest(relativeOrAbsolutePath, expectedSha256, root) {
  if (!/^[a-f0-9]{64}$/.test(expectedSha256 ?? "")) {
    throw new Error("Florida backup manifest requires an exact SHA-256");
  }
  ...
  const bytes = readFileSync(absolute);
  if (sha256(bytes) !== expectedSha256) {
    throw new Error("Florida backup manifest SHA-256 drifted");
  }
...
function productionUrl(environment = process.env) {
  ...
  if (!first && !second) {
    throw new Error("Florida production release requires an explicit unpooled PostgreSQL URL");
  }
  if (first && second && first !== second) {
    throw new Error("Florida production release found conflicting unpooled PostgreSQL URLs");
  }
  return first || second;
}
...
function immutableJson(root, relativePath, value) {
  ...
  if (existsSync(absolute)) {
    if (!readFileSync(absolute).equals(bytes)) {
      throw new Error("Refusing to overwrite different Florida release evidence");
    }
...
function reserveReceipt(
  ...
  const absolute = safeEtlPath(
    root,
    relativePath,
    ".etl/production-release-receipts/FL/",
  );
  if (existsSync(absolute)) throw new Error("Florida production receipt already exists");
  ...
  if (existsSync(pending)) {
    ...
    throw new Error("A Florida release recovery marker already exists; reconcile it before retrying");
  }
...
function finishReceipt(reservation, value) {
  ...
  if (existsSync(temporary)) {
    if (!readFileSync(temporary).equals(bytes)) {
      throw new Error("Florida hidden receipt temporary file drifted");
    }
  } else {
...
function defaultTemplatePath(packageSha256) {
  return ".etl/production-authorizations/FL/fl-precinct-authorization-template-"
...
function defaultReceiptPath(packageSha256, authorizationId) {
  ...
  return ".etl/production-release-receipts/FL/fl-precinct-hidden-load-"
    + packageSha256.slice(0, 12)
    + "-"
    + safeId
    + ".json";
...
async function ensureFloridaDerivationConstraint(tx, migrationArtifact) {
  ...
  if (beforeRows.length !== 1) {
    throw new Error("Florida derivation-method constraint is missing or ambiguous");
  }
  ...
  if (
    ...
    throw new Error("Florida derivation-method constraint preimage drifted");
  }
  ...
  if (statements.length !== 2) {
    throw new Error("Florida migration 0009 statement set drifted");
  }
  ...
  if (
    ...
    throw new Error("Florida migration 0009 did not establish the required constraint");
  }
  return { before: "absent", after: "complete", statementsApplied: 2 };
}

export async function runFloridaProductionRelease(options = {}) {
  ...
  const releaseCandidate = assertFloridaReleaseCandidateDocument(
    packageDocument,
  ...
  const currentTime = () => {
    ...
    if (Number.isNaN(result.getTime())) {
      throw new Error("Florida production release clock returned an invalid time");
    }
    return result;
  };
  if (parsed.writeAuthorizationTemplate) {
    ...
    const artifact = immutableJson(root, relativePath, buildFloridaProductionAuthorizationTemplate({
      releaseCandidate,
  ...
  const preflight = readEtlJson(
    ...
    ".etl/production-preflight-candidates/FL/",
  );
  ...
  const validateEvidenceAt = (now) => {
    const preflightSummary = validateFloridaProductionPreflightEvidence(
      preflight.value,
      { releaseCandidate, endpointFingerprint, now },
    );
    const backupSummary = validateFloridaProductionBackupEvidence(backup.value, {
      releaseCandidate,
      endpointFingerprint,
      preflightCapturedAtUtc: preflightSummary.capturedAtUtc,
      now,
    });
    const authorizationSummary = validateFloridaProductionAuthorization(
      authorization.value,
      {
        releaseCandidate,
  ...
  if (parsed.recoverReceipt) {
    if (
      ...
      || environment.CRM_FL_PRECINCT_GEOGRAPHY_HIDDEN_RECEIPT_RECOVERY
        !== packageArtifact.sha256
      || environment.CRM_FL_PRECINCT_GEOGRAPHY_PRODUCTION_AUTHORIZATION_SHA256
        !== authorization.sha256
    ) {
      throw new Error("Florida hidden receipt recovery is not explicitly read-only and hash-authorized");
    }
    ...
    try {
      sql = (options.postgresFactory ?? postgres)(databaseUrl, {
        ...
        idle_timeout: 20,
        connection: {
          application_name: "civicresultmaps-fl-precinct-hidden-receipt-recovery",
    ...
    try {
      recovered = await sql.begin("read only", async (nv) => {
        const persistedAudit = await readFloridaPersistedProductionReleaseAudit(
          nv,
        ...
        if (committedAt.getTime() > currentTime().getTime()) {
          throw new Error("Florida hidden receipt recovery found a future commit timestamp");
        }
        ...
        if (JSON.stringify(persistedAudit) !== JSON.stringify(expectedAudit)) {
          throw new Error("Florida hidden receipt recovery evidence does not match the durable database audit");
        }
        ...
        const validation = await validateFloridaPrecinctGisClient(nv, plan, {
          executionContext,
          readOnlySession: true,
        });
        if (Number(validation.revision) !== persistedAudit.transaction.publicRevision) {
          throw new Error("Florida hidden receipt recovery public revision drifted");
        }
  ...
  if (
    ...
    || environment.CRM_FL_PRECINCT_GEOGRAPHY_PRODUCTION_WRITES !== packageArtifact.sha256
  ) {
    throw new Error("Florida production writes are not explicitly package-authorized");
  }
  if (
    environment.CRM_FL_PRECINCT_GEOGRAPHY_PRODUCTION_AUTHORIZATION_ID
      !== initialEvidence.authorizationSummary.authorizationId
    || environment.CRM_FL_PRECINCT_GEOGRAPHY_PRODUCTION_AUTHORIZATION_SHA256
      !== authorization.sha256
  ) {
    throw new Error("Florida production authorization environment acknowledgement drifted");
  }
  const reservation = reserveReceipt(
    root,
    receiptPath,
  ...
  try {
    sql = (options.postgresFactory ?? postgres)(databaseUrl, {
      ...
      connect_timeout: 10,
      idle_timeout: 20,
      connection: { application_name: "civicresultmaps-fl-precinct-production-release" },
    });
  } catch (error) {
  ...
  try {
    committed = await sql.begin(async (nv) => {
      ...
      const migration0009 = await ensureFloridaDerivationConstraint(
        nv,
      ...
      if (revisions.length !== 1) throw new Error("Florida production public revision is missing");
      ...
      const applied = await applyFloridaPrecinctGisTransaction(nv, plan, {
        executionContext,
      });
      if (applied.revision !== expectedRevision) {
        throw new Error("Florida production public revision increment drifted");
      }
      const validation = await validateFloridaPrecinctGisClient(nv, plan, {
        executionContext,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  runFloridaProductionRelease().then(
