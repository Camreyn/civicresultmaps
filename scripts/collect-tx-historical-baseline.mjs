...
const state = "TX";
const sourceId = "tx-historical-presidential-wikipedia-county";
const output = "data/tx-historical-presidential-baseline.csv";
...
function columnIndexes(table) {
  ...
  if (demHeader === -1 || repHeader === -1 || totalHeader === -1) throw new Error(`Could not identify vote columns: ${headers.join(" | ")}`);
...
function parseCountyRows(table, year) {
  ...
  for (const chunk of table.split(/^\|-/m).slice(1)) {
    ...
    if (cells.length <= Math.max(columns.dem, columns.rep, columns.total)) continue;

    const county = clean(cells[0])
      .replace(/ County,? Texas$/i, "")
      .replace(/ County$/i, "");
    if (!county || county === "County" || /^Totals?$/i.test(county) || county.includes("|") || county.length > 32) continue;
    ...
    rows.push({
      ...
      source_url: `https://en.wikipedia.org/wiki/${year}_United_States_presidential_election_in_Texas`,
      dem_votes: demVotes,
      rep_votes: repVotes,
...
async function fetchRows(year) {
  const page = `${year}_United_States_presidential_election_in_Texas`;
...
for (const year of years) {
  ...
  if (yearRows.length !== 254) throw new Error(`${year} expected 254 county rows, got ${yearRows.length}`);
}
