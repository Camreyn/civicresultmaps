...
import {
  buildPennsylvaniaPrecinctExecutionContext,
  validatePennsylvaniaPrecinctGisClient,
} from "./lib/pa-precinct-gis-db.mjs";
import { buildPennsylvaniaPrecinctGisPlan } from "./lib/pa-precinct-gis-plan.mjs";
import { productionEndpointFingerprint } from "./lib/pa-precinct-production-preflight.mjs";
import {
  ...
  inspectPennsylvaniaPublicationPlan,
  semanticallyEqual,
...
} from "./lib/pa-precinct-publication.mjs";
...
function parseArguments(args) {
  ...
  for (const arg of args) {
    if (
      ...
      throw new Error("Unknown Pennsylvania publication-status option: " + arg);
    }
  }
  const modes = [
    parsed.writePlan,
  ...
  if (modes > 1) throw new Error("Pennsylvania publication-status modes are mutually exclusive");
  for (const field of [
    "packagePath",
    "packageSha256",
    "hiddenReceiptPath",
    ...
    if (!parsed[field]) throw new Error("Pennsylvania publication-status requires " + field);
  }
  if (
    parsed.rollback
    && (!parsed.publicationReceiptPath || !parsed.publicationReceiptSha256)
  ) {
    throw new Error("Pennsylvania rollback requires the exact publication receipt and SHA-256");
  }
  return parsed;
}
...
function safeJson(root, relativePath, expectedSha256, allowedRoot) {
  if (
    ...
    throw new Error("Pennsylvania publication-status JSON path is unsafe");
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ...allowedRoot.split("/"));
  if (!absolute.startsWith(allowed + path.sep) || !existsSync(absolute)) {
    throw new Error("Pennsylvania publication-status JSON is missing");
  }
  const bytes = readFileSync(absolute);
  if (sha256(bytes) !== expectedSha256) {
    throw new Error("Pennsylvania publication-status JSON SHA-256 drifted");
  }
  return {
    path: relativePath,
    absolute,
...
function immutableJson(root, relativePath, value, allowedRoot) {
  if (
    ...
    throw new Error("Pennsylvania publication-status output path is unsafe");
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ...allowedRoot.split("/"));
  if (!absolute.startsWith(allowed + path.sep)) {
    throw new Error("Pennsylvania publication-status output escapes its fixed root");
  }
  const bytes = serializePennsylvaniaPublicationDocument(value);
  if (existsSync(absolute)) {
    if (!readFileSync(absolute).equals(bytes)) {
      throw new Error("Refusing to overwrite different Pennsylvania publication evidence");
    }
...
export function verifyPennsylvaniaPublicationGitCandidate(
  root,
  publicAuthorization,
  runner = execFileSync,
) {
  ...
  try {
    ...
    throw new Error("Pennsylvania publication Git evidence could not be resolved locally");
  }
  if (
    ...
    throw new Error("Pennsylvania publication checkout or deployment Git evidence drifted");
  }
  return {
    gitSha: head,
    gitTreeSha: headTree,
...
export function assertPennsylvaniaPublicationRecoveryVersionSet(versions) {
  if (!Array.isArray(versions) || versions.length !== 2) {
    throw new Error("Pennsylvania publication receipt recovery found an incomplete version set");
  }
  return versions;
}
...
async function verifyPostconditions(
  ...
  if (versions.length !== 2) {
    throw new Error("Pennsylvania publication postcondition has an incomplete version set");
  }
  for (const [index, row] of versions.entries()) {
    ...
    const expectedCaveat = publicAuthorized
      ? "Reviewed Pennsylvania election-specific precinct geometry is publicly authorized under activation "
    ...
    if (
      ...
      throw new Error("Pennsylvania " + manifest.year + " geography publication drifted");
    }
  }
  const linked = await nv.unsafe([
    ...
    "where gv.state_code='PA' and gv.metadata->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256, String(publicAuthorized)]);
  if (Number(linked[0]?.total) !== 14_819 || Number(linked[0]?.exact) !== 14_819) {
    throw new Error("Pennsylvania publication crosswalk postcondition failed");
  }
  const units = await nv.unsafe([
    ...
    "from reporting_units where state_code='PA'",
    " and reporting_grain='precinct'",
    " and metadata->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256, String(publicAuthorized)]);
  if (Number(units[0]?.total) !== 14_819 || Number(units[0]?.exact) !== 14_819) {
    throw new Error("Pennsylvania publication reporting-unit postcondition failed");
  }
  const sources = await nv.unsafe([
    "select count(*)::int total, count(*) filter (where",
    " metadata->>'publicDeliveryAuthorized'=$2 and",
    " metadata->'releaseCandidate'->>'publicDeliveryAuthorized'=$2)::int exact",
    "from source_documents where state_code='PA'",
    " and metadata->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256, String(publicAuthorized)]);
  if (Number(sources[0]?.total) !== 4 || Number(sources[0]?.exact) !== 4) {
    throw new Error("Pennsylvania publication source-document postcondition failed");
  }
  const runs = await nv.unsafe([
    ...
    "from import_runs where state_code='PA'",
    " and summary->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256, String(publicAuthorized)]);
  if (Number(runs[0]?.total) !== 2 || Number(runs[0]?.exact) !== 2) {
    throw new Error("Pennsylvania publication import-run postcondition failed");
  }
  ...
  if (Number(results[0]?.total) !== 44_457) {
    throw new Error("Pennsylvania publication result-row postcondition failed");
  }
  ...
  if (Number(invalid[0]?.count) !== 0) {
    throw new Error("Pennsylvania publication left invalid public constraints");
  }
  const revisionRows = await nv.unsafe([
    "select revision::int revision,reason from public_data_revisions",
    "where scope='public'",
  ].join("\n"));
  const expectedReason = "Pennsylvania precinct unit geometry " + mode + " "
    + (publicAuthorized
  ...
  if (
    ...
    throw new Error("Pennsylvania publication revision postcondition failed");
  }
  return {
    mode,
    status: wantedStatus,
...
export async function applyPennsylvaniaGeographyPublicationTransaction(nv, context) {
  const mode = context.mode ?? "publish";
  if (!new Set(["publish", "rollback"]).has(mode)) {
    throw new Error("Pennsylvania publication transaction mode is invalid");
  }
  const publicAuthorized = mode === "publish";
  if (mode === "rollback" && !context.publicationReceipt) {
    throw new Error("Pennsylvania rollback requires the exact publication receipt");
  }
  if (mode === "rollback" && (
    ...
    throw new Error("Pennsylvania rollback authorization drifted from the publication receipt");
  }
  ...
  if (
    ...
    throw new Error("Pennsylvania publication transaction database identity drifted");
  }
  ...
  if (
    ...
    throw new Error("Pennsylvania public revision is missing or invalid");
  }
  const revision = currentRevision + 1;
  const gisPlan = context.gisPlan;
  if (mode === "publish") {
    await (context.validateCurrentDatabase ?? validatePennsylvaniaPrecinctGisClient)(nv, gisPlan, {
  ...
  if (versions.length !== 2) {
    throw new Error("Pennsylvania publication requires two exact geography versions");
  }
  for (const [index, version] of versions.entries()) {
    ...
    if (mode === "publish" && (
      ...
      throw new Error("Pennsylvania " + manifest.year + " publication precondition drifted");
    }
    if (mode === "rollback" && (
      commonDrift
    ...
    if (updated.length !== 1) {
      throw new Error("Pennsylvania " + manifest.year + " publication update lost its lock");
    }
  }
  const crosswalks = await nv.unsafe([
    ...
    " and gv.state_code='PA' and gv.metadata->'releaseCandidate'->>'sha256'=$1",
    "returning x.id",
  ].join("\n"), [context.plan.releaseCandidate.sha256, JSON.stringify(publicAuthorized)]);
  if (crosswalks.length !== 14_819) {
    throw new Error("Pennsylvania publication crosswalk update count drifted");
  }
  const units = await nv.unsafe([
  ...
  if (units.length !== 14_819) {
    throw new Error("Pennsylvania publication reporting-unit update count drifted");
  }
  const sources = await nv.unsafe([
    "update source_documents set metadata=",
    " jsonb_set(jsonb_set(metadata,'{publicDeliveryAuthorized}',$2::text::jsonb,true),",
    " '{releaseCandidate,publicDeliveryAuthorized}',$2::text::jsonb,true)",
    "where state_code='PA' and metadata->'releaseCandidate'->>'sha256'=$1 returning id",
  ].join("\n"), [context.plan.releaseCandidate.sha256, JSON.stringify(publicAuthorized)]);
  if (sources.length !== 4) {
    throw new Error("Pennsylvania publication source-document update count drifted");
  }
  const runs = await nv.unsafe([
    "update import_runs set summary=",
    " jsonb_set(jsonb_set(summary,'{publicDeliveryAuthorized}',$2::text::jsonb,true),",
    " '{releaseCandidate,publicDeliveryAuthorized}',$2::text::jsonb,true)",
    "where state_code='PA' and summary->'releaseCandidate'->>'sha256'=$1 returning id",
  ].join("\n"), [context.plan.releaseCandidate.sha256, JSON.stringify(publicAuthorized)]);
  if (runs.length !== 2) {
    throw new Error("Pennsylvania publication import-run update count drifted");
  }
  const revisionRows = await nv.unsafe([
    ...
    "Pennsylvania precinct unit geometry " + mode + " "
      + (publicAuthorized
        ? context.authorization.activationId
        : context.authorization.rollbackId),
  ]);
  if (Number(revisionRows[0]?.revision) !== revision) {
    throw new Error("Pennsylvania publication revision increment drifted");
  }
...
function defaultPlanPath(planSha256) {
  return ".etl/precinct-publication-plans/PA/pa-precinct-publication-plan-"
    + planSha256.slice(0, 12) + ".json";
}

function defaultAuthorizationPath(planSha256, mode = "publish") {
  return ".etl/production-authorizations/PA/pa-precinct-" + mode + "-authorization-template-"
    + planSha256.slice(0, 12) + ".json";
}

export function inspectPennsylvaniaPublicationReceipt(root, options, built) {
  const artifact = safeJson(
    root,
    options.publicationReceiptPath,
    options.publicationReceiptSha256,
    ".etl/production-publication-receipts/PA",
  );
  ...
  const publicAuthorizationArtifact = safeJson(
    ...
    ".etl/production-authorizations/PA",
  );
  const publicAuthorization = validatePennsylvaniaPublicationAuthorization(
  ...
  if (
    ...
    || value?.postconditions?.geographyVersions !== 2
    || value?.postconditions?.crosswalks !== 14_819
    || value?.postconditions?.reportingUnits !== 14_819
    || value?.postconditions?.sourceDocuments !== 4
    || value?.postconditions?.importRuns !== 2
    || value?.postconditions?.resultRows !== 44_457
    || value?.postconditions?.invalidConstraints !== 0
    || value?.productionMutationPerformed !== true
    || value?.publicDeliveryAuthorized !== true
  ) {
    throw new Error("Pennsylvania publication receipt is incomplete or incompatible");
  }
  return {
    artifact,
    publicAuthorization,
...
function defaultReceiptPath(planSha256, activationId, mode = "publish") {
  ...
  return ".etl/production-publication-receipts/PA/pa-precinct-" + mode + "-"
...
function reserveReceipt(
  ...
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ".etl", "production-publication-receipts", "PA");
  if (!absolute.startsWith(allowed + path.sep) || !relativePath.endsWith(".json")) {
    throw new Error("Pennsylvania publication receipt path is unsafe");
  }
  if (existsSync(absolute)) throw new Error("Pennsylvania publication receipt already exists");
  mkdirSync(path.dirname(absolute), { recursive: true });
  ...
  const pendingBytes = serializePennsylvaniaPublicationDocument(pendingDocument);
  if (existsSync(pending)) {
    ...
    throw new Error("A Pennsylvania publication recovery marker already exists; reconcile it before retrying");
  }
...
function finishPublicationReceipt(reservation, receipt) {
  const receiptBytes = serializePennsylvaniaPublicationDocument(receipt);
  ...
  if (existsSync(temporary)) {
    if (!readFileSync(temporary).equals(receiptBytes)) {
      throw new Error("Pennsylvania publication receipt temporary file drifted");
    }
  } else {
...
export async function runPennsylvaniaGeographyPublication(options = {}) {
  ...
  const currentTime = () => {
    ...
    if (Number.isNaN(result.getTime())) {
      throw new Error("Pennsylvania publication clock returned an invalid time");
    }
    return result;
  };
  const built = inspectPennsylvaniaPublicationPlan({ ...parsed, root });
  if (parsed.planSha256 && parsed.planSha256 !== built.sha256) {
    throw new Error("Pennsylvania publication plan SHA-256 drifted");
  }
  ...
  if (parsed.writePlan) {
    if (mode === "rollback") {
      throw new Error("Pennsylvania rollback reuses the exact hash-pinned publication plan");
    }
    const artifact = immutableJson(
      root,
      parsed.outputPath ?? defaultPlanPath(built.sha256),
      built.plan,
      ".etl/precinct-publication-plans/PA",
    );
    return {
      mode: "write_plan",
  ...
  if (parsed.writeAuthorizationTemplate) {
    const template = mode === "rollback"
      ...
      : buildPennsylvaniaPublicationAuthorizationTemplate(built.plan, built.sha256);
    const artifact = immutableJson(
      root,
  ...
  const validateAuthorizationAt = (now, recovery = false) => mode === "rollback"
    ? validatePennsylvaniaPublicRollbackAuthorization(authorizationArtifact.value, {
      plan: built.plan,
      planSha256: built.sha256,
    ...
    : validatePennsylvaniaPublicationAuthorization(authorizationArtifact.value, {
      plan: built.plan,
      planSha256: built.sha256,
      now: now.getTime(),
      recovery,
  ...
  if (endpointFingerprint !== built.plan.hiddenLoad.endpointFingerprint) {
    throw new Error("Pennsylvania publication database endpoint drifted from the hidden load");
  }
  const gisPlan = await buildPennsylvaniaPrecinctGisPlan({
    root,
    years: [2016, 2020],
  });
  ...
  buildPennsylvaniaPrecinctExecutionContext(executionContext);
  ...
  if (parsed.recoverReceipt) {
    if (
      ...
      || environment.CRM_PA_PRECINCT_GEOGRAPHY_PUBLICATION_RECEIPT_RECOVERY !== built.sha256
      || environment.CRM_PA_PRECINCT_GEOGRAPHY_PUBLICATION_AUTHORIZATION_SHA256
        !== authorizationArtifact.sha256
      || (mode === "rollback"
        && environment.CRM_PA_PRECINCT_GEOGRAPHY_PUBLICATION_RECEIPT_SHA256
          !== publicationReceipt.summary.sha256)
    ) {
      throw new Error(
        "Pennsylvania publication receipt recovery is not explicitly read-only and hash-authorized",
      );
    }
    ...
    try {
      sql = (options.postgresFactory ?? postgres)(databaseUrl, {
        ...
        idle_timeout: 20,
        connection: {
          application_name: "civicresultmaps-pa-precinct-publication-receipt-recovery",
    ...
    try {
      recovered = await sql.begin("read only", async (nv) => {
        ...
        if (
          ...
          throw new Error("Pennsylvania publication receipt recovery database identity drifted");
        }
        const versions = await nv.unsafe([
        ...
        assertPennsylvaniaPublicationRecoveryVersionSet(versions);
        ...
        if (
          ...
          throw new Error("Pennsylvania publication receipt recovery audit is incomplete");
        }
  ...
  const writeAuthorized = mode === "publish"
    ? environment.CRM_PA_PRECINCT_GEOGRAPHY_PUBLICATION_WRITES
        === "I_ACKNOWLEDGE_ATOMIC_PENNSYLVANIA_PRECINCT_PUBLIC_CUTOVER"
      && environment.CRM_PA_PRECINCT_GEOGRAPHY_PUBLICATION_ACTIVATION_ID
        === authorization.activationId
    : environment.CRM_PA_PRECINCT_GEOGRAPHY_ROLLBACK_WRITES
        === "I_ACKNOWLEDGE_DATABASE_FIRST_PENNSYLVANIA_PRECINCT_PUBLIC_ROLLBACK"
      && environment.CRM_PA_PRECINCT_GEOGRAPHY_ROLLBACK_ID === authorization.rollbackId
      && environment.CRM_PA_PRECINCT_GEOGRAPHY_PUBLICATION_RECEIPT_SHA256
        === publicationReceipt.summary.sha256;
  if (
    environment.CRM_DATABASE_ENVIRONMENT !== "production"
    || !writeAuthorized
    || environment.CRM_PA_PRECINCT_GEOGRAPHY_PUBLICATION_PACKAGE_SHA256
      !== built.plan.releaseCandidate.sha256
    || environment.CRM_PA_PRECINCT_GEOGRAPHY_PUBLICATION_PLAN_SHA256 !== built.sha256
    || environment.CRM_PA_PRECINCT_GEOGRAPHY_PUBLICATION_AUTHORIZATION_SHA256
      !== authorizationArtifact.sha256
  ) {
    throw new Error("Pennsylvania public " + mode + " is not explicitly hash-authorized");
  }
  const activePublicAuthorization = mode === "publish"
    ? authorization
    : publicAuthorization;
  verifyPennsylvaniaPublicationGitCandidate(root, activePublicAuthorization, options.gitRunner);
  const reservation = reserveReceipt(
    root,
    receiptPath,
    built.sha256,
  ...
  try {
    sql = (options.postgresFactory ?? postgres)(databaseUrl, {
      ...
      connect_timeout: 10,
      idle_timeout: 20,
      connection: { application_name: "civicresultmaps-pa-precinct-publication-status" },
    });
  } catch (error) {
  ...
  try {
    committed = await sql.begin(async (nv) => {
      ...
      if (finalAuthorization.activationId !== authorization.activationId) {
        throw new Error("Pennsylvania publication authorization drifted before the transaction");
      }
      ...
      const result = await applyPennsylvaniaGeographyPublicationTransaction(nv, {
        mode,
        plan: built.plan,
        planSha256: built.sha256,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  runPennsylvaniaGeographyPublication().then(
