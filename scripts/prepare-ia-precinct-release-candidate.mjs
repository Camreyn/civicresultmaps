...
import {
  buildIowaPrecinctReleaseCandidate,
  iowaReleaseCandidateOutputRoot,
} from "./lib/ia-precinct-release-candidate.mjs";
...
function immutableWrite(root, relativePath, bytes) {
  ...
  const expectedRoot = path.resolve(root, ".etl", "precinct-release-candidates", "IA");
  if (!absolutePath.startsWith(expectedRoot + path.sep)) {
    throw new Error("Iowa release output escapes its content-addressed root");
  }
  if (existsSync(absolutePath)) {
    if (!readFileSync(absolutePath).equals(bytes)) {
      throw new Error("Refusing to overwrite different Iowa release bytes: " + relativePath);
    }
...
export async function prepareIowaPrecinctReleaseCandidate(options = {}) {
  ...
  const outputRoot = iowaReleaseCandidateOutputRoot(packageSha256);
  const files = [
...
async function main() {
  ...
  if (unknown.length) throw new Error("Unknown Iowa release-candidate option: " + unknown[0]);
  const validationReportPath = args.find((arg) => arg.startsWith("--validation-report="))
    ?.slice("--validation-report=".length);
  console.log(JSON.stringify(await prepareIowaPrecinctReleaseCandidate({
