...
import {
  ...
  collectWisconsinProductionPreflight,
...
function parseArguments(args) {
  ...
  for (const arg of args) {
    if (
      ...
      throw new Error("Unknown Wisconsin production-preflight option: " + arg);
    }
  }
  const packagePath = packageArgument?.slice("--package=".length);
  if (!packagePath) {
    throw new Error("--package is required for Wisconsin production preflight");
  }
  return {
    packagePath,
...
function safeReportPath(root, requested, packageSha256, reportSha256) {
  ...
  if (
    ...
    || !relativePath.startsWith(".etl/production-preflight-candidates/WI/")
  ) {
    throw new Error("Wisconsin production preflight report must remain in its .etl root");
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ".etl", "production-preflight-candidates", "WI");
  if (!absolute.startsWith(allowed + path.sep)) {
    throw new Error("Wisconsin production preflight report escapes its .etl root");
  }
...
function loadReleaseCandidate(root, relativePath) {
  if (
    ...
    || relativePath.includes("\\")
    || relativePath.split("/").includes("..")
    || !relativePath.startsWith(".etl/precinct-release-candidates/WI/")
    || !relativePath.endsWith("/release-candidate.json")
  ) {
    throw new Error("Wisconsin production preflight package path is unsafe");
  }
...
export async function runWisconsinProductionPreflight(options = {}) {
  ...
  if (!parsed.connectReadOnly) {
    return {
      ...
      requiredEnvironment: {
        ...
        CRM_WI_LOCAL_GEOGRAPHY_PRODUCTION_PREFLIGHT_ACK: loaded.artifact.sha256,
  ...
  if (process.env.CRM_DATABASE_ENVIRONMENT !== "production-read-only") {
    throw new Error("Wisconsin production preflight requires production-read-only environment");
  }
  if (
    process.env.CRM_WI_LOCAL_GEOGRAPHY_PRODUCTION_PREFLIGHT_ACK
      !== loaded.artifact.sha256
  ) {
    throw new Error("Wisconsin production preflight acknowledgement must equal the package SHA-256");
  }
  ...
  const sql = (options.postgresFactory ?? postgres)(databaseUrl, {
    ...
    idle_timeout: 20,
    connection: {
      application_name: "civicresultmaps-wi-local-production-preflight",
  ...
  try {
    report = await sql.begin("read only", (nv) =>
      collectWisconsinProductionPreflight(nv, {
  ...
  if (existsSync(output.absolute)) {
    if (!readFileSync(output.absolute).equals(bytes)) {
      throw new Error("Refusing to overwrite different Wisconsin preflight evidence");
    }
  } else {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    console.log(JSON.stringify(await runWisconsinProductionPreflight(), null, 2));
  } catch (error) {
