...
const sourceId = "ia-historical-presidential-wikipedia-county";
const output = "data/ia-historical-presidential-baseline.csv";
...
function parseCountyRows(table, year) {
  ...
  for (const chunk of table.split(/^\|-/m).slice(1)) {
    ...
    const demVotes = year === 2012 ? numberFromCell(cells[1]) : numberFromCell(cells[3]);
    const repVotes = year === 2012 ? numberFromCell(cells[3]) : numberFromCell(cells[1]);
    const otherVotes = numberFromCell(cells[5]);
    const totalVotes = demVotes + repVotes + otherVotes;
    const tableTotal = numberFromCell(cells[9]);
    if (![demVotes, repVotes, otherVotes, tableTotal].every(Number.isFinite)) continue;
    if (totalVotes !== tableTotal) {
      throw new Error(`${year} ${county} total mismatch: ${totalVotes} != ${tableTotal}`);
    }
    rows.push({
      state,
      election_year: year,
...
for (const year of years) {
  ...
  if (yearRows.length !== 99) throw new Error(`${year} expected 99 county rows, got ${yearRows.length}`);
}
