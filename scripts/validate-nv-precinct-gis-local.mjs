...
for (const arg of args) {
  if (!arg.startsWith("--years=") && !arg.startsWith("--report=")) {
    throw new Error("Unknown Nevada GIS validation option: " + arg);
  }
}
...
try {
  ...
  const { validateNevadaPrecinctGisDatabase } =
  ...
  const report = {
    ...
    scope: "local-only Nevada precinct GIS validation",
    productionMutationPerformed: false,
    publicDeliveryAuthorized: false,
    plan: summarizeNevadaPrecinctGisPlan(plan),
    validation,
  };
  const reportPath = writeReport(
    reportArgument
      ? reportArgument.slice("--report=".length)
      : ".etl/local-db/nv-precinct-gis-validation.json",
    report,
  );
