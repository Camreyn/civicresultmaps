...
import {
  ...
  readNevadaPersistedProductionReleaseAudit,
...
import { inspectReleaseArtifact } from "./lib/nv-precinct-release-candidate.mjs";
import {
  assertNevadaReleaseCandidateDocument,
  productionEndpointFingerprint,
} from "./lib/nv-precinct-production-preflight.mjs";
import {
...
function parseArguments(args) {
  ...
  for (const arg of args) {
    ...
    if (!known.has(key)) throw new Error("Unknown Nevada production-release option: " + arg);
  }
  if (!parsed.packagePath || !parsed.packageSha256) {
    throw new Error("Nevada production release requires --package and --package-sha256");
  }
...
function safeEtlPath(root, relativePath, requiredPrefix) {
  if (
    ...
    throw new Error("Unsafe Nevada release evidence path: " + relativePath);
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ...requiredPrefix.replace(/\/$/, "").split("/"));
  if (!absolute.startsWith(allowed + path.sep)) {
    throw new Error("Nevada release evidence escapes its .etl root");
  }
  return absolute;
}
...
function readEtlJson(root, relativePath, expectedSha256, prefix) {
  if (!/^[a-f0-9]{64}$/.test(expectedSha256 ?? "")) {
    throw new Error("Nevada release evidence requires an exact SHA-256");
  }
  const absolute = safeEtlPath(root, relativePath, prefix);
  const bytes = readFileSync(absolute);
  if (sha256(bytes) !== expectedSha256) {
    throw new Error("Nevada release evidence SHA-256 drifted: " + relativePath);
  }
...
function readBackupManifest(relativeOrAbsolutePath, expectedSha256, root) {
  if (!/^[a-f0-9]{64}$/.test(expectedSha256 ?? "")) {
    throw new Error("Nevada backup manifest requires an exact SHA-256");
  }
  ...
  const bytes = readFileSync(absolute);
  if (sha256(bytes) !== expectedSha256) {
    throw new Error("Nevada backup manifest SHA-256 drifted");
  }
...
function productionUrl(environment = process.env) {
  ...
  if (!first && !second) {
    throw new Error("Nevada production release requires an explicit unpooled PostgreSQL URL");
  }
  if (first && second && first !== second) {
    throw new Error("Nevada production release found conflicting unpooled PostgreSQL URLs");
  }
  return first || second;
}
...
function immutableJson(root, relativePath, value) {
  ...
  if (existsSync(absolute)) {
    if (!readFileSync(absolute).equals(bytes)) {
      throw new Error("Refusing to overwrite different Nevada release evidence");
    }
...
function reserveReceipt(
  ...
  const absolute = safeEtlPath(
    root,
    relativePath,
    ".etl/production-release-receipts/NV/",
  );
  if (existsSync(absolute)) throw new Error("Nevada production receipt already exists");
  ...
  if (existsSync(pending)) {
    ...
    throw new Error("A Nevada release recovery marker already exists; reconcile it before retrying");
  }
...
function finishReceipt(reservation, value) {
  ...
  if (existsSync(temporary)) {
    if (!readFileSync(temporary).equals(bytes)) {
      throw new Error("Nevada hidden receipt temporary file drifted");
    }
  } else {
...
function defaultReceiptPath(packageSha256, authorizationId) {
  ...
  return ".etl/production-release-receipts/NV/nv-precinct-hidden-load-"
    + packageSha256.slice(0, 12)
    + "-"
    + safeId
    + ".json";
...
async function ensureNevadaDerivationConstraint(tx, migrationArtifact) {
  ...
  if (beforeRows.length !== 1) {
    throw new Error("Nevada derivation-method constraint is missing or ambiguous");
  }
  ...
  if (
    ...
    throw new Error("Nevada derivation-method constraint preimage drifted");
  }
  ...
  if (statements.length !== 2) {
    throw new Error("Nevada migration 0009 statement set drifted");
  }
  ...
  if (
    ...
    throw new Error("Nevada migration 0009 did not establish the required constraint");
  }
  return { before: "absent", after: "complete", statementsApplied: 2 };
}

export async function runNevadaProductionRelease(options = {}) {
  ...
  const releaseCandidate = assertNevadaReleaseCandidateDocument(
    packageDocument,
  ...
  const currentTime = () => {
    ...
    if (Number.isNaN(result.getTime())) {
      throw new Error("Nevada production release clock returned an invalid time");
    }
    return result;
  };
  ...
  const preflight = readEtlJson(
    ...
    ".etl/production-preflight-candidates/NV/",
  );
  ...
  const validateEvidenceAt = (now) => {
    ...
    const authorizationSummary = validateNevadaProductionAuthorization(
      authorization.value,
      {
        releaseCandidate,
  ...
  if (parsed.recoverReceipt) {
    if (
      ...
      || environment.CRM_NV_PRECINCT_HIDDEN_RECEIPT_RECOVERY
        !== packageArtifact.sha256
    ...
    try {
      sql = (options.postgresFactory ?? postgres)(databaseUrl, {
        ...
        idle_timeout: 20,
        connection: {
          application_name: "civicresultmaps-nv-precinct-hidden-receipt-recovery",
    ...
    try {
      recovered = await sql.begin("read only", async (nv) => {
        const persistedAudit = await readNevadaPersistedProductionReleaseAudit(
          nv,
        ...
        if (committedAt.getTime() > currentTime().getTime()) {
          throw new Error("Nevada hidden receipt recovery found a future commit timestamp");
        }
        ...
        if (Number(validation.revision) !== persistedAudit.transaction.publicRevision) {
          throw new Error("Nevada hidden receipt recovery public revision drifted");
        }
  ...
  if (
    ...
    || environment.CRM_NV_PRECINCT_PRODUCTION_WRITES !== packageArtifact.sha256
  ) {
    throw new Error("Nevada production writes are not explicitly package-authorized");
  }
  if (
    environment.CRM_NV_PRECINCT_PRODUCTION_AUTHORIZATION_ID
  ...
  try {
    sql = (options.postgresFactory ?? postgres)(databaseUrl, {
      ...
      connect_timeout: 10,
      idle_timeout: 20,
      connection: { application_name: "civicresultmaps-nv-precinct-production-release" },
    });
  } catch (error) {
  ...
  try {
    committed = await sql.begin(async (nv) => {
      ...
      const migration0009 = await ensureNevadaDerivationConstraint(
        nv,
      ...
      if (revisions.length !== 1) throw new Error("Nevada production public revision is missing");
      ...
      if (applied.revision !== expectedRevision) {
        throw new Error("Nevada production public revision increment drifted");
      }
      const validation = await validateNevadaPrecinctGisClient(nv, plan, {
        executionContext,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  runNevadaProductionRelease().then(
