...
} from "./lib/tx-precinct-blob-publication.mjs";
...
function parseArguments(args) {
  ...
  for (const argument of args) {
    if (
      ...
      throw new Error("Unknown Texas Blob publication option: " + argument);
    }
  }
  if (!packagePath || !packageSha256) {
    throw new Error(
      "usage: npm run precinct-gis:delivery-publish:tx -- "
      + "--package=<release-candidate.json> --package-sha256=<sha256> "
      + "[--concurrency=4] [--write]",
    );
  }
  if (!Number.isInteger(concurrency) || concurrency < 1 || concurrency > 8) {
    throw new Error("Texas Blob publication concurrency must be 1 through 8");
  }
...
async function verifyPublicBlob(url, artifact) {
  ...
  if (!response.ok) {
    throw new Error(
      "Published Texas geometry returned HTTP " + response.status,
    );
  }
  const bytes = Buffer.from(await response.arrayBuffer());
  if (bytes.length !== artifact.byteCount || sha256(bytes) !== artifact.sha256) {
    throw new Error("Published Texas geometry failed byte/hash verification");
  }
}
...
async function publishArtifact(artifact, sdk) {
  ...
  if (existing) {
    if (existing.size !== artifact.byteCount) {
      throw new Error(
        "Existing immutable Texas geometry has a different byte count",
      );
    }
  ...
  if (
    ...
    throw new Error("Published Texas geometry URL/pathname drifted");
  }
...
export async function publishTexasPrecinctDeliveryAssets(options) {
  const root = path.resolve(options.root ?? process.cwd());
  const plan = inspectTexasPrecinctBlobPublicationPlan({
    root,
  ...
  const authorization = validateTexasBlobPublicationAuthorization(plan);
  ...
  if (origins.size !== 1) {
    throw new Error("Texas geometry assets were not published to one origin");
  }
...
async function main() {
  ...
  const result = await publishTexasPrecinctDeliveryAssets(args);
