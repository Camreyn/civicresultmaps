...
import {
  buildNevadaPrecinctExecutionContext,
  validateNevadaPrecinctGisClient,
} from "./lib/nv-precinct-gis-db.mjs";
import { buildNevadaPrecinctGisPlan } from "./lib/nv-precinct-gis-plan.mjs";
import { productionEndpointFingerprint } from "./lib/nv-precinct-production-preflight.mjs";
import {
  buildNevadaPublicationAuthorizationTemplate,
  inspectNevadaPublicationPlan,
  semanticallyEqual,
  serializeNevadaPublicationDocument,
  sha256,
  validateNevadaPublicationAuthorization,
} from "./lib/nv-precinct-publication.mjs";
...
function parseArguments(args) {
  ...
  for (const arg of args) {
    if (
      ...
      throw new Error("Unknown Nevada publication-status option: " + arg);
    }
  }
  const modes = [
    parsed.writePlan,
  ...
  if (modes > 1) throw new Error("Nevada publication-status modes are mutually exclusive");
  for (const field of [
    "packagePath",
    "packageSha256",
    "hiddenReceiptPath",
    ...
    if (!parsed[field]) throw new Error("Nevada publication-status requires " + field);
  }
  return parsed;
}
...
function safeJson(root, relativePath, expectedSha256, allowedRoot) {
  if (
    ...
    throw new Error("Nevada publication-status JSON path is unsafe");
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ...allowedRoot.split("/"));
  if (!absolute.startsWith(allowed + path.sep) || !existsSync(absolute)) {
    throw new Error("Nevada publication-status JSON is missing");
  }
  const bytes = readFileSync(absolute);
  if (sha256(bytes) !== expectedSha256) {
    throw new Error("Nevada publication-status JSON SHA-256 drifted");
  }
  return {
    path: relativePath,
    absolute,
...
function immutableJson(root, relativePath, value, allowedRoot) {
  if (
    ...
    throw new Error("Nevada publication-status output path is unsafe");
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ...allowedRoot.split("/"));
  if (!absolute.startsWith(allowed + path.sep)) {
    throw new Error("Nevada publication-status output escapes its fixed root");
  }
  const bytes = serializeNevadaPublicationDocument(value);
  if (existsSync(absolute)) {
    if (!readFileSync(absolute).equals(bytes)) {
      throw new Error("Refusing to overwrite different Nevada publication evidence");
    }
...
function currentGitSha(root) {
  ...
  if (!/^[a-f0-9]{40}$/.test(sha)) {
    throw new Error("Nevada publication checkout HEAD is invalid");
  }
  ...
  if (trackedStatus) {
    throw new Error("Nevada publication checkout has tracked changes");
  }
  return sha;
}
...
async function verifyPostconditions(nv, context, revision) {
  ...
  if (versions.length !== 3) {
    throw new Error("Nevada publication postcondition has an incomplete version set");
  }
  for (const [index, row] of versions.entries()) {
    ...
    if (
      ...
      throw new Error("Nevada " + manifest.year + " geography publication drifted");
    }
  }
  ...
  const sources = await nv.unsafe([
    ...
    " metadata->>'publicDeliveryAuthorized'='true' and",
    " metadata->'releaseCandidate'->>'publicDeliveryAuthorized'='true')::int exact",
    "from source_documents where state_code='NV'",
    " and metadata->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256]);
  if (Number(sources[0]?.total) !== 6 || Number(sources[0]?.exact) !== 6) {
    throw new Error("Nevada publication source-document postcondition failed");
  }
  const runs = await nv.unsafe([
    ...
    "from import_runs where state_code='NV'",
    " and summary->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256]);
  if (Number(runs[0]?.total) !== 3 || Number(runs[0]?.exact) !== 3) {
    throw new Error("Nevada publication import-run postcondition failed");
  }
  ...
  if (Number(invalid[0]?.count) !== 0) {
    throw new Error("Nevada publication left invalid public constraints");
  }
  return {
    geographyVersions: 3,
    crosswalks: 5_230,
...
export async function applyNevadaGeographyPublicationTransaction(nv, context) {
  ...
  if (
    ...
    throw new Error("Nevada publication transaction database identity drifted");
  }
  await nv.unsafe("select set_config('lock_timeout','30s',true)");
  await nv.unsafe(
    "select pg_advisory_xact_lock(hashtextextended('crm-nv-precinct-gis-release-v1',0))",
  );
  const revisions = await nv.unsafe(
    "select revision::int revision from public_data_revisions where scope='public' for update",
  );
  if (revisions.length !== 1) throw new Error("Nevada public revision is missing");
  const revision = Number(revisions[0].revision) + 1;
  const gisPlan = context.gisPlan;
  await validateNevadaPrecinctGisClient(nv, gisPlan, {
  ...
  if (versions.length !== 3) {
    throw new Error("Nevada publication requires three blocked geography versions");
  }
  for (const [index, version] of versions.entries()) {
    ...
    if (
      ...
      throw new Error("Nevada " + manifest.year + " publication precondition drifted");
    }
    ...
    const updated = await nv.unsafe([
      ...
      version.id,
      "Reviewed Nevada election-specific precinct geometry is publicly authorized under activation "
        + context.authorization.activationId + ".",
      JSON.stringify(activation),
    ]);
    if (updated.length !== 1) {
      throw new Error("Nevada " + manifest.year + " publication update lost its lock");
    }
  }
  ...
  const sources = await nv.unsafe([
    ...
    " jsonb_set(jsonb_set(metadata,'{publicDeliveryAuthorized}','true'::jsonb,true),",
    " '{releaseCandidate,publicDeliveryAuthorized}','true'::jsonb,true)",
    "where state_code='NV' and metadata->'releaseCandidate'->>'sha256'=$1 returning id",
  ].join("\n"), [context.plan.releaseCandidate.sha256]);
  if (sources.length !== 6) {
    throw new Error("Nevada publication source-document update count drifted");
  }
  const runs = await nv.unsafe([
    "update import_runs set summary=",
    " jsonb_set(jsonb_set(summary,'{publicDeliveryAuthorized}','true'::jsonb,true),",
    " '{releaseCandidate,publicDeliveryAuthorized}','true'::jsonb,true)",
    "where state_code='NV' and summary->'releaseCandidate'->>'sha256'=$1 returning id",
  ].join("\n"), [context.plan.releaseCandidate.sha256]);
  if (runs.length !== 3) {
    throw new Error("Nevada publication import-run update count drifted");
  }
  const revisionRows = await nv.unsafe([
    ...
    "Nevada precinct geometry publish " + context.authorization.activationId,
  ]);
  if (Number(revisionRows[0]?.revision) !== revision) {
    throw new Error("Nevada publication revision increment drifted");
  }
...
function defaultPlanPath(planSha256) {
  return ".etl/precinct-publication-plans/NV/nv-publication-plan-"
    + planSha256.slice(0, 12) + ".json";
}

function defaultAuthorizationPath(planSha256) {
  return ".etl/production-authorizations/NV/nv-publication-authorization-template-"
    + planSha256.slice(0, 12) + ".json";
}

function defaultReceiptPath(planSha256, activationId) {
  const safeId = activationId.replace(/[^a-zA-Z0-9._-]+/g, "-").slice(0, 80);
  return ".etl/production-publication-receipts/NV/nv-publication-"
...
function reserveReceipt(
  ...
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ".etl", "production-publication-receipts", "NV");
  if (!absolute.startsWith(allowed + path.sep) || !relativePath.endsWith(".json")) {
    throw new Error("Nevada publication receipt path is unsafe");
  }
  if (existsSync(absolute)) throw new Error("Nevada publication receipt already exists");
  mkdirSync(path.dirname(absolute), { recursive: true });
  ...
  const pendingBytes = serializeNevadaPublicationDocument(pendingDocument);
  if (existsSync(pending)) {
    ...
    throw new Error("A Nevada publication recovery marker already exists; reconcile it before retrying");
  }
...
function finishPublicationReceipt(reservation, receipt) {
  const receiptBytes = serializeNevadaPublicationDocument(receipt);
  ...
  if (existsSync(temporary)) {
    if (!readFileSync(temporary).equals(receiptBytes)) {
      throw new Error("Nevada publication receipt temporary file drifted");
    }
  } else {
...
export async function runNevadaGeographyPublication(options = {}) {
  const root = path.resolve(options.root ?? process.cwd());
  const parsed = options.packagePath ? options : parseArguments(process.argv.slice(2));
  const built = inspectNevadaPublicationPlan({ ...parsed, root });
  if (parsed.planSha256 && parsed.planSha256 !== built.sha256) {
    throw new Error("Nevada publication plan SHA-256 drifted");
  }
  if (parsed.writePlan) {
    const artifact = immutableJson(
      ...
      built.plan,
      ".etl/precinct-publication-plans/NV",
    );
    return {
      mode: "write_plan",
  ...
  if (parsed.writeAuthorizationTemplate) {
    const artifact = immutableJson(
      ...
      buildNevadaPublicationAuthorizationTemplate(built.plan, built.sha256),
      ".etl/production-authorizations/NV",
    );
    return {
  ...
  const currentTime = () => {
    ...
    if (Number.isNaN(result.getTime())) {
      throw new Error("Nevada publication clock returned an invalid time");
    }
    return result;
  };
  const validateAuthorizationAt = (now) => validateNevadaPublicationAuthorization(
  ...
  if (endpointFingerprint !== built.plan.hiddenLoad.endpointFingerprint) {
    throw new Error("Nevada publication database endpoint drifted from the hidden load");
  }
  ...
  buildNevadaPrecinctExecutionContext(executionContext);
  ...
  if (parsed.recoverReceipt) {
    if (
      ...
      || environment.CRM_NV_PRECINCT_PUBLICATION_RECEIPT_RECOVERY !== built.sha256
      || environment.CRM_NV_PRECINCT_PUBLICATION_AUTHORIZATION_SHA256
        !== authorizationArtifact.sha256
    ) {
      throw new Error("Nevada publication receipt recovery is not explicitly read-only and hash-authorized");
    }
    ...
    try {
      sql = (options.postgresFactory ?? postgres)(databaseUrl, {
        ...
        idle_timeout: 20,
        connection: {
          application_name: "civicresultmaps-nv-precinct-publication-receipt-recovery",
    ...
    try {
      recovered = await sql.begin("read only", async (nv) => {
        ...
        if (
          ...
          throw new Error("Nevada publication receipt recovery database identity drifted");
        }
        const versions = await nv.unsafe([
        ...
        if (versions.length !== 3) {
          throw new Error("Nevada publication receipt recovery found an incomplete version set");
        }
        ...
        if (
          ...
          throw new Error("Nevada publication receipt recovery activation metadata is incomplete");
        }
        ...
        if (currentGitSha(root) !== authorization.productionDeployment.gitSha) {
          throw new Error("Nevada publication recovery checkout does not match the verified deployment");
        }
        const context = {
          plan: built.plan,
        ...
        if (
          ...
          || String(revisionRows[0]?.reason)
            !== "Nevada precinct geometry publish " + authorization.activationId
        ) {
          throw new Error("Nevada publication receipt recovery public revision drifted");
        }
  ...
  if (
    ...
    || environment.CRM_NV_PRECINCT_PUBLICATION_WRITES
      !== "I_ACKNOWLEDGE_ATOMIC_NEVADA_PRECINCT_PUBLIC_CUTOVER"
    || environment.CRM_NV_PRECINCT_PUBLICATION_PACKAGE_SHA256
      !== built.plan.releaseCandidate.sha256
    || environment.CRM_NV_PRECINCT_PUBLICATION_PLAN_SHA256 !== built.sha256
    || environment.CRM_NV_PRECINCT_PUBLICATION_AUTHORIZATION_SHA256
      !== authorizationArtifact.sha256
    || environment.CRM_NV_PRECINCT_PUBLICATION_ACTIVATION_ID
      !== authorization.activationId
  ) {
    throw new Error("Nevada public cutover is not explicitly hash-authorized");
  }
  if (currentGitSha(root) !== authorization.productionDeployment.gitSha) {
    throw new Error("Nevada publication checkout does not match the verified deployment");
  }
  const reservation = reserveReceipt(
    root,
    receiptPath,
  ...
  try {
    sql = (options.postgresFactory ?? postgres)(databaseUrl, {
      ...
      connect_timeout: 10,
      idle_timeout: 20,
      connection: { application_name: "civicresultmaps-nv-precinct-publication-status" },
    });
  } catch (error) {
  ...
  try {
    committed = await sql.begin(async (nv) => {
      ...
      if (
        ...
        throw new Error("Nevada publication authorization or checkout drifted before the transaction");
      }
      const result = await applyNevadaGeographyPublicationTransaction(nv, {
        plan: built.plan,
        planSha256: built.sha256,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  runNevadaGeographyPublication().then(
