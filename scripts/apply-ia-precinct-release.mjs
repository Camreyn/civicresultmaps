...
import {
  applyIowaPrecinctGisTransaction,
  readIowaPersistedProductionReleaseAudit,
  validateIowaPrecinctGisClient,
} from "./lib/ia-precinct-gis-db.mjs";
import { buildIowaPrecinctGisPlan } from "./lib/ia-precinct-gis-plan.mjs";
import { inspectReleaseArtifact } from "./lib/ia-precinct-release-candidate.mjs";
import {
  assertIowaReleaseCandidateDocument,
  productionEndpointFingerprint,
} from "./lib/ia-precinct-production-preflight.mjs";
import {
...
} from "./lib/ia-precinct-production-release.mjs";
...
function parseArguments(args) {
  ...
  for (const arg of args) {
    ...
    if (!known.has(key)) throw new Error("Unknown Iowa production-release option: " + arg);
  }
  if (!parsed.packagePath || !parsed.packageSha256) {
    throw new Error("Iowa production release requires --package and --package-sha256");
  }
  if ([parsed.apply, parsed.recoverReceipt, parsed.writeAuthorizationTemplate]
    .filter(Boolean).length > 1) {
    throw new Error("Iowa production release modes are mutually exclusive");
  }
  return parsed;
}
...
function safeEtlPath(root, relativePath, requiredPrefix) {
  if (
    ...
    throw new Error("Unsafe Iowa release evidence path: " + relativePath);
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ...requiredPrefix.replace(/\/$/, "").split("/"));
  if (!absolute.startsWith(allowed + path.sep)) {
    throw new Error("Iowa release evidence escapes its .etl root");
  }
  return absolute;
}
...
function readEtlJson(root, relativePath, expectedSha256, prefix) {
  if (!/^[a-f0-9]{64}$/.test(expectedSha256 ?? "")) {
    throw new Error("Iowa release evidence requires an exact SHA-256");
  }
  const absolute = safeEtlPath(root, relativePath, prefix);
  const bytes = readFileSync(absolute);
  if (sha256(bytes) !== expectedSha256) {
    throw new Error("Iowa release evidence SHA-256 drifted: " + relativePath);
  }
...
function readBackupManifest(relativeOrAbsolutePath, expectedSha256, root) {
  if (!/^[a-f0-9]{64}$/.test(expectedSha256 ?? "")) {
    throw new Error("Iowa backup manifest requires an exact SHA-256");
  }
  ...
  const bytes = readFileSync(absolute);
  if (sha256(bytes) !== expectedSha256) {
    throw new Error("Iowa backup manifest SHA-256 drifted");
  }
...
function productionUrl(environment = process.env) {
  ...
  if (!first && !second) {
    throw new Error("Iowa production release requires an explicit unpooled PostgreSQL URL");
  }
  if (first && second && first !== second) {
    throw new Error("Iowa production release found conflicting unpooled PostgreSQL URLs");
  }
  return first || second;
}
...
function immutableJson(root, relativePath, value) {
  ...
  if (existsSync(absolute)) {
    if (!readFileSync(absolute).equals(bytes)) {
      throw new Error("Refusing to overwrite different Iowa release evidence");
    }
...
function reserveReceipt(
  ...
  const absolute = safeEtlPath(
    root,
    relativePath,
    ".etl/production-release-receipts/IA/",
  );
  if (existsSync(absolute)) throw new Error("Iowa production receipt already exists");
  ...
  if (existsSync(pending)) {
    ...
    throw new Error("An Iowa release recovery marker already exists; reconcile it before retrying");
  }
...
function finishReceipt(reservation, value) {
  ...
  if (existsSync(temporary)) {
    if (!readFileSync(temporary).equals(bytes)) {
      throw new Error("Iowa hidden receipt temporary file drifted");
    }
  } else {
...
function defaultTemplatePath(packageSha256) {
  return ".etl/production-authorizations/IA/ia-precinct-authorization-template-"
...
function defaultReceiptPath(packageSha256, authorizationId) {
  ...
  return ".etl/production-release-receipts/IA/ia-precinct-hidden-load-"
    + packageSha256.slice(0, 12)
    + "-"
    + safeId
    + ".json";
...
async function ensureIowaDerivationConstraint(tx, migrationArtifact) {
  ...
  if (beforeRows.length !== 1) {
    throw new Error("Iowa derivation-method constraint is missing or ambiguous");
  }
  ...
  if (
    ...
    throw new Error("Iowa derivation-method constraint preimage drifted");
  }
  ...
  if (statements.length !== 2) {
    throw new Error("Iowa migration 0009 statement set drifted");
  }
  ...
  if (
    ...
    throw new Error("Iowa migration 0009 did not establish the required constraint");
  }
  return { before: "absent", after: "complete", statementsApplied: 2 };
}

export async function runIowaProductionRelease(options = {}) {
  ...
  const releaseCandidate = assertIowaReleaseCandidateDocument(
    packageDocument,
  ...
  const currentTime = () => {
    ...
    if (Number.isNaN(result.getTime())) {
      throw new Error("Iowa production release clock returned an invalid time");
    }
    return result;
  };
  if (parsed.writeAuthorizationTemplate) {
    ...
    const artifact = immutableJson(root, relativePath, buildIowaProductionAuthorizationTemplate({
      releaseCandidate,
  ...
  const preflight = readEtlJson(
    ...
    ".etl/production-preflight-candidates/IA/",
  );
  ...
  const validateEvidenceAt = (now) => {
    const preflightSummary = validateIowaProductionPreflightEvidence(
      preflight.value,
      { releaseCandidate, endpointFingerprint, now },
    );
    const backupSummary = validateIowaProductionBackupEvidence(backup.value, {
      releaseCandidate,
      endpointFingerprint,
      preflightCapturedAtUtc: preflightSummary.capturedAtUtc,
      now,
    });
    const authorizationSummary = validateIowaProductionAuthorization(
      authorization.value,
      {
        releaseCandidate,
  ...
  if (parsed.recoverReceipt) {
    if (
      ...
      || environment.CRM_IA_PRECINCT_HIDDEN_RECEIPT_RECOVERY
        !== packageArtifact.sha256
      || environment.CRM_IA_PRECINCT_PRODUCTION_AUTHORIZATION_SHA256
        !== authorization.sha256
    ) {
      throw new Error("Iowa hidden receipt recovery is not explicitly read-only and hash-authorized");
    }
    ...
    try {
      sql = (options.postgresFactory ?? postgres)(databaseUrl, {
        ...
        idle_timeout: 20,
        connection: {
          application_name: "civicresultmaps-ia-precinct-hidden-receipt-recovery",
    ...
    try {
      recovered = await sql.begin("read only", async (nv) => {
        const persistedAudit = await readIowaPersistedProductionReleaseAudit(
          nv,
        ...
        if (committedAt.getTime() > currentTime().getTime()) {
          throw new Error("Iowa hidden receipt recovery found a future commit timestamp");
        }
        ...
        if (JSON.stringify(persistedAudit) !== JSON.stringify(expectedAudit)) {
          throw new Error("Iowa hidden receipt recovery evidence does not match the durable database audit");
        }
        ...
        const validation = await validateIowaPrecinctGisClient(nv, plan, {
          executionContext,
          readOnlySession: true,
        });
        if (Number(validation.revision) !== persistedAudit.transaction.publicRevision) {
          throw new Error("Iowa hidden receipt recovery public revision drifted");
        }
  ...
  if (
    ...
    || environment.CRM_IA_PRECINCT_PRODUCTION_WRITES !== packageArtifact.sha256
  ) {
    throw new Error("Iowa production writes are not explicitly package-authorized");
  }
  if (
    environment.CRM_IA_PRECINCT_PRODUCTION_AUTHORIZATION_ID
      !== initialEvidence.authorizationSummary.authorizationId
    || environment.CRM_IA_PRECINCT_PRODUCTION_AUTHORIZATION_SHA256
      !== authorization.sha256
  ) {
    throw new Error("Iowa production authorization environment acknowledgement drifted");
  }
  const reservation = reserveReceipt(
    root,
    receiptPath,
  ...
  try {
    sql = (options.postgresFactory ?? postgres)(databaseUrl, {
      ...
      connect_timeout: 10,
      idle_timeout: 20,
      connection: { application_name: "civicresultmaps-ia-precinct-production-release" },
    });
  } catch (error) {
  ...
  try {
    committed = await sql.begin(async (nv) => {
      ...
      const migration0009 = await ensureIowaDerivationConstraint(
        nv,
      ...
      if (revisions.length !== 1) throw new Error("Iowa production public revision is missing");
      ...
      const applied = await applyIowaPrecinctGisTransaction(nv, plan, {
        executionContext,
      });
      if (applied.revision !== expectedRevision) {
        throw new Error("Iowa production public revision increment drifted");
      }
      const validation = await validateIowaPrecinctGisClient(nv, plan, {
        executionContext,
