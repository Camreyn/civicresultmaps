...
import { inspectReleaseArtifact } from "./lib/mn-precinct-release-candidate.mjs";
import {
  assertMinnesotaReleaseCandidateDocument,
  collectMinnesotaProductionPreflight,
...
function parseArguments(args) {
  ...
  for (const arg of args) {
    if (
      ...
      throw new Error("Unknown Minnesota production-preflight option: " + arg);
    }
  }
  const packagePath = packageArgument?.slice("--package=".length);
  if (!packagePath) {
    throw new Error("--package is required for Minnesota production preflight");
  }
  return {
    packagePath,
...
function safeReportPath(root, requested, packageSha256, reportSha256) {
  const relativePath = requested ?? path.posix.join(
    ...
    "MN",
    "mn-precinct-production-preflight-"
  ...
  if (
    ...
    || !relativePath.startsWith(".etl/production-preflight-candidates/MN/")
  ) {
    throw new Error("Minnesota production preflight report must remain in its .etl root");
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ".etl", "production-preflight-candidates", "MN");
  if (!absolute.startsWith(allowed + path.sep)) {
    throw new Error("Minnesota production preflight report escapes its .etl root");
  }
...
function loadReleaseCandidate(root, relativePath) {
  if (
    ...
    throw new Error("Minnesota production preflight package path is unsafe");
  }
  ...
  const releaseCandidate = assertMinnesotaReleaseCandidateDocument(
    document,
    artifact.sha256,
  );
...
export async function runMinnesotaProductionPreflight(options = {}) {
  ...
  if (!parsed.connectReadOnly) {
    return {
      ...
      requiredEnvironment: {
        ...
        CRM_MN_PRODUCTION_PREFLIGHT_ACK: loaded.artifact.sha256,
  ...
  if (process.env.CRM_DATABASE_ENVIRONMENT !== "production-read-only") {
    throw new Error("Minnesota production preflight requires production-read-only environment");
  }
  if (process.env.CRM_MN_PRODUCTION_PREFLIGHT_ACK !== loaded.artifact.sha256) {
    throw new Error("Minnesota production preflight acknowledgement must equal the package SHA-256");
  }
  ...
  const sql = (options.postgresFactory ?? postgres)(databaseUrl, {
    ...
    idle_timeout: 20,
    connection: {
      application_name: "civicresultmaps-mn-precinct-production-preflight",
      default_transaction_read_only: true,
    },
  });
  let report;
  try {
    report = await sql.begin("read only", (tx) =>
  ...
  if (existsSync(output.absolute)) {
    if (!readFileSync(output.absolute).equals(bytes)) {
      throw new Error("Refusing to overwrite different Minnesota preflight evidence");
    }
  } else {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    console.log(JSON.stringify(await runMinnesotaProductionPreflight(), null, 2));
  } catch (error) {
