...
import {
  inspectFloridaPublicActivationPlan,
} from "./lib/fl-precinct-public-activation.mjs";
...
function parseArguments(args) {
  ...
  for (const arg of args) {
    if (
      ...
      throw new Error("Unknown Florida public-activation option: " + arg);
    }
  }
  if (!packagePath || !packageSha256) {
    throw new Error(
      "Florida public activation requires --package and --package-sha256",
    );
  }
...
function immutableWrite(root, relativePath, bytes) {
  ...
  const allowed = path.resolve(root, ".etl", "precinct-public-activations", "FL");
  if (!absolutePath.startsWith(allowed + path.sep)) {
    throw new Error("Florida activation evidence escapes its fixed .etl root");
  }
  if (existsSync(absolutePath)) {
    if (!readFileSync(absolutePath).equals(bytes)) {
      throw new Error("Refusing to overwrite different Florida activation evidence");
    }
...
export function writeFloridaActivationTrackedOutputs(outputs) {
  if (outputs.length !== 4 || new Set(outputs.map((item) => item.path)).size !== 4) {
    throw new Error("Florida activation requires exactly four unique tracked outputs");
  }
  const staged = [];
  const committed = [];
  try {
    for (const output of outputs) {
      ...
      if (before.length !== output.preimage.byteCount) {
        throw new Error("Florida activation target changed after plan: " + output.path);
      }
      const currentHash = createHash("sha256").update(before).digest("hex");
      if (currentHash !== output.preimage.sha256) {
        throw new Error("Florida activation target preimage drifted: " + output.path);
      }
      const temporaryPath = output.absolutePath
        + ".crm-fl-activation-" + randomUUID() + ".tmp";
    ...
    for (const item of committed.reverse()) {
      const rollbackPath = item.output.absolutePath
        + ".crm-fl-rollback-" + randomUUID() + ".tmp";
...
export async function prepareFloridaPublicActivation(options = {}) {
  const root = path.resolve(options.root ?? process.cwd());
  const built = inspectFloridaPublicActivationPlan({ ...options, root });
  if (!options.write) {
    return {
      mode: "plan",
  ...
  const trackedOutputs = writeFloridaActivationTrackedOutputs(built.outputs);
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  prepareFloridaPublicActivation(parseArguments(process.argv.slice(2))).then(
