...
const state = "SD";
const sourceId = "sd-historical-presidential-wikipedia-county";
const output = "data/sd-historical-presidential-baseline.csv";
...
function parseCountyRows(table, year) {
  ...
  for (const chunk of table.split(/^\|-/m).slice(1)) {
    ...
    rows.push({
      ...
      source_url: "https://en.wikipedia.org/wiki/" + year + "_United_States_presidential_election_in_South_Dakota",
      dem_votes: demVotes,
      rep_votes: repVotes,
...
async function fetchRows(year) {
  const page = year + "_United_States_presidential_election_in_South_Dakota";
...
for (const year of years) {
  ...
  if (yearRows.length !== 66) throw new Error(year + " expected 66 county rows, got " + yearRows.length);
}
