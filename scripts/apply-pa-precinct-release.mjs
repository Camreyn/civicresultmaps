...
import {
  applyPennsylvaniaPrecinctGisTransaction,
  readPennsylvaniaPersistedProductionReleaseAudit,
...
import { inspectReleaseArtifact } from "./lib/pa-precinct-release-candidate.mjs";
import {
  assertPennsylvaniaReleaseCandidateDocument,
  productionEndpointFingerprint,
} from "./lib/pa-precinct-production-preflight.mjs";
import {
...
} from "./lib/pa-precinct-production-release.mjs";

export const PENNSYLVANIA_PRODUCTION_BACKUP_ROOT = path.resolve(
  "C:\\tmp\\crm-db-clone\\pa-precinct-release-backups",
);
...
function parseArguments(args) {
  ...
  for (const arg of args) {
    ...
    if (!known.has(key)) throw new Error("Unknown Pennsylvania production-release option: " + arg);
  }
  if (!parsed.packagePath || !parsed.packageSha256) {
    throw new Error("Pennsylvania production release requires --package and --package-sha256");
  }
  if ([parsed.apply, parsed.recoverReceipt, parsed.writeAuthorizationTemplate]
    .filter(Boolean).length > 1) {
    throw new Error("Pennsylvania production release modes are mutually exclusive");
  }
  return parsed;
}
...
function safeEtlPath(root, relativePath, requiredPrefix) {
  if (
    ...
    throw new Error("Unsafe Pennsylvania release evidence path: " + relativePath);
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ...requiredPrefix.replace(/\/$/, "").split("/"));
  if (!absolute.startsWith(allowed + path.sep)) {
    throw new Error("Pennsylvania release evidence escapes its .etl root");
  }
  return absolute;
}
...
function readEtlJson(root, relativePath, expectedSha256, prefix) {
  if (!/^[a-f0-9]{64}$/.test(expectedSha256 ?? "")) {
    throw new Error("Pennsylvania release evidence requires an exact SHA-256");
  }
  const absolute = safeEtlPath(root, relativePath, prefix);
  const bytes = readFileSync(absolute);
  if (sha256(bytes) !== expectedSha256) {
    throw new Error("Pennsylvania release evidence SHA-256 drifted: " + relativePath);
  }
  return { path: relativePath, absolute, bytes, sha256: expectedSha256, value: JSON.parse(bytes) };
}

export function readPennsylvaniaBackupManifest(
  manifestPath,
  expectedSha256,
  options = {},
) {
  if (!/^[a-f0-9]{64}$/.test(expectedSha256 ?? "")) {
    throw new Error("Pennsylvania backup manifest requires an exact SHA-256");
  }
  if (typeof manifestPath !== "string" || !path.isAbsolute(manifestPath)) {
    throw new Error("Pennsylvania backup manifest must use its fixed absolute backup path");
  }
  const backupRoot = path.resolve(
    options.backupRoot ?? PENNSYLVANIA_PRODUCTION_BACKUP_ROOT,
  );
  const absolute = path.resolve(manifestPath);
  const relative = path.relative(backupRoot, absolute);
  if (
    !relative
    ...
    || !/^pa-precinct-full-public-\d{8}T\d{6}Z\.manifest\.json$/.test(relative)
  ) {
    throw new Error("Pennsylvania backup manifest escapes its fixed backup root");
  }
  const bytes = readFileSync(absolute);
  if (sha256(bytes) !== expectedSha256) {
    throw new Error("Pennsylvania backup manifest SHA-256 drifted");
  }
  const value = JSON.parse(bytes);
  const expectedDumpFile = relative.replace(/\.manifest\.json$/, ".dump");
  if (
    value?.dumpFile !== expectedDumpFile
    || !/^[a-f0-9]{64}$/.test(value?.dumpSha256 ?? "")
  ) {
    throw new Error("Pennsylvania backup manifest dump identity is unsafe");
  }
  const dumpAbsolute = path.resolve(backupRoot, expectedDumpFile);
  if (
    path.relative(backupRoot, dumpAbsolute) !== expectedDumpFile
    || !existsSync(dumpAbsolute)
    || sha256(readFileSync(dumpAbsolute)) !== value.dumpSha256
  ) {
    throw new Error("Pennsylvania backup dump is missing or hash-drifted");
  }
  return {
    absolute,
    bytes,
...
function productionUrl(environment = process.env) {
  ...
  if (!first && !second) {
    throw new Error("Pennsylvania production release requires an explicit unpooled PostgreSQL URL");
  }
  if (first && second && first !== second) {
    throw new Error("Pennsylvania production release found conflicting unpooled PostgreSQL URLs");
  }
  return first || second;
}
...
function immutableJson(root, relativePath, value) {
  ...
  if (existsSync(absolute)) {
    if (!readFileSync(absolute).equals(bytes)) {
      throw new Error("Refusing to overwrite different Pennsylvania release evidence");
    }
...
function reserveReceipt(
  ...
  const absolute = safeEtlPath(
    root,
    relativePath,
    ".etl/production-release-receipts/PA/",
  );
  if (existsSync(absolute)) throw new Error("Pennsylvania production receipt already exists");
  ...
  if (existsSync(pending)) {
    ...
    throw new Error("A Pennsylvania release recovery marker already exists; reconcile it before retrying");
  }
...
function finishReceipt(reservation, value) {
  ...
  if (existsSync(temporary)) {
    if (!readFileSync(temporary).equals(bytes)) {
      throw new Error("Pennsylvania hidden receipt temporary file drifted");
    }
  } else {
...
function defaultTemplatePath(packageSha256) {
  return ".etl/production-authorizations/PA/pa-precinct-authorization-template-"
...
function defaultReceiptPath(packageSha256, authorizationId) {
  ...
  return ".etl/production-release-receipts/PA/pa-precinct-hidden-load-"
    + packageSha256.slice(0, 12)
    + "-"
    + safeId
    + ".json";
...
async function ensurePennsylvaniaDerivationConstraint(tx, migrationArtifact) {
  ...
  if (beforeRows.length !== 1) {
    throw new Error("Pennsylvania derivation-method constraint is missing or ambiguous");
  }
  ...
  if (
    ...
    throw new Error("Pennsylvania derivation-method constraint preimage drifted");
  }
  ...
  if (statements.length !== 2) {
    throw new Error("Pennsylvania migration 0009 statement set drifted");
  }
  ...
  if (
    ...
    throw new Error("Pennsylvania migration 0009 did not establish the required constraint");
  }
  return { before: "absent", after: "complete", statementsApplied: 2 };
}

export async function runPennsylvaniaProductionRelease(options = {}) {
  ...
  const releaseCandidate = assertPennsylvaniaReleaseCandidateDocument(
    packageDocument,
  ...
  const currentTime = () => {
    ...
    if (Number.isNaN(result.getTime())) {
      throw new Error("Pennsylvania production release clock returned an invalid time");
    }
    return result;
  };
  if (parsed.writeAuthorizationTemplate) {
    ...
    const artifact = immutableJson(root, relativePath, buildPennsylvaniaProductionAuthorizationTemplate({
      releaseCandidate,
  ...
  const preflight = readEtlJson(
    ...
    ".etl/production-preflight-candidates/PA/",
  );
  const backup = readPennsylvaniaBackupManifest(
  ...
  const validateEvidenceAt = (now) => {
    const preflightSummary = validatePennsylvaniaProductionPreflightEvidence(
      preflight.value,
      { releaseCandidate, endpointFingerprint, now },
    );
    const backupSummary = validatePennsylvaniaProductionBackupEvidence(backup.value, {
      releaseCandidate,
      endpointFingerprint,
      preflightCapturedAtUtc: preflightSummary.capturedAtUtc,
      now,
    });
    const authorizationSummary = validatePennsylvaniaProductionAuthorization(
      authorization.value,
      {
        releaseCandidate,
  ...
  if (parsed.recoverReceipt) {
    if (
      ...
      || environment.CRM_PA_PRECINCT_GEOGRAPHY_HIDDEN_RECEIPT_RECOVERY
        !== packageArtifact.sha256
      || environment.CRM_PA_PRECINCT_GEOGRAPHY_PRODUCTION_AUTHORIZATION_SHA256
        !== authorization.sha256
    ) {
      throw new Error("Pennsylvania hidden receipt recovery is not explicitly read-only and hash-authorized");
    }
    ...
    try {
      sql = (options.postgresFactory ?? postgres)(databaseUrl, {
        ...
        idle_timeout: 20,
        connection: {
          application_name: "civicresultmaps-pa-precinct-hidden-receipt-recovery",
    ...
    try {
      recovered = await sql.begin("read only", async (nv) => {
        const persistedAudit = await readPennsylvaniaPersistedProductionReleaseAudit(
          nv,
        ...
        if (committedAt.getTime() > currentTime().getTime()) {
          throw new Error("Pennsylvania hidden receipt recovery found a future commit timestamp");
        }
        ...
        if (JSON.stringify(persistedAudit) !== JSON.stringify(expectedAudit)) {
          throw new Error("Pennsylvania hidden receipt recovery evidence does not match the durable database audit");
        }
        ...
        const validation = await validatePennsylvaniaPrecinctGisClient(nv, plan, {
          executionContext,
          readOnlySession: true,
        });
        if (Number(validation.revision) !== persistedAudit.transaction.publicRevision) {
          throw new Error("Pennsylvania hidden receipt recovery public revision drifted");
        }
  ...
  if (
    ...
    || environment.CRM_PA_PRECINCT_GEOGRAPHY_PRODUCTION_WRITES !== packageArtifact.sha256
  ) {
    throw new Error("Pennsylvania production writes are not explicitly package-authorized");
  }
  if (
    environment.CRM_PA_PRECINCT_GEOGRAPHY_PRODUCTION_AUTHORIZATION_ID
      !== initialEvidence.authorizationSummary.authorizationId
    || environment.CRM_PA_PRECINCT_GEOGRAPHY_PRODUCTION_AUTHORIZATION_SHA256
      !== authorization.sha256
  ) {
    throw new Error("Pennsylvania production authorization environment acknowledgement drifted");
  }
  const reservation = reserveReceipt(
    root,
    receiptPath,
  ...
  try {
    sql = (options.postgresFactory ?? postgres)(databaseUrl, {
      ...
      connect_timeout: 10,
      idle_timeout: 20,
      connection: { application_name: "civicresultmaps-pa-precinct-production-release" },
    });
  } catch (error) {
  ...
  try {
    committed = await sql.begin(async (nv) => {
      ...
      const migration0009 = await ensurePennsylvaniaDerivationConstraint(
        nv,
      ...
      if (revisions.length !== 1) throw new Error("Pennsylvania production public revision is missing");
      ...
      const applied = await applyPennsylvaniaPrecinctGisTransaction(nv, plan, {
        executionContext,
      });
      if (applied.revision !== expectedRevision) {
        throw new Error("Pennsylvania production public revision increment drifted");
      }
      const validation = await validatePennsylvaniaPrecinctGisClient(nv, plan, {
        executionContext,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  runPennsylvaniaProductionRelease().then(
