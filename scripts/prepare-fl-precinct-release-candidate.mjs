...
import {
  buildFloridaPrecinctReleaseCandidate,
  floridaReleaseCandidateOutputRoot,
} from "./lib/fl-precinct-release-candidate.mjs";
...
function immutableWrite(root, relativePath, bytes) {
  ...
  const expectedRoot = path.resolve(root, ".etl", "precinct-release-candidates", "FL");
  if (!absolutePath.startsWith(expectedRoot + path.sep)) {
    throw new Error("Florida release output escapes its content-addressed root");
  }
  if (existsSync(absolutePath)) {
    if (!readFileSync(absolutePath).equals(bytes)) {
      throw new Error("Refusing to overwrite different Florida release bytes: " + relativePath);
    }
...
export async function prepareFloridaPrecinctReleaseCandidate(options = {}) {
  ...
  const outputRoot = floridaReleaseCandidateOutputRoot(packageSha256);
  const files = [
...
async function main() {
  ...
  if (unknown.length) throw new Error("Unknown Florida release-candidate option: " + unknown[0]);
  const validationReportPath = args.find((arg) => arg.startsWith("--validation-report="))
    ?.slice("--validation-report=".length);
  console.log(JSON.stringify(await prepareFloridaPrecinctReleaseCandidate({
