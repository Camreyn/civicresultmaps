...
import {
  ...
  texasReleaseCandidateOutputRoot,
} from "./lib/tx-precinct-release-candidate.mjs";
...
function immutableWrite(root, relativePath, bytes) {
  ...
  const expectedRoot = path.resolve(root, ".etl", "precinct-release-candidates", "TX");
  if (!absolutePath.startsWith(expectedRoot + path.sep)) {
    throw new Error("Texas release output escapes its content-addressed root");
  }
  if (existsSync(absolutePath)) {
    if (!readFileSync(absolutePath).equals(bytes)) {
      throw new Error("Refusing to overwrite different Texas release bytes: " + relativePath);
    }
...
export async function prepareTexasPrecinctReleaseCandidate(options = {}) {
  ...
  const outputRoot = texasReleaseCandidateOutputRoot(packageSha256);
  const files = [
...
async function main() {
  ...
  if (unknown.length) throw new Error("Unknown Texas release-candidate option: " + unknown[0]);
  const validationReportPath = args.find((arg) => arg.startsWith("--validation-report="))
    ?.slice("--validation-report=".length);
  console.log(JSON.stringify(await prepareTexasPrecinctReleaseCandidate({
