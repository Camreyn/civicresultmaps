...
for (const arg of args) {
  if (
    ...
    throw new Error("Unknown Texas GIS setup option: " + arg);
  }
}
...
try {
  ...
  if (!apply) {
    ...
    const {
      applyTexasPrecinctGisPlan,
      validateTexasPrecinctGisDatabase,
    } = await import("./lib/tx-precinct-gis-db.mjs");
    const applied = await applyTexasPrecinctGisPlan(plan);
    const validation = await validateTexasPrecinctGisDatabase(plan);
    const report = {
      schemaVersion: 1,
      generatedAtUtc: new Date().toISOString(),
      scope: "local-only Texas precinct GIS setup",
      productionMutationPerformed: false,
      publicDeliveryAuthorized: false,
      plan: summary,
      applied,
    ...
    const reportPath = writeReport(
      reportArgument
        ...
        : ".etl/local-db/tx-precinct-gis-setup-report.json",
      report,
    );
