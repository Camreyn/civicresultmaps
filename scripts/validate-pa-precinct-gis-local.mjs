...
for (const arg of args) {
  if (!arg.startsWith("--years=") && !arg.startsWith("--report=")) {
    throw new Error("Unknown Pennsylvania GIS validation option: " + arg);
  }
}
...
try {
  ...
  const { validatePennsylvaniaPrecinctGisDatabase } =
  ...
  const report = {
    ...
    scope: "local-only Pennsylvania precinct GIS validation",
    productionMutationPerformed: false,
    publicDeliveryAuthorized: false,
    plan: summarizePennsylvaniaPrecinctGisPlan(plan),
    validation,
  };
  const reportPath = writeReport(
    reportArgument
      ? reportArgument.slice("--report=".length)
      : ".etl/local-db/pa-precinct-gis-validation.json",
    report,
  );
