...
import {
  buildAlaskaPrecinctExecutionContext,
  validateAlaskaPrecinctGisClient,
} from "./lib/ak-precinct-gis-db.mjs";
import { buildAlaskaPrecinctGisPlan } from "./lib/ak-precinct-gis-plan.mjs";
import { productionEndpointFingerprint } from "./lib/ak-precinct-production-preflight.mjs";
import {
  ALASKA_PUBLIC_ROLLBACK_SCOPES,
  buildAlaskaPublicRollbackAuthorizationTemplate,
  buildAlaskaPublicationAuthorizationTemplate,
  inspectAlaskaPublicationPlan,
  semanticallyEqual,
  serializeAlaskaPublicationDocument,
  sha256,
  validateAlaskaPublicRollbackAuthorization,
  validateAlaskaPublicationAuthorization,
} from "./lib/ak-precinct-publication.mjs";
...
function parseArguments(args) {
  ...
  for (const arg of args) {
    if (
      ...
      throw new Error("Unknown Alaska publication-status option: " + arg);
    }
  }
  const modes = [
    parsed.writePlan,
  ...
  if (modes > 1) throw new Error("Alaska publication-status modes are mutually exclusive");
  for (const field of [
    "packagePath",
    "packageSha256",
    "hiddenReceiptPath",
    ...
    if (!parsed[field]) throw new Error("Alaska publication-status requires " + field);
  }
  if (
    parsed.rollback
    && (!parsed.publicationReceiptPath || !parsed.publicationReceiptSha256)
  ) {
    throw new Error("Alaska rollback requires the exact publication receipt and SHA-256");
  }
  return parsed;
}
...
function safeJson(root, relativePath, expectedSha256, allowedRoot) {
  if (
    ...
    throw new Error("Alaska publication-status JSON path is unsafe");
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ...allowedRoot.split("/"));
  if (!absolute.startsWith(allowed + path.sep) || !existsSync(absolute)) {
    throw new Error("Alaska publication-status JSON is missing");
  }
  const bytes = readFileSync(absolute);
  if (sha256(bytes) !== expectedSha256) {
    throw new Error("Alaska publication-status JSON SHA-256 drifted");
  }
  return {
    path: relativePath,
    absolute,
...
function immutableJson(root, relativePath, value, allowedRoot) {
  if (
    ...
    throw new Error("Alaska publication-status output path is unsafe");
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ...allowedRoot.split("/"));
  if (!absolute.startsWith(allowed + path.sep)) {
    throw new Error("Alaska publication-status output escapes its fixed root");
  }
  const bytes = serializeAlaskaPublicationDocument(value);
  if (existsSync(absolute)) {
    if (!readFileSync(absolute).equals(bytes)) {
      throw new Error("Refusing to overwrite different Alaska publication evidence");
    }
...
export function verifyAlaskaPublicationGitCandidate(
  root,
  publicAuthorization,
  runner = execFileSync,
) {
  ...
  try {
    ...
    throw new Error("Alaska publication Git evidence could not be resolved locally");
  }
  if (
    ...
    throw new Error("Alaska publication checkout or deployment Git evidence drifted");
  }
  return {
    gitSha: head,
    gitTreeSha: headTree,
...
async function verifyPostconditions(
  ...
  if (versions.length !== 4) {
    throw new Error("Alaska publication postcondition has an incomplete version set");
  }
  for (const [index, row] of versions.entries()) {
    ...
    const expectedCaveat = publicAuthorized
      ? "Reviewed Alaska election-specific precinct geometry is publicly authorized under activation "
    ...
    if (
      ...
      throw new Error("Alaska " + manifest.year + " geography publication drifted");
    }
  }
  const linked = await nv.unsafe([
    ...
    "   and x.geometry_feature_id is not null)",
    "  or (x.relationship_type='non_geographic'",
    "   and x.match_method='official_crosswalk'",
    "   and x.geometry_feature_id is null))",
    ...
    "where gv.state_code='AK' and gv.metadata->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256, String(publicAuthorized)]);
  if (Number(linked[0]?.total) !== 2_205 || Number(linked[0]?.exact) !== 2_205) {
    throw new Error("Alaska publication crosswalk postcondition failed");
  }
  const units = await nv.unsafe([
    ...
    "from reporting_units where state_code='AK'",
    " and reporting_grain in ('precinct','administrative_reporting_unit')",
    " and metadata->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256, String(publicAuthorized)]);
  if (Number(units[0]?.total) !== 2_205 || Number(units[0]?.exact) !== 2_205) {
    throw new Error("Alaska publication reporting-unit postcondition failed");
  }
  const sources = await nv.unsafe([
    "select count(*)::int total, count(*) filter (where",
    " metadata->>'publicDeliveryAuthorized'=$2 and",
    " metadata->'releaseCandidate'->>'publicDeliveryAuthorized'=$2)::int exact",
    "from source_documents where state_code='AK'",
    " and metadata->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256, String(publicAuthorized)]);
  if (Number(sources[0]?.total) !== 8 || Number(sources[0]?.exact) !== 8) {
    throw new Error("Alaska publication source-document postcondition failed");
  }
  const runs = await nv.unsafe([
    ...
    "from import_runs where state_code='AK'",
    " and summary->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256, String(publicAuthorized)]);
  if (Number(runs[0]?.total) !== 4 || Number(runs[0]?.exact) !== 4) {
    throw new Error("Alaska publication import-run postcondition failed");
  }
  const results = await nv.unsafe([
    ...
    "where rr.state_code='AK' and rr.level='precinct'",
    " and ru.metadata->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256]);
  if (Number(results[0]?.total) !== 12_021) {
    throw new Error("Alaska publication result-row postcondition failed");
  }
  ...
  if (Number(invalid[0]?.count) !== 0) {
    throw new Error("Alaska publication left invalid public constraints");
  }
  const revisionRows = await nv.unsafe([
    "select revision::int revision,reason from public_data_revisions",
    "where scope='public'",
  ].join("\n"));
  const expectedReason = "Alaska precinct unit geometry " + mode + " "
    + (publicAuthorized
  ...
  if (
    ...
    throw new Error("Alaska publication revision postcondition failed");
  }
  return {
    mode,
    status: wantedStatus,
...
export async function applyAlaskaGeographyPublicationTransaction(nv, context) {
  const mode = context.mode ?? "publish";
  if (!new Set(["publish", "rollback"]).has(mode)) {
    throw new Error("Alaska publication transaction mode is invalid");
  }
  const publicAuthorized = mode === "publish";
  if (mode === "rollback" && !context.publicationReceipt) {
    throw new Error("Alaska rollback requires the exact publication receipt");
  }
  if (mode === "rollback" && (
    ...
    throw new Error("Alaska rollback authorization drifted from the publication receipt");
  }
  ...
  if (
    ...
    throw new Error("Alaska publication transaction database identity drifted");
  }
  await nv.unsafe("select set_config('lock_timeout','30s',true)");
  await nv.unsafe(
    "select pg_advisory_xact_lock(hashtextextended('crm-ak-precinct-gis-release-v1',0))",
  );
  ...
  if (
    ...
    throw new Error("Alaska public revision is missing or invalid");
  }
  const revision = currentRevision + 1;
  const gisPlan = context.gisPlan;
  if (mode === "publish") {
    await (context.validateCurrentDatabase ?? validateAlaskaPrecinctGisClient)(nv, gisPlan, {
  ...
  if (versions.length !== 4) {
    throw new Error("Alaska publication requires four exact geography versions");
  }
  for (const [index, version] of versions.entries()) {
    ...
    if (mode === "publish" && (
      ...
      throw new Error("Alaska " + manifest.year + " publication precondition drifted");
    }
    if (mode === "rollback" && (
      commonDrift
    ...
    if (updated.length !== 1) {
      throw new Error("Alaska " + manifest.year + " publication update lost its lock");
    }
  }
  const crosswalks = await nv.unsafe([
    ...
    " and gv.state_code='AK' and gv.metadata->'releaseCandidate'->>'sha256'=$1",
    "returning x.id",
  ].join("\n"), [context.plan.releaseCandidate.sha256, JSON.stringify(publicAuthorized)]);
  if (crosswalks.length !== 2_205) {
    throw new Error("Alaska publication crosswalk update count drifted");
  }
  const units = await nv.unsafe([
  ...
  if (units.length !== 2_205) {
    throw new Error("Alaska publication reporting-unit update count drifted");
  }
  const sources = await nv.unsafe([
    "update source_documents set metadata=",
    " jsonb_set(jsonb_set(metadata,'{publicDeliveryAuthorized}',$2::text::jsonb,true),",
    " '{releaseCandidate,publicDeliveryAuthorized}',$2::text::jsonb,true)",
    "where state_code='AK' and metadata->'releaseCandidate'->>'sha256'=$1 returning id",
  ].join("\n"), [context.plan.releaseCandidate.sha256, JSON.stringify(publicAuthorized)]);
  if (sources.length !== 8) {
    throw new Error("Alaska publication source-document update count drifted");
  }
  const runs = await nv.unsafe([
    "update import_runs set summary=",
    " jsonb_set(jsonb_set(summary,'{publicDeliveryAuthorized}',$2::text::jsonb,true),",
    " '{releaseCandidate,publicDeliveryAuthorized}',$2::text::jsonb,true)",
    "where state_code='AK' and summary->'releaseCandidate'->>'sha256'=$1 returning id",
  ].join("\n"), [context.plan.releaseCandidate.sha256, JSON.stringify(publicAuthorized)]);
  if (runs.length !== 4) {
    throw new Error("Alaska publication import-run update count drifted");
  }
  const revisionRows = await nv.unsafe([
    ...
    "Alaska precinct unit geometry " + mode + " "
      + (publicAuthorized
        ? context.authorization.activationId
        : context.authorization.rollbackId),
  ]);
  if (Number(revisionRows[0]?.revision) !== revision) {
    throw new Error("Alaska publication revision increment drifted");
  }
...
function defaultPlanPath(planSha256) {
  return ".etl/precinct-publication-plans/AK/ak-precinct-publication-plan-"
    + planSha256.slice(0, 12) + ".json";
}

function defaultAuthorizationPath(planSha256, mode = "publish") {
  return ".etl/production-authorizations/AK/ak-precinct-" + mode + "-authorization-template-"
    + planSha256.slice(0, 12) + ".json";
}

export function inspectAlaskaPublicationReceipt(root, options, built) {
  const artifact = safeJson(
    root,
    options.publicationReceiptPath,
    options.publicationReceiptSha256,
    ".etl/production-publication-receipts/AK",
  );
  ...
  const publicAuthorizationArtifact = safeJson(
    ...
    ".etl/production-authorizations/AK",
  );
  const publicAuthorization = validateAlaskaPublicationAuthorization(
  ...
  if (
    ...
    || value?.postconditions?.geographyVersions !== 4
    || value?.postconditions?.crosswalks !== 2_205
    || value?.postconditions?.reportingUnits !== 2_205
    || value?.postconditions?.sourceDocuments !== 8
    || value?.postconditions?.importRuns !== 4
    || value?.postconditions?.resultRows !== 12_021
    || value?.postconditions?.invalidConstraints !== 0
    || value?.productionMutationPerformed !== true
    || value?.publicDeliveryAuthorized !== true
  ) {
    throw new Error("Alaska publication receipt is incomplete or incompatible");
  }
  return {
    artifact,
    publicAuthorization,
...
function defaultReceiptPath(planSha256, activationId, mode = "publish") {
  ...
  return ".etl/production-publication-receipts/AK/ak-precinct-" + mode + "-"
...
function reserveReceipt(
  ...
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ".etl", "production-publication-receipts", "AK");
  if (!absolute.startsWith(allowed + path.sep) || !relativePath.endsWith(".json")) {
    throw new Error("Alaska publication receipt path is unsafe");
  }
  if (existsSync(absolute)) throw new Error("Alaska publication receipt already exists");
  mkdirSync(path.dirname(absolute), { recursive: true });
  ...
  const pendingBytes = serializeAlaskaPublicationDocument(pendingDocument);
  if (existsSync(pending)) {
    ...
    throw new Error("A Alaska publication recovery marker already exists; reconcile it before retrying");
  }
...
function finishPublicationReceipt(reservation, receipt) {
  const receiptBytes = serializeAlaskaPublicationDocument(receipt);
  ...
  if (existsSync(temporary)) {
    if (!readFileSync(temporary).equals(receiptBytes)) {
      throw new Error("Alaska publication receipt temporary file drifted");
    }
  } else {
...
export async function runAlaskaGeographyPublication(options = {}) {
  ...
  const currentTime = () => {
    ...
    if (Number.isNaN(result.getTime())) {
      throw new Error("Alaska publication clock returned an invalid time");
    }
    return result;
  };
  const built = inspectAlaskaPublicationPlan({ ...parsed, root });
  if (parsed.planSha256 && parsed.planSha256 !== built.sha256) {
    throw new Error("Alaska publication plan SHA-256 drifted");
  }
  ...
  if (parsed.writePlan) {
    if (mode === "rollback") {
      throw new Error("Alaska rollback reuses the exact hash-pinned publication plan");
    }
    const artifact = immutableJson(
      root,
      parsed.outputPath ?? defaultPlanPath(built.sha256),
      built.plan,
      ".etl/precinct-publication-plans/AK",
    );
    return {
      mode: "write_plan",
  ...
  if (parsed.writeAuthorizationTemplate) {
    const template = mode === "rollback"
      ...
      : buildAlaskaPublicationAuthorizationTemplate(built.plan, built.sha256);
    const artifact = immutableJson(
      root,
  ...
  const validateAuthorizationAt = (now, recovery = false) => mode === "rollback"
    ? validateAlaskaPublicRollbackAuthorization(authorizationArtifact.value, {
      plan: built.plan,
      planSha256: built.sha256,
    ...
    : validateAlaskaPublicationAuthorization(authorizationArtifact.value, {
      plan: built.plan,
      planSha256: built.sha256,
      now: now.getTime(),
      recovery,
  ...
  if (endpointFingerprint !== built.plan.hiddenLoad.endpointFingerprint) {
    throw new Error("Alaska publication database endpoint drifted from the hidden load");
  }
  const gisPlan = await buildAlaskaPrecinctGisPlan({
    root,
  ...
  buildAlaskaPrecinctExecutionContext(executionContext);
  ...
  if (parsed.recoverReceipt) {
    if (
      ...
      || environment.CRM_AK_PRECINCT_GEOGRAPHY_PUBLICATION_RECEIPT_RECOVERY !== built.sha256
      || environment.CRM_AK_PRECINCT_GEOGRAPHY_PUBLICATION_AUTHORIZATION_SHA256
        !== authorizationArtifact.sha256
      || (mode === "rollback"
        && environment.CRM_AK_PRECINCT_GEOGRAPHY_PUBLICATION_RECEIPT_SHA256
          !== publicationReceipt.summary.sha256)
    ) {
      throw new Error(
        "Alaska publication receipt recovery is not explicitly read-only and hash-authorized",
      );
    }
    ...
    try {
      sql = (options.postgresFactory ?? postgres)(databaseUrl, {
        ...
        idle_timeout: 20,
        connection: {
          application_name: "civicresultmaps-ak-precinct-publication-receipt-recovery",
    ...
    try {
      recovered = await sql.begin("read only", async (nv) => {
        ...
        if (
          ...
          throw new Error("Alaska publication receipt recovery database identity drifted");
        }
        const versions = await nv.unsafe([
        ...
        if (
          ...
          throw new Error("Alaska publication receipt recovery audit is incomplete");
        }
  ...
  const writeAuthorized = mode === "publish"
    ? environment.CRM_AK_PRECINCT_GEOGRAPHY_PUBLICATION_WRITES
        === "I_ACKNOWLEDGE_ATOMIC_ALASKA_PRECINCT_PUBLIC_CUTOVER"
      && environment.CRM_AK_PRECINCT_GEOGRAPHY_PUBLICATION_ACTIVATION_ID
        === authorization.activationId
    : environment.CRM_AK_PRECINCT_GEOGRAPHY_ROLLBACK_WRITES
        === "I_ACKNOWLEDGE_DATABASE_FIRST_ALASKA_PRECINCT_PUBLIC_ROLLBACK"
      && environment.CRM_AK_PRECINCT_GEOGRAPHY_ROLLBACK_ID === authorization.rollbackId
      && environment.CRM_AK_PRECINCT_GEOGRAPHY_PUBLICATION_RECEIPT_SHA256
        === publicationReceipt.summary.sha256;
  if (
    environment.CRM_DATABASE_ENVIRONMENT !== "production"
    || !writeAuthorized
    || environment.CRM_AK_PRECINCT_GEOGRAPHY_PUBLICATION_PACKAGE_SHA256
      !== built.plan.releaseCandidate.sha256
    || environment.CRM_AK_PRECINCT_GEOGRAPHY_PUBLICATION_PLAN_SHA256 !== built.sha256
    || environment.CRM_AK_PRECINCT_GEOGRAPHY_PUBLICATION_AUTHORIZATION_SHA256
      !== authorizationArtifact.sha256
  ) {
    throw new Error("Alaska public " + mode + " is not explicitly hash-authorized");
  }
  const activePublicAuthorization = mode === "publish"
    ? authorization
    : publicAuthorization;
  verifyAlaskaPublicationGitCandidate(root, activePublicAuthorization, options.gitRunner);
  const reservation = reserveReceipt(
    root,
    receiptPath,
    built.sha256,
  ...
  try {
    sql = (options.postgresFactory ?? postgres)(databaseUrl, {
      ...
      connect_timeout: 10,
      idle_timeout: 20,
      connection: { application_name: "civicresultmaps-ak-precinct-publication-status" },
    });
  } catch (error) {
  ...
  try {
    committed = await sql.begin(async (nv) => {
      ...
      if (finalAuthorization.activationId !== authorization.activationId) {
        throw new Error("Alaska publication authorization drifted before the transaction");
      }
      ...
      const result = await applyAlaskaGeographyPublicationTransaction(nv, {
        mode,
        plan: built.plan,
        planSha256: built.sha256,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  runAlaskaGeographyPublication().then(
