...
import {
  inspectFloridaPrecinctBlobPublicationPlan,
  validateFloridaBlobPublicationAuthorization,
} from "./lib/fl-precinct-blob-publication.mjs";
...
function parseArguments(args) {
  ...
  for (const argument of args) {
    if (
      ...
      throw new Error("Unknown Florida Blob publication option: " + argument);
    }
  }
  ...
  if (!Number.isInteger(concurrency) || concurrency < 1 || concurrency > 8) {
    throw new Error("Florida Blob publication concurrency must be 1 through 8");
  }
...
async function verifyPublicBlob(url, artifact) {
  ...
  if (!response.ok) {
    throw new Error(
      "Published Florida geometry returned HTTP " + response.status,
    );
  }
  const bytes = Buffer.from(await response.arrayBuffer());
  if (bytes.length !== artifact.byteCount || sha256(bytes) !== artifact.sha256) {
    throw new Error("Published Florida geometry failed byte/hash verification");
  }
}
...
async function publishArtifact(artifact, sdk) {
  ...
  if (existing) {
    if (existing.size !== artifact.byteCount) {
      throw new Error(
        "Existing immutable Florida geometry has a different byte count",
      );
    }
  ...
  if (
    ...
    throw new Error("Published Florida geometry URL/pathname drifted");
  }
...
export async function publishFloridaPrecinctDeliveryAssets(options) {
  ...
  const authorization = validateFloridaBlobPublicationAuthorization(plan);
  ...
  if (origins.size !== 1) {
    throw new Error("Florida geometry assets were not published to one origin");
  }
...
async function main() {
  ...
  const result = await publishFloridaPrecinctDeliveryAssets(args);
