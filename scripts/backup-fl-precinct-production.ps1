...
$VerifyDatabase = 'crm_fl_precinct_restore_verify'
$BackupRoot = 'C:\tmp\crm-db-clone\fl-precinct-release-backups'
$ContainerBackupRoot = '/backups/fl-precinct-release-backups'
$ExpectedLegacyEndpointFingerprint = 'bf2bf2213814'

function Fail([string]$Message) {
  throw "Florida production backup aborted: $Message"
}
...
function Assert-ReleasePackage {
  $candidateRoot = [IO.Path]::GetFullPath((Join-Path $RepoRoot '.etl\precinct-release-candidates\FL'))
  ...
  if (
    ...
    Fail 'The release package must be an existing release-candidate.json under .etl/precinct-release-candidates/FL.'
  }
  ...
  if (
    ...
    $document.id -ne 'fl-precinct-gis-three-election-v1' -or
    $document.state -ne 'FL' -or
...
if (-not $Execute) {
  [ordered]@{
    ...
    acknowledgementRequired = @(
      'CRM_FL_PRECINCT_GEOGRAPHY_BACKUP_ACK=CREATE_FULL_PUBLIC_SCHEMA_ROLLBACK_BACKUP',
      'CRM_FL_PRECINCT_GEOGRAPHY_BACKUP_ENDPOINT_FINGERPRINT=<fresh 64-hex preflight endpoint fingerprint>'
    )
...
if (
  [Environment]::GetEnvironmentVariable('CRM_FL_PRECINCT_GEOGRAPHY_BACKUP_ACK') -ne 'CREATE_FULL_PUBLIC_SCHEMA_ROLLBACK_BACKUP' -or
  [Environment]::GetEnvironmentVariable('CRM_FL_PRECINCT_GEOGRAPHY_BACKUP_PACKAGE_SHA256') -ne $ReleasePackageSha256
) {
...
if (
  ...
  [Environment]::GetEnvironmentVariable('CRM_FL_PRECINCT_GEOGRAPHY_BACKUP_ENDPOINT_FINGERPRINT') -ne $sourceEndpointFingerprint
) {
...
$dumpFile = "fl-precinct-full-public-$stamp.dump"
$dumpPath = Join-Path $BackupRoot $dumpFile
$containerDumpPath = "$ContainerBackupRoot/$dumpFile"
$manifestPath = Join-Path $BackupRoot "fl-precinct-full-public-$stamp.manifest.json"
