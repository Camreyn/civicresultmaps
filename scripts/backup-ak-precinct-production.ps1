...
$VerifyDatabase = 'crm_ak_precinct_restore_verify'
$BackupRoot = 'C:\tmp\crm-db-clone\ak-precinct-release-backups'
$ContainerBackupRoot = '/backups/ak-precinct-release-backups'
$ExpectedLegacyEndpointFingerprint = 'bf2bf2213814'

function Fail([string]$Message) {
  throw "Alaska production backup aborted: $Message"
}
...
function Assert-ReleasePackage {
  $candidateRoot = [IO.Path]::GetFullPath((Join-Path $RepoRoot '.etl\precinct-release-candidates\AK'))
  ...
  if (
    ...
    Fail 'The release package must be an existing release-candidate.json under .etl/precinct-release-candidates/AK.'
  }
  ...
  if (
    ...
    $document.id -ne 'ak-precinct-gis-four-election-v1' -or
    $document.state -ne 'AK' -or
...
if (-not $Execute) {
  [ordered]@{
    ...
    acknowledgementRequired = @(
      'CRM_AK_PRECINCT_GEOGRAPHY_BACKUP_ACK=CREATE_FULL_PUBLIC_SCHEMA_ROLLBACK_BACKUP',
      'CRM_AK_PRECINCT_GEOGRAPHY_BACKUP_ENDPOINT_FINGERPRINT=<fresh 64-hex preflight endpoint fingerprint>'
    )
...
if (
  [Environment]::GetEnvironmentVariable('CRM_AK_PRECINCT_GEOGRAPHY_BACKUP_ACK') -ne 'CREATE_FULL_PUBLIC_SCHEMA_ROLLBACK_BACKUP' -or
  [Environment]::GetEnvironmentVariable('CRM_AK_PRECINCT_GEOGRAPHY_BACKUP_PACKAGE_SHA256') -ne $ReleasePackageSha256
) {
...
if (
  ...
  [Environment]::GetEnvironmentVariable('CRM_AK_PRECINCT_GEOGRAPHY_BACKUP_ENDPOINT_FINGERPRINT') -ne $sourceEndpointFingerprint
) {
...
$dumpFile = "ak-precinct-full-public-$stamp.dump"
$dumpPath = Join-Path $BackupRoot $dumpFile
$containerDumpPath = "$ContainerBackupRoot/$dumpFile"
$manifestPath = Join-Path $BackupRoot "ak-precinct-full-public-$stamp.manifest.json"
