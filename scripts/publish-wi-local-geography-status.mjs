...
import {
  buildWisconsinLocalExecutionContext,
  validateWisconsinLocalGisClient,
} from "./lib/wi-local-gis-db.mjs";
import { buildWisconsinLocalGisPlan } from "./lib/wi-local-gis-plan.mjs";
import { productionEndpointFingerprint } from "./lib/wi-local-production-preflight.mjs";
import {
  ...
  inspectWisconsinPublicationPlan,
  semanticallyEqual,
  serializeWisconsinPublicationDocument,
  sha256,
  validateWisconsinPublicRollbackAuthorization,
  validateWisconsinPublicationAuthorization,
} from "./lib/wi-local-publication.mjs";
...
function parseArguments(args) {
  ...
  for (const arg of args) {
    if (
      ...
      throw new Error("Unknown Wisconsin publication-status option: " + arg);
    }
  }
  const modes = [
    parsed.writePlan,
  ...
  if (modes > 1) throw new Error("Wisconsin publication-status modes are mutually exclusive");
  for (const field of [
    "packagePath",
    "packageSha256",
    "hiddenReceiptPath",
    ...
    if (!parsed[field]) throw new Error("Wisconsin publication-status requires " + field);
  }
  if (
    parsed.rollback
    && (!parsed.publicationReceiptPath || !parsed.publicationReceiptSha256)
  ) {
    throw new Error("Wisconsin rollback requires the exact publication receipt and SHA-256");
  }
  return parsed;
}
...
function safeJson(root, relativePath, expectedSha256, allowedRoot) {
  if (
    ...
    throw new Error("Wisconsin publication-status JSON path is unsafe");
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ...allowedRoot.split("/"));
  if (!absolute.startsWith(allowed + path.sep) || !existsSync(absolute)) {
    throw new Error("Wisconsin publication-status JSON is missing");
  }
  const bytes = readFileSync(absolute);
  if (sha256(bytes) !== expectedSha256) {
    throw new Error("Wisconsin publication-status JSON SHA-256 drifted");
  }
  return {
    path: relativePath,
    absolute,
...
function immutableJson(root, relativePath, value, allowedRoot) {
  if (
    ...
    throw new Error("Wisconsin publication-status output path is unsafe");
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ...allowedRoot.split("/"));
  if (!absolute.startsWith(allowed + path.sep)) {
    throw new Error("Wisconsin publication-status output escapes its fixed root");
  }
  const bytes = serializeWisconsinPublicationDocument(value);
  if (existsSync(absolute)) {
    if (!readFileSync(absolute).equals(bytes)) {
      throw new Error("Refusing to overwrite different Wisconsin publication evidence");
    }
...
export function verifyWisconsinPublicationGitCandidate(
  root,
  publicAuthorization,
  runner = execFileSync,
) {
  ...
  try {
    ...
    throw new Error("Wisconsin publication Git evidence could not be resolved locally");
  }
  if (
    ...
    throw new Error("Wisconsin publication checkout or deployment Git evidence drifted");
  }
  return {
    gitSha: head,
    gitTreeSha: headTree,
...
async function verifyPostconditions(
  ...
  const versions = await nv.unsafe([
    ...
    "where gv.state_code='WI' and gv.geography_type='local_reporting_unit'",
    " and gv.metadata->'releaseCandidate'->>'sha256'=$1",
    "order by e.year",
  ].join("\n"), [context.plan.releaseCandidate.sha256]);
  if (versions.length !== 3) {
    throw new Error("Wisconsin publication postcondition has an incomplete version set");
  }
  for (const [index, row] of versions.entries()) {
    ...
    const expectedCaveat = publicAuthorized
      ? "Reviewed Wisconsin election-specific local reporting geometry is publicly authorized under activation "
    ...
    if (
      ...
      throw new Error("Wisconsin " + manifest.year + " geography publication drifted");
    }
  }
  const linked = await nv.unsafe([
    "select count(*)::int total,",
    " count(*) filter (where ((x.relationship_type='one_to_one'",
    "   and x.match_method in ('reviewed_name','spatial_review')",
    ...
    "where gv.state_code='WI' and gv.metadata->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256, String(publicAuthorized)]);
  if (Number(linked[0]?.total) !== 10_937 || Number(linked[0]?.exact) !== 10_937) {
    throw new Error("Wisconsin publication crosswalk postcondition failed");
  }
  const units = await nv.unsafe([
    ...
    "from reporting_units where state_code='WI'",
    " and reporting_grain in ('local_reporting_unit','administrative_reporting_unit')",
    " and metadata->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256, String(publicAuthorized)]);
  if (Number(units[0]?.total) !== 10_937 || Number(units[0]?.exact) !== 10_937) {
    throw new Error("Wisconsin publication reporting-unit postcondition failed");
  }
  const sources = await nv.unsafe([
    "select count(*)::int total, count(*) filter (where",
    " metadata->>'publicDeliveryAuthorized'=$2 and",
    " metadata->'releaseCandidate'->>'publicDeliveryAuthorized'=$2)::int exact",
    "from source_documents where state_code='WI'",
    " and metadata->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256, String(publicAuthorized)]);
  if (Number(sources[0]?.total) !== 6 || Number(sources[0]?.exact) !== 6) {
    throw new Error("Wisconsin publication source-document postcondition failed");
  }
  const runs = await nv.unsafe([
    ...
    "from import_runs where state_code='WI'",
    " and summary->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256, String(publicAuthorized)]);
  if (Number(runs[0]?.total) !== 3 || Number(runs[0]?.exact) !== 3) {
    throw new Error("Wisconsin publication import-run postcondition failed");
  }
  const results = await nv.unsafe([
    ...
    "where rr.state_code='WI' and rr.level='local_reporting_unit'",
    " and ru.metadata->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256]);
  if (Number(results[0]?.total) !== 32_475) {
    throw new Error("Wisconsin publication result-row postcondition failed");
  }
  ...
  if (Number(invalid[0]?.count) !== 0) {
    throw new Error("Wisconsin publication left invalid public constraints");
  }
  const revisionRows = await nv.unsafe([
    "select revision::int revision,reason from public_data_revisions",
    "where scope='public'",
  ].join("\n"));
  const expectedReason = "Wisconsin local reporting unit geometry " + mode + " "
    + (publicAuthorized
  ...
  if (
    ...
    throw new Error("Wisconsin publication revision postcondition failed");
  }
  return {
    mode,
    status: wantedStatus,
...
export async function applyWisconsinGeographyPublicationTransaction(nv, context) {
  const mode = context.mode ?? "publish";
  if (!new Set(["publish", "rollback"]).has(mode)) {
    throw new Error("Wisconsin publication transaction mode is invalid");
  }
  const publicAuthorized = mode === "publish";
  if (mode === "rollback" && !context.publicationReceipt) {
    throw new Error("Wisconsin rollback requires the exact publication receipt");
  }
  if (mode === "rollback" && (
    ...
    throw new Error("Wisconsin rollback authorization drifted from the publication receipt");
  }
  ...
  if (
    ...
    throw new Error("Wisconsin publication transaction database identity drifted");
  }
  await nv.unsafe("select set_config('lock_timeout','30s',true)");
  await nv.unsafe(
    "select pg_advisory_xact_lock(hashtextextended('crm-wi-local-gis-release-v1',0))",
  );
  ...
  if (
    ...
    throw new Error("Wisconsin public revision is missing or invalid");
  }
  const revision = currentRevision + 1;
  const gisPlan = context.gisPlan;
  if (mode === "publish") {
    await (context.validateCurrentDatabase ?? validateWisconsinLocalGisClient)(nv, gisPlan, {
  ...
  if (versions.length !== 3) {
    throw new Error("Wisconsin publication requires three exact geography versions");
  }
  for (const [index, version] of versions.entries()) {
    ...
    if (mode === "publish" && (
      ...
      throw new Error("Wisconsin " + manifest.year + " publication precondition drifted");
    }
    if (mode === "rollback" && (
      commonDrift
    ...
    if (updated.length !== 1) {
      throw new Error("Wisconsin " + manifest.year + " publication update lost its lock");
    }
  }
  const crosswalks = await nv.unsafe([
    ...
    " and gv.state_code='WI' and gv.metadata->'releaseCandidate'->>'sha256'=$1",
    "returning x.id",
  ].join("\n"), [context.plan.releaseCandidate.sha256, JSON.stringify(publicAuthorized)]);
  if (crosswalks.length !== 10_937) {
    throw new Error("Wisconsin publication crosswalk update count drifted");
  }
  const units = await nv.unsafe([
  ...
  if (units.length !== 10_937) {
    throw new Error("Wisconsin publication reporting-unit update count drifted");
  }
  const sources = await nv.unsafe([
    "update source_documents set metadata=",
    " jsonb_set(jsonb_set(metadata,'{publicDeliveryAuthorized}',$2::text::jsonb,true),",
    " '{releaseCandidate,publicDeliveryAuthorized}',$2::text::jsonb,true)",
    "where state_code='WI' and metadata->'releaseCandidate'->>'sha256'=$1 returning id",
  ].join("\n"), [context.plan.releaseCandidate.sha256, JSON.stringify(publicAuthorized)]);
  if (sources.length !== 6) {
    throw new Error("Wisconsin publication source-document update count drifted");
  }
  const runs = await nv.unsafe([
    "update import_runs set summary=",
    " jsonb_set(jsonb_set(summary,'{publicDeliveryAuthorized}',$2::text::jsonb,true),",
    " '{releaseCandidate,publicDeliveryAuthorized}',$2::text::jsonb,true)",
    "where state_code='WI' and summary->'releaseCandidate'->>'sha256'=$1 returning id",
  ].join("\n"), [context.plan.releaseCandidate.sha256, JSON.stringify(publicAuthorized)]);
  if (runs.length !== 3) {
    throw new Error("Wisconsin publication import-run update count drifted");
  }
  const revisionRows = await nv.unsafe([
    ...
    "Wisconsin local reporting unit geometry " + mode + " "
      + (publicAuthorized
        ? context.authorization.activationId
        : context.authorization.rollbackId),
  ]);
  if (Number(revisionRows[0]?.revision) !== revision) {
    throw new Error("Wisconsin publication revision increment drifted");
  }
...
function defaultPlanPath(planSha256) {
  return ".etl/precinct-publication-plans/WI/wi-local-publication-plan-"
    + planSha256.slice(0, 12) + ".json";
}

function defaultAuthorizationPath(planSha256, mode = "publish") {
  return ".etl/production-authorizations/WI/wi-local-" + mode + "-authorization-template-"
    + planSha256.slice(0, 12) + ".json";
}

export function inspectWisconsinPublicationReceipt(root, options, built) {
  const artifact = safeJson(
    root,
    options.publicationReceiptPath,
    options.publicationReceiptSha256,
    ".etl/production-publication-receipts/WI",
  );
  ...
  const publicAuthorizationArtifact = safeJson(
    ...
    ".etl/production-authorizations/WI",
  );
  const publicAuthorization = validateWisconsinPublicationAuthorization(
  ...
  if (
    ...
    || value?.postconditions?.crosswalks !== 10_937
    || value?.postconditions?.reportingUnits !== 10_937
    || value?.postconditions?.sourceDocuments !== 6
    || value?.postconditions?.importRuns !== 3
    || value?.postconditions?.resultRows !== 32_475
    || value?.postconditions?.invalidConstraints !== 0
    || value?.productionMutationPerformed !== true
    || value?.publicDeliveryAuthorized !== true
  ) {
    throw new Error("Wisconsin publication receipt is incomplete or incompatible");
  }
  return {
    artifact,
    publicAuthorization,
...
function defaultReceiptPath(planSha256, activationId, mode = "publish") {
  ...
  return ".etl/production-publication-receipts/WI/wi-local-" + mode + "-"
...
function reserveReceipt(
  ...
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ".etl", "production-publication-receipts", "WI");
  if (!absolute.startsWith(allowed + path.sep) || !relativePath.endsWith(".json")) {
    throw new Error("Wisconsin publication receipt path is unsafe");
  }
  if (existsSync(absolute)) throw new Error("Wisconsin publication receipt already exists");
  mkdirSync(path.dirname(absolute), { recursive: true });
  ...
  const pendingBytes = serializeWisconsinPublicationDocument(pendingDocument);
  if (existsSync(pending)) {
    ...
    throw new Error("A Wisconsin publication recovery marker already exists; reconcile it before retrying");
  }
...
function finishPublicationReceipt(reservation, receipt) {
  const receiptBytes = serializeWisconsinPublicationDocument(receipt);
  ...
  if (existsSync(temporary)) {
    if (!readFileSync(temporary).equals(receiptBytes)) {
      throw new Error("Wisconsin publication receipt temporary file drifted");
    }
  } else {
...
export async function runWisconsinGeographyPublication(options = {}) {
  ...
  const currentTime = () => {
    ...
    if (Number.isNaN(result.getTime())) {
      throw new Error("Wisconsin publication clock returned an invalid time");
    }
    return result;
  };
  const built = inspectWisconsinPublicationPlan({ ...parsed, root });
  if (parsed.planSha256 && parsed.planSha256 !== built.sha256) {
    throw new Error("Wisconsin publication plan SHA-256 drifted");
  }
  const publicationReceipt = mode === "rollback"
    ? inspectWisconsinPublicationReceipt(root, {
      ...parsed,
  ...
  if (parsed.writePlan) {
    if (mode === "rollback") {
      throw new Error("Wisconsin rollback reuses the exact hash-pinned publication plan");
    }
    const artifact = immutableJson(
      root,
      parsed.outputPath ?? defaultPlanPath(built.sha256),
      built.plan,
      ".etl/precinct-publication-plans/WI",
    );
    return {
      mode: "write_plan",
  ...
  if (parsed.writeAuthorizationTemplate) {
    const template = mode === "rollback"
      ...
      : buildWisconsinPublicationAuthorizationTemplate(built.plan, built.sha256);
    const artifact = immutableJson(
      root,
  ...
  const validateAuthorizationAt = (now, recovery = false) => mode === "rollback"
    ? validateWisconsinPublicRollbackAuthorization(authorizationArtifact.value, {
      plan: built.plan,
      planSha256: built.sha256,
    ...
    : validateWisconsinPublicationAuthorization(authorizationArtifact.value, {
      plan: built.plan,
      planSha256: built.sha256,
      now: now.getTime(),
      recovery,
  ...
  if (endpointFingerprint !== built.plan.hiddenLoad.endpointFingerprint) {
    throw new Error("Wisconsin publication database endpoint drifted from the hidden load");
  }
  const gisPlan = await buildWisconsinLocalGisPlan({
    root,
  ...
  buildWisconsinLocalExecutionContext(executionContext);
  ...
  if (parsed.recoverReceipt) {
    if (
      ...
      || environment.CRM_WI_LOCAL_GEOGRAPHY_PUBLICATION_RECEIPT_RECOVERY !== built.sha256
      || environment.CRM_WI_LOCAL_GEOGRAPHY_PUBLICATION_AUTHORIZATION_SHA256
        !== authorizationArtifact.sha256
      || (mode === "rollback"
        && environment.CRM_WI_LOCAL_GEOGRAPHY_PUBLICATION_RECEIPT_SHA256
          !== publicationReceipt.summary.sha256)
    ) {
      throw new Error(
        "Wisconsin publication receipt recovery is not explicitly read-only and hash-authorized",
      );
    }
    ...
    try {
      sql = (options.postgresFactory ?? postgres)(databaseUrl, {
        ...
        idle_timeout: 20,
        connection: {
          application_name: "civicresultmaps-wi-local-publication-receipt-recovery",
    ...
    try {
      recovered = await sql.begin("read only", async (nv) => {
        ...
        if (
          ...
          throw new Error("Wisconsin publication receipt recovery database identity drifted");
        }
        const versions = await nv.unsafe([
        ...
        if (versions.length !== 3) {
          throw new Error("Wisconsin publication receipt recovery found an incomplete version set");
        }
        ...
        if (
          ...
          throw new Error("Wisconsin publication receipt recovery audit is incomplete");
        }
  ...
  const writeAuthorized = mode === "publish"
    ? environment.CRM_WI_LOCAL_GEOGRAPHY_PUBLICATION_WRITES
        === "I_ACKNOWLEDGE_ATOMIC_WISCONSIN_LOCAL_PUBLIC_CUTOVER"
      && environment.CRM_WI_LOCAL_GEOGRAPHY_PUBLICATION_ACTIVATION_ID
        === authorization.activationId
    : environment.CRM_WI_LOCAL_GEOGRAPHY_ROLLBACK_WRITES
        === "I_ACKNOWLEDGE_DATABASE_FIRST_WISCONSIN_LOCAL_PUBLIC_ROLLBACK"
      && environment.CRM_WI_LOCAL_GEOGRAPHY_ROLLBACK_ID === authorization.rollbackId
      && environment.CRM_WI_LOCAL_GEOGRAPHY_PUBLICATION_RECEIPT_SHA256
        === publicationReceipt.summary.sha256;
  if (
    environment.CRM_DATABASE_ENVIRONMENT !== "production"
    || !writeAuthorized
    || environment.CRM_WI_LOCAL_GEOGRAPHY_PUBLICATION_PACKAGE_SHA256
      !== built.plan.releaseCandidate.sha256
    || environment.CRM_WI_LOCAL_GEOGRAPHY_PUBLICATION_PLAN_SHA256 !== built.sha256
    || environment.CRM_WI_LOCAL_GEOGRAPHY_PUBLICATION_AUTHORIZATION_SHA256
      !== authorizationArtifact.sha256
  ) {
    throw new Error("Wisconsin public " + mode + " is not explicitly hash-authorized");
  }
  const activePublicAuthorization = mode === "publish"
    ? authorization
    : publicAuthorization;
  verifyWisconsinPublicationGitCandidate(root, activePublicAuthorization, options.gitRunner);
  const reservation = reserveReceipt(
    root,
    receiptPath,
    built.sha256,
  ...
  try {
    sql = (options.postgresFactory ?? postgres)(databaseUrl, {
      ...
      connect_timeout: 10,
      idle_timeout: 20,
      connection: { application_name: "civicresultmaps-wi-local-publication-status" },
    });
  } catch (error) {
  ...
  try {
    committed = await sql.begin(async (nv) => {
      ...
      if (finalAuthorization.activationId !== authorization.activationId) {
        throw new Error("Wisconsin publication authorization drifted before the transaction");
      }
      verifyWisconsinPublicationGitCandidate(
        root,
        activePublicAuthorization,
        options.gitRunner,
      );
      const result = await applyWisconsinGeographyPublicationTransaction(nv, {
        mode,
        plan: built.plan,
        planSha256: built.sha256,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  runWisconsinGeographyPublication().then(
