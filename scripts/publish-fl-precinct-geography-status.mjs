...
import {
  buildFloridaPrecinctExecutionContext,
  validateFloridaPrecinctGisClient,
} from "./lib/fl-precinct-gis-db.mjs";
import { buildFloridaPrecinctGisPlan } from "./lib/fl-precinct-gis-plan.mjs";
import { productionEndpointFingerprint } from "./lib/fl-precinct-production-preflight.mjs";
import {
...
function parseArguments(args) {
  ...
  for (const arg of args) {
    if (
      ...
      throw new Error("Unknown Florida publication-status option: " + arg);
    }
  }
  const modes = [
    parsed.writePlan,
  ...
  if (modes > 1) throw new Error("Florida publication-status modes are mutually exclusive");
  for (const field of [
    "packagePath",
    "packageSha256",
    "hiddenReceiptPath",
    ...
    if (!parsed[field]) throw new Error("Florida publication-status requires " + field);
  }
  if (
    parsed.rollback
    && (!parsed.publicationReceiptPath || !parsed.publicationReceiptSha256)
  ) {
    throw new Error("Florida rollback requires the exact publication receipt and SHA-256");
  }
  return parsed;
}
...
function safeJson(root, relativePath, expectedSha256, allowedRoot) {
  if (
    ...
    throw new Error("Florida publication-status JSON path is unsafe");
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ...allowedRoot.split("/"));
  if (!absolute.startsWith(allowed + path.sep) || !existsSync(absolute)) {
    throw new Error("Florida publication-status JSON is missing");
  }
  const bytes = readFileSync(absolute);
  if (sha256(bytes) !== expectedSha256) {
    throw new Error("Florida publication-status JSON SHA-256 drifted");
  }
  return {
    path: relativePath,
    absolute,
...
function immutableJson(root, relativePath, value, allowedRoot) {
  if (
    ...
    throw new Error("Florida publication-status output path is unsafe");
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ...allowedRoot.split("/"));
  if (!absolute.startsWith(allowed + path.sep)) {
    throw new Error("Florida publication-status output escapes its fixed root");
  }
  const bytes = serializeFloridaPublicationDocument(value);
  if (existsSync(absolute)) {
    if (!readFileSync(absolute).equals(bytes)) {
      throw new Error("Refusing to overwrite different Florida publication evidence");
    }
...
export function verifyFloridaPublicationGitCandidate(
  root,
  publicAuthorization,
  runner = execFileSync,
) {
  ...
  try {
    ...
    throw new Error("Florida publication Git evidence could not be resolved locally");
  }
  if (
    ...
    throw new Error("Florida publication checkout or deployment Git evidence drifted");
  }
  return {
    gitSha: head,
    gitTreeSha: headTree,
...
export function assertFloridaPublicationRecoveryVersionSet(versions) {
  if (!Array.isArray(versions) || versions.length !== 3) {
    throw new Error("Florida publication receipt recovery found an incomplete version set");
  }
  return versions;
}
...
async function verifyPostconditions(
  ...
  const versions = await nv.unsafe([
    ...
    "where gv.state_code='FL' and gv.geography_type='precinct'",
    " and gv.metadata->'releaseCandidate'->>'sha256'=$1",
    "order by e.year",
  ].join("\n"), [context.plan.releaseCandidate.sha256]);
  if (versions.length !== 3) {
    throw new Error("Florida publication postcondition has an incomplete version set");
  }
  for (const [index, row] of versions.entries()) {
    ...
    const expectedCaveat = publicAuthorized
      ? "Reviewed Florida election-specific precinct geometry is publicly authorized under activation "
    ...
    if (
      ...
      throw new Error("Florida " + manifest.year + " geography publication drifted");
    }
  }
  const linked = await nv.unsafe([
    ...
    "where gv.state_code='FL' and gv.metadata->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256, String(publicAuthorized)]);
  if (Number(linked[0]?.total) !== 17_424 || Number(linked[0]?.exact) !== 17_424) {
    throw new Error("Florida publication crosswalk postcondition failed");
  }
  const units = await nv.unsafe([
    ...
    "from reporting_units where state_code='FL'",
    " and reporting_grain='precinct'",
    " and metadata->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256, String(publicAuthorized)]);
  if (Number(units[0]?.total) !== 17_424 || Number(units[0]?.exact) !== 17_424) {
    throw new Error("Florida publication reporting-unit postcondition failed");
  }
  const sources = await nv.unsafe([
    "select count(*)::int total, count(*) filter (where",
    " metadata->>'publicDeliveryAuthorized'=$2 and",
    " metadata->'releaseCandidate'->>'publicDeliveryAuthorized'=$2)::int exact",
    "from source_documents where state_code='FL'",
    " and metadata->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256, String(publicAuthorized)]);
  if (Number(sources[0]?.total) !== 6 || Number(sources[0]?.exact) !== 6) {
    throw new Error("Florida publication source-document postcondition failed");
  }
  const runs = await nv.unsafe([
    ...
    "from import_runs where state_code='FL'",
    " and summary->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256, String(publicAuthorized)]);
  if (Number(runs[0]?.total) !== 3 || Number(runs[0]?.exact) !== 3) {
    throw new Error("Florida publication import-run postcondition failed");
  }
  const results = await nv.unsafe([
    ...
    "where rr.state_code='FL' and rr.level='precinct'",
    " and ru.metadata->'releaseCandidate'->>'sha256'=$1",
  ].join("\n"), [context.plan.releaseCandidate.sha256]);
  if (Number(results[0]?.total) !== 52_272) {
    throw new Error("Florida publication result-row postcondition failed");
  }
  ...
  if (Number(invalid[0]?.count) !== 0) {
    throw new Error("Florida publication left invalid public constraints");
  }
  const revisionRows = await nv.unsafe([
    "select revision::int revision,reason from public_data_revisions",
    "where scope='public'",
  ].join("\n"));
  const expectedReason = "Florida precinct unit geometry " + mode + " "
    + (publicAuthorized
  ...
  if (
    ...
    throw new Error("Florida publication revision postcondition failed");
  }
  return {
    mode,
    status: wantedStatus,
...
export async function applyFloridaGeographyPublicationTransaction(nv, context) {
  const mode = context.mode ?? "publish";
  if (!new Set(["publish", "rollback"]).has(mode)) {
    throw new Error("Florida publication transaction mode is invalid");
  }
  const publicAuthorized = mode === "publish";
  if (mode === "rollback" && !context.publicationReceipt) {
    throw new Error("Florida rollback requires the exact publication receipt");
  }
  if (mode === "rollback" && (
    ...
    throw new Error("Florida rollback authorization drifted from the publication receipt");
  }
  ...
  if (
    ...
    throw new Error("Florida publication transaction database identity drifted");
  }
  await nv.unsafe("select set_config('lock_timeout','30s',true)");
  await nv.unsafe(
    "select pg_advisory_xact_lock(hashtextextended('crm-fl-precinct-gis-release-v1',0))",
  );
  ...
  if (
    ...
    throw new Error("Florida public revision is missing or invalid");
  }
  const revision = currentRevision + 1;
  const gisPlan = context.gisPlan;
  if (mode === "publish") {
    await (context.validateCurrentDatabase ?? validateFloridaPrecinctGisClient)(nv, gisPlan, {
  ...
  if (versions.length !== 3) {
    throw new Error("Florida publication requires three exact geography versions");
  }
  for (const [index, version] of versions.entries()) {
    ...
    if (mode === "publish" && (
      ...
      throw new Error("Florida " + manifest.year + " publication precondition drifted");
    }
    if (mode === "rollback" && (
      commonDrift
    ...
    if (updated.length !== 1) {
      throw new Error("Florida " + manifest.year + " publication update lost its lock");
    }
  }
  const crosswalks = await nv.unsafe([
    ...
    " and gv.state_code='FL' and gv.metadata->'releaseCandidate'->>'sha256'=$1",
    "returning x.id",
  ].join("\n"), [context.plan.releaseCandidate.sha256, JSON.stringify(publicAuthorized)]);
  if (crosswalks.length !== 17_424) {
    throw new Error("Florida publication crosswalk update count drifted");
  }
  const units = await nv.unsafe([
  ...
  if (units.length !== 17_424) {
    throw new Error("Florida publication reporting-unit update count drifted");
  }
  const sources = await nv.unsafe([
    "update source_documents set metadata=",
    " jsonb_set(jsonb_set(metadata,'{publicDeliveryAuthorized}',$2::text::jsonb,true),",
    " '{releaseCandidate,publicDeliveryAuthorized}',$2::text::jsonb,true)",
    "where state_code='FL' and metadata->'releaseCandidate'->>'sha256'=$1 returning id",
  ].join("\n"), [context.plan.releaseCandidate.sha256, JSON.stringify(publicAuthorized)]);
  if (sources.length !== 6) {
    throw new Error("Florida publication source-document update count drifted");
  }
  const runs = await nv.unsafe([
    "update import_runs set summary=",
    " jsonb_set(jsonb_set(summary,'{publicDeliveryAuthorized}',$2::text::jsonb,true),",
    " '{releaseCandidate,publicDeliveryAuthorized}',$2::text::jsonb,true)",
    "where state_code='FL' and summary->'releaseCandidate'->>'sha256'=$1 returning id",
  ].join("\n"), [context.plan.releaseCandidate.sha256, JSON.stringify(publicAuthorized)]);
  if (runs.length !== 3) {
    throw new Error("Florida publication import-run update count drifted");
  }
  const revisionRows = await nv.unsafe([
    ...
    "Florida precinct unit geometry " + mode + " "
      + (publicAuthorized
        ? context.authorization.activationId
        : context.authorization.rollbackId),
  ]);
  if (Number(revisionRows[0]?.revision) !== revision) {
    throw new Error("Florida publication revision increment drifted");
  }
...
function defaultPlanPath(planSha256) {
  return ".etl/precinct-publication-plans/FL/fl-precinct-publication-plan-"
    + planSha256.slice(0, 12) + ".json";
}

function defaultAuthorizationPath(planSha256, mode = "publish") {
  return ".etl/production-authorizations/FL/fl-precinct-" + mode + "-authorization-template-"
    + planSha256.slice(0, 12) + ".json";
}

export function inspectFloridaPublicationReceipt(root, options, built) {
  const artifact = safeJson(
    root,
    options.publicationReceiptPath,
    options.publicationReceiptSha256,
    ".etl/production-publication-receipts/FL",
  );
  ...
  const publicAuthorizationArtifact = safeJson(
    ...
    ".etl/production-authorizations/FL",
  );
  const publicAuthorization = validateFloridaPublicationAuthorization(
  ...
  if (
    ...
    || value?.postconditions?.crosswalks !== 17_424
    || value?.postconditions?.reportingUnits !== 17_424
    || value?.postconditions?.sourceDocuments !== 6
    || value?.postconditions?.importRuns !== 3
    || value?.postconditions?.resultRows !== 52_272
    || value?.postconditions?.invalidConstraints !== 0
    || value?.productionMutationPerformed !== true
    || value?.publicDeliveryAuthorized !== true
  ) {
    throw new Error("Florida publication receipt is incomplete or incompatible");
  }
  return {
    artifact,
    publicAuthorization,
...
function defaultReceiptPath(planSha256, activationId, mode = "publish") {
  ...
  return ".etl/production-publication-receipts/FL/fl-precinct-" + mode + "-"
...
function reserveReceipt(
  ...
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ".etl", "production-publication-receipts", "FL");
  if (!absolute.startsWith(allowed + path.sep) || !relativePath.endsWith(".json")) {
    throw new Error("Florida publication receipt path is unsafe");
  }
  if (existsSync(absolute)) throw new Error("Florida publication receipt already exists");
  mkdirSync(path.dirname(absolute), { recursive: true });
  ...
  const pendingBytes = serializeFloridaPublicationDocument(pendingDocument);
  if (existsSync(pending)) {
    ...
    throw new Error("A Florida publication recovery marker already exists; reconcile it before retrying");
  }
...
function finishPublicationReceipt(reservation, receipt) {
  const receiptBytes = serializeFloridaPublicationDocument(receipt);
  ...
  if (existsSync(temporary)) {
    if (!readFileSync(temporary).equals(receiptBytes)) {
      throw new Error("Florida publication receipt temporary file drifted");
    }
  } else {
...
export async function runFloridaGeographyPublication(options = {}) {
  ...
  const currentTime = () => {
    ...
    if (Number.isNaN(result.getTime())) {
      throw new Error("Florida publication clock returned an invalid time");
    }
    return result;
  };
  const built = inspectFloridaPublicationPlan({ ...parsed, root });
  if (parsed.planSha256 && parsed.planSha256 !== built.sha256) {
    throw new Error("Florida publication plan SHA-256 drifted");
  }
  ...
  if (parsed.writePlan) {
    if (mode === "rollback") {
      throw new Error("Florida rollback reuses the exact hash-pinned publication plan");
    }
    const artifact = immutableJson(
      root,
      parsed.outputPath ?? defaultPlanPath(built.sha256),
      built.plan,
      ".etl/precinct-publication-plans/FL",
    );
    return {
      mode: "write_plan",
  ...
  if (parsed.writeAuthorizationTemplate) {
    const template = mode === "rollback"
      ...
      : buildFloridaPublicationAuthorizationTemplate(built.plan, built.sha256);
    const artifact = immutableJson(
      root,
  ...
  const validateAuthorizationAt = (now, recovery = false) => mode === "rollback"
    ? validateFloridaPublicRollbackAuthorization(authorizationArtifact.value, {
      plan: built.plan,
      planSha256: built.sha256,
    ...
    : validateFloridaPublicationAuthorization(authorizationArtifact.value, {
      plan: built.plan,
      planSha256: built.sha256,
      now: now.getTime(),
      recovery,
  ...
  if (endpointFingerprint !== built.plan.hiddenLoad.endpointFingerprint) {
    throw new Error("Florida publication database endpoint drifted from the hidden load");
  }
  const gisPlan = await buildFloridaPrecinctGisPlan({
    root,
  ...
  buildFloridaPrecinctExecutionContext(executionContext);
  ...
  if (parsed.recoverReceipt) {
    if (
      ...
      || environment.CRM_FL_PRECINCT_GEOGRAPHY_PUBLICATION_RECEIPT_RECOVERY !== built.sha256
      || environment.CRM_FL_PRECINCT_GEOGRAPHY_PUBLICATION_AUTHORIZATION_SHA256
        !== authorizationArtifact.sha256
      || (mode === "rollback"
        && environment.CRM_FL_PRECINCT_GEOGRAPHY_PUBLICATION_RECEIPT_SHA256
          !== publicationReceipt.summary.sha256)
    ) {
      throw new Error(
        "Florida publication receipt recovery is not explicitly read-only and hash-authorized",
      );
    }
    ...
    try {
      sql = (options.postgresFactory ?? postgres)(databaseUrl, {
        ...
        idle_timeout: 20,
        connection: {
          application_name: "civicresultmaps-fl-precinct-publication-receipt-recovery",
    ...
    try {
      recovered = await sql.begin("read only", async (nv) => {
        ...
        if (
          ...
          throw new Error("Florida publication receipt recovery database identity drifted");
        }
        const versions = await nv.unsafe([
        ...
        assertFloridaPublicationRecoveryVersionSet(versions);
        ...
        if (
          ...
          throw new Error("Florida publication receipt recovery audit is incomplete");
        }
  ...
  const writeAuthorized = mode === "publish"
    ? environment.CRM_FL_PRECINCT_GEOGRAPHY_PUBLICATION_WRITES
        === "I_ACKNOWLEDGE_ATOMIC_FLORIDA_PRECINCT_PUBLIC_CUTOVER"
      && environment.CRM_FL_PRECINCT_GEOGRAPHY_PUBLICATION_ACTIVATION_ID
        === authorization.activationId
    : environment.CRM_FL_PRECINCT_GEOGRAPHY_ROLLBACK_WRITES
        === "I_ACKNOWLEDGE_DATABASE_FIRST_FLORIDA_PRECINCT_PUBLIC_ROLLBACK"
      && environment.CRM_FL_PRECINCT_GEOGRAPHY_ROLLBACK_ID === authorization.rollbackId
      && environment.CRM_FL_PRECINCT_GEOGRAPHY_PUBLICATION_RECEIPT_SHA256
        === publicationReceipt.summary.sha256;
  if (
    environment.CRM_DATABASE_ENVIRONMENT !== "production"
    || !writeAuthorized
    || environment.CRM_FL_PRECINCT_GEOGRAPHY_PUBLICATION_PACKAGE_SHA256
      !== built.plan.releaseCandidate.sha256
    || environment.CRM_FL_PRECINCT_GEOGRAPHY_PUBLICATION_PLAN_SHA256 !== built.sha256
    || environment.CRM_FL_PRECINCT_GEOGRAPHY_PUBLICATION_AUTHORIZATION_SHA256
      !== authorizationArtifact.sha256
  ) {
    throw new Error("Florida public " + mode + " is not explicitly hash-authorized");
  }
  const activePublicAuthorization = mode === "publish"
    ? authorization
    : publicAuthorization;
  verifyFloridaPublicationGitCandidate(root, activePublicAuthorization, options.gitRunner);
  const reservation = reserveReceipt(
    root,
    receiptPath,
    built.sha256,
  ...
  try {
    sql = (options.postgresFactory ?? postgres)(databaseUrl, {
      ...
      connect_timeout: 10,
      idle_timeout: 20,
      connection: { application_name: "civicresultmaps-fl-precinct-publication-status" },
    });
  } catch (error) {
  ...
  try {
    committed = await sql.begin(async (nv) => {
      ...
      if (finalAuthorization.activationId !== authorization.activationId) {
        throw new Error("Florida publication authorization drifted before the transaction");
      }
      ...
      const result = await applyFloridaGeographyPublicationTransaction(nv, {
        mode,
        plan: built.plan,
        planSha256: built.sha256,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  runFloridaGeographyPublication().then(
