...
import {
  inspectAlaskaPublicActivationPlan,
} from "./lib/ak-precinct-public-activation.mjs";
...
function parseArguments(args) {
  ...
  for (const arg of args) {
    if (
      ...
      throw new Error("Unknown Alaska public-activation option: " + arg);
    }
  }
  if (!packagePath || !packageSha256) {
    throw new Error(
      "Alaska public activation requires --package and --package-sha256",
    );
  }
...
function immutableWrite(root, relativePath, bytes) {
  ...
  const allowed = path.resolve(root, ".etl", "precinct-public-activations", "AK");
  if (!absolutePath.startsWith(allowed + path.sep)) {
    throw new Error("Alaska activation evidence escapes its fixed .etl root");
  }
  if (existsSync(absolutePath)) {
    if (!readFileSync(absolutePath).equals(bytes)) {
      throw new Error("Refusing to overwrite different Alaska activation evidence");
    }
  ...
  try {
    for (const output of outputs) {
      ...
      if (before.length !== output.preimage.byteCount) {
        throw new Error("Alaska activation target changed after plan: " + output.path);
      }
      const currentHash = createHash("sha256").update(before).digest("hex");
      if (currentHash !== output.preimage.sha256) {
        throw new Error("Alaska activation target preimage drifted: " + output.path);
      }
      const temporaryPath = output.absolutePath
        + ".crm-ak-activation-" + randomUUID() + ".tmp";
    ...
    for (const item of committed.reverse()) {
      const rollbackPath = item.output.absolutePath
        + ".crm-ak-rollback-" + randomUUID() + ".tmp";
...
export async function prepareAlaskaPublicActivation(options = {}) {
  const root = path.resolve(options.root ?? process.cwd());
  const built = inspectAlaskaPublicActivationPlan({ ...options, root });
  if (!options.write) {
    return {
      mode: "plan",
  ...
  const trackedOutputs = writeAlaskaActivationTrackedOutputs(built.outputs);
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  prepareAlaskaPublicActivation(parseArguments(process.argv.slice(2))).then(
