...
$VerifyDatabase = 'crm_nv_precinct_restore_verify'
$BackupRoot = 'C:\tmp\crm-db-clone\nv-release-backups'
$ContainerBackupRoot = '/backups/nv-release-backups'
$ExpectedLegacyEndpointFingerprint = 'bf2bf2213814'

function Fail([string]$Message) {
  throw "Nevada production backup aborted: $Message"
}
...
function Assert-ReleasePackage {
  $candidateRoot = [IO.Path]::GetFullPath((Join-Path $RepoRoot '.etl\precinct-release-candidates\NV'))
  ...
  if (
    ...
    Fail 'The release package must be an existing release-candidate.json under .etl/precinct-release-candidates/NV.'
  }
...
if (-not $Execute) {
  [ordered]@{
    ...
    acknowledgementRequired = @(
      'CRM_NV_PRECINCT_BACKUP_ACK=CREATE_FULL_PUBLIC_SCHEMA_ROLLBACK_BACKUP',
      'CRM_NV_PRECINCT_BACKUP_ENDPOINT_FINGERPRINT=<fresh 64-hex preflight endpoint fingerprint>'
    )
...
if (
  [Environment]::GetEnvironmentVariable('CRM_NV_PRECINCT_BACKUP_ACK') -ne 'CREATE_FULL_PUBLIC_SCHEMA_ROLLBACK_BACKUP' -or
  [Environment]::GetEnvironmentVariable('CRM_NV_PRECINCT_BACKUP_PACKAGE_SHA256') -ne $ReleasePackageSha256
) {
...
if (
  ...
  [Environment]::GetEnvironmentVariable('CRM_NV_PRECINCT_BACKUP_ENDPOINT_FINGERPRINT') -ne $sourceEndpointFingerprint
) {
...
$dumpFile = "nv-precinct-full-public-$stamp.dump"
$dumpPath = Join-Path $BackupRoot $dumpFile
$containerDumpPath = "$ContainerBackupRoot/$dumpFile"
$manifestPath = Join-Path $BackupRoot "nv-precinct-full-public-$stamp.manifest.json"
