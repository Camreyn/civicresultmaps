...
import {
  buildAlaskaPrecinctReleaseCandidate,
  alaskaReleaseCandidateOutputRoot,
} from "./lib/ak-precinct-release-candidate.mjs";
...
function immutableWrite(root, relativePath, bytes) {
  ...
  const expectedRoot = path.resolve(root, ".etl", "precinct-release-candidates", "AK");
  if (!absolutePath.startsWith(expectedRoot + path.sep)) {
    throw new Error("Alaska release output escapes its content-addressed root");
  }
  if (existsSync(absolutePath)) {
    if (!readFileSync(absolutePath).equals(bytes)) {
      throw new Error("Refusing to overwrite different Alaska release bytes: " + relativePath);
    }
...
export async function prepareAlaskaPrecinctReleaseCandidate(options = {}) {
  ...
  const outputRoot = alaskaReleaseCandidateOutputRoot(packageSha256);
  const files = [
...
async function main() {
  ...
  if (unknown.length) throw new Error("Unknown Alaska release-candidate option: " + unknown[0]);
  const validationReportPath = args.find((arg) => arg.startsWith("--validation-report="))
    ?.slice("--validation-report=".length);
  console.log(JSON.stringify(await prepareAlaskaPrecinctReleaseCandidate({
