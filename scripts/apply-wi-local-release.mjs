...
import {
  applyWisconsinLocalGisTransaction,
  readWisconsinPersistedProductionReleaseAudit,
...
import { inspectReleaseArtifact } from "./lib/wi-local-release-candidate.mjs";
import {
  assertWisconsinReleaseCandidateDocument,
  productionEndpointFingerprint,
} from "./lib/wi-local-production-preflight.mjs";
import {
...
} from "./lib/wi-local-production-release.mjs";
...
function parseArguments(args) {
  ...
  for (const arg of args) {
    ...
    if (!known.has(key)) throw new Error("Unknown Wisconsin production-release option: " + arg);
  }
  if (!parsed.packagePath || !parsed.packageSha256) {
    throw new Error("Wisconsin production release requires --package and --package-sha256");
  }
  if ([parsed.apply, parsed.recoverReceipt, parsed.writeAuthorizationTemplate]
    .filter(Boolean).length > 1) {
    throw new Error("Wisconsin production release modes are mutually exclusive");
  }
  return parsed;
}
...
function safeEtlPath(root, relativePath, requiredPrefix) {
  if (
    ...
    throw new Error("Unsafe Wisconsin release evidence path: " + relativePath);
  }
  const absolute = path.resolve(root, ...relativePath.split("/"));
  const allowed = path.resolve(root, ...requiredPrefix.replace(/\/$/, "").split("/"));
  if (!absolute.startsWith(allowed + path.sep)) {
    throw new Error("Wisconsin release evidence escapes its .etl root");
  }
  return absolute;
}
...
function readEtlJson(root, relativePath, expectedSha256, prefix) {
  if (!/^[a-f0-9]{64}$/.test(expectedSha256 ?? "")) {
    throw new Error("Wisconsin release evidence requires an exact SHA-256");
  }
  const absolute = safeEtlPath(root, relativePath, prefix);
  const bytes = readFileSync(absolute);
  if (sha256(bytes) !== expectedSha256) {
    throw new Error("Wisconsin release evidence SHA-256 drifted: " + relativePath);
  }
...
function readBackupManifest(relativeOrAbsolutePath, expectedSha256, root) {
  if (!/^[a-f0-9]{64}$/.test(expectedSha256 ?? "")) {
    throw new Error("Wisconsin backup manifest requires an exact SHA-256");
  }
  ...
  const bytes = readFileSync(absolute);
  if (sha256(bytes) !== expectedSha256) {
    throw new Error("Wisconsin backup manifest SHA-256 drifted");
  }
...
function productionUrl(environment = process.env) {
  ...
  if (!first && !second) {
    throw new Error("Wisconsin production release requires an explicit unpooled PostgreSQL URL");
  }
  if (first && second && first !== second) {
    throw new Error("Wisconsin production release found conflicting unpooled PostgreSQL URLs");
  }
  return first || second;
}
...
function immutableJson(root, relativePath, value) {
  ...
  if (existsSync(absolute)) {
    if (!readFileSync(absolute).equals(bytes)) {
      throw new Error("Refusing to overwrite different Wisconsin release evidence");
    }
...
function reserveReceipt(
  ...
  const absolute = safeEtlPath(
    root,
    relativePath,
    ".etl/production-release-receipts/WI/",
  );
  if (existsSync(absolute)) throw new Error("Wisconsin production receipt already exists");
  ...
  if (existsSync(pending)) {
    ...
    throw new Error("A Wisconsin release recovery marker already exists; reconcile it before retrying");
  }
...
function finishReceipt(reservation, value) {
  ...
  if (existsSync(temporary)) {
    if (!readFileSync(temporary).equals(bytes)) {
      throw new Error("Wisconsin hidden receipt temporary file drifted");
    }
  } else {
...
function defaultTemplatePath(packageSha256) {
  return ".etl/production-authorizations/WI/wi-local-authorization-template-"
...
function defaultReceiptPath(packageSha256, authorizationId) {
  ...
  return ".etl/production-release-receipts/WI/wi-local-hidden-load-"
    + packageSha256.slice(0, 12)
    + "-"
    + safeId
    + ".json";
...
async function ensureWisconsinDerivationConstraint(tx, migrationArtifact) {
  ...
  if (beforeRows.length !== 1) {
    throw new Error("Wisconsin derivation-method constraint is missing or ambiguous");
  }
  ...
  if (
    ...
    throw new Error("Wisconsin derivation-method constraint preimage drifted");
  }
  ...
  if (statements.length !== 2) {
    throw new Error("Wisconsin migration 0009 statement set drifted");
  }
  ...
  if (
    ...
    throw new Error("Wisconsin migration 0009 did not establish the required constraint");
  }
  return { before: "absent", after: "complete", statementsApplied: 2 };
}

export async function runWisconsinProductionRelease(options = {}) {
  ...
  const packageArtifact = inspectReleaseArtifact(root, parsed.packagePath, {
    allowedRoots: [".etl/precinct-release-candidates/WI/"],
    sha256: parsed.packageSha256,
  });
  const packageDocument = JSON.parse(packageArtifact.bytes.toString("utf8"));
  const releaseCandidate = assertWisconsinReleaseCandidateDocument(
    packageDocument,
  ...
  const currentTime = () => {
    ...
    if (Number.isNaN(result.getTime())) {
      throw new Error("Wisconsin production release clock returned an invalid time");
    }
    return result;
  };
  if (parsed.writeAuthorizationTemplate) {
    ...
    const artifact = immutableJson(root, relativePath, buildWisconsinProductionAuthorizationTemplate({
      releaseCandidate,
  ...
  const preflight = readEtlJson(
    ...
    ".etl/production-preflight-candidates/WI/",
  );
  ...
  const validateEvidenceAt = (now) => {
    const preflightSummary = validateWisconsinProductionPreflightEvidence(
      preflight.value,
      { releaseCandidate, endpointFingerprint, now },
    );
    const backupSummary = validateWisconsinProductionBackupEvidence(backup.value, {
      releaseCandidate,
      endpointFingerprint,
      preflightCapturedAtUtc: preflightSummary.capturedAtUtc,
      now,
    });
    const authorizationSummary = validateWisconsinProductionAuthorization(
      authorization.value,
      {
        releaseCandidate,
  ...
  if (parsed.recoverReceipt) {
    if (
      ...
      || environment.CRM_WI_LOCAL_GEOGRAPHY_HIDDEN_RECEIPT_RECOVERY
        !== packageArtifact.sha256
      || environment.CRM_WI_LOCAL_GEOGRAPHY_PRODUCTION_AUTHORIZATION_SHA256
        !== authorization.sha256
    ) {
      throw new Error("Wisconsin hidden receipt recovery is not explicitly read-only and hash-authorized");
    }
    ...
    try {
      sql = (options.postgresFactory ?? postgres)(databaseUrl, {
        ...
        idle_timeout: 20,
        connection: {
          application_name: "civicresultmaps-wi-local-hidden-receipt-recovery",
    ...
    try {
      recovered = await sql.begin("read only", async (nv) => {
        const persistedAudit = await readWisconsinPersistedProductionReleaseAudit(
          nv,
        ...
        if (committedAt.getTime() > currentTime().getTime()) {
          throw new Error("Wisconsin hidden receipt recovery found a future commit timestamp");
        }
        ...
        if (JSON.stringify(persistedAudit) !== JSON.stringify(expectedAudit)) {
          throw new Error("Wisconsin hidden receipt recovery evidence does not match the durable database audit");
        }
        ...
        const validation = await validateWisconsinLocalGisClient(nv, plan, {
          executionContext,
          readOnlySession: true,
        });
        if (Number(validation.revision) !== persistedAudit.transaction.publicRevision) {
          throw new Error("Wisconsin hidden receipt recovery public revision drifted");
        }
  ...
  if (
    ...
    || environment.CRM_WI_LOCAL_GEOGRAPHY_PRODUCTION_WRITES !== packageArtifact.sha256
  ) {
    throw new Error("Wisconsin production writes are not explicitly package-authorized");
  }
  if (
    environment.CRM_WI_LOCAL_GEOGRAPHY_PRODUCTION_AUTHORIZATION_ID
      !== initialEvidence.authorizationSummary.authorizationId
    || environment.CRM_WI_LOCAL_GEOGRAPHY_PRODUCTION_AUTHORIZATION_SHA256
      !== authorization.sha256
  ) {
    throw new Error("Wisconsin production authorization environment acknowledgement drifted");
  }
  const reservation = reserveReceipt(
    root,
    receiptPath,
  ...
  try {
    sql = (options.postgresFactory ?? postgres)(databaseUrl, {
      ...
      connect_timeout: 10,
      idle_timeout: 20,
      connection: { application_name: "civicresultmaps-wi-local-production-release" },
    });
  } catch (error) {
  ...
  try {
    committed = await sql.begin(async (nv) => {
      ...
      const migration0009 = await ensureWisconsinDerivationConstraint(
        nv,
      ...
      if (revisions.length !== 1) throw new Error("Wisconsin production public revision is missing");
      ...
      const applied = await applyWisconsinLocalGisTransaction(nv, plan, {
        executionContext,
      });
      if (applied.revision !== expectedRevision) {
        throw new Error("Wisconsin production public revision increment drifted");
      }
      const validation = await validateWisconsinLocalGisClient(nv, plan, {
        executionContext,
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  runWisconsinProductionRelease().then(
