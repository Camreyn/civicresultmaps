...
Colorado currently uses official EAC 2024 V2 county/jurisdiction fallback turnout rows at `data/eac-2024-state-turnout/co-2024-eac-turnout.csv`, totaling 64 rows, 3,240,754 ballots cast, and 4,583,280 registered voters. Wave 14 confirmed an official Colorado Secretary of State Clarity 2024 General Election detail XML turnout lead at `https://results.enr.clarityelections.com/CO/122598/367167/reports/detailxml.zip`: the inspected `ElectionVoterTurnout` rows report 64 counties, 3,241,120 ballotsCast, and 4,058,938 totalVoters. Keep EAC fallback active until Colorado `totalVoters` denominator semantics and the 366-ballot Clarity-minus-EAC ballots-cast difference are reviewed. See `data/co-2024-data-coverage-inventory.json` and `data/co-2024-source-request-matrix.tsv` for the current source lead, caveats, and follow-up request fields.
...
## North Dakota Wave 15 Update

North Dakota currently uses official EAC 2024 V2 county/jurisdiction fallback turnout rows at `data/eac-2024-state-turnout/nd-2024-eac-turnout.csv`, totaling 53 rows, 371,974 ballots cast, and 0 registered voters. This zero denominator is expected source semantics because North Dakota does not require voter registration. Wave 15 confirmed a state-native eligible-voter turnout lead on the official North Dakota Secretary of State results dashboard and turnout page: the dashboard reports 371,975 voter turnout, 594,140 eligible voters, 62.61% statewide turnout, and 385 of 385 precincts fully reported. Keep EAC fallback active until an SOS turnout/export normalizer captures eligible-voter fields, reconciles the one-ballot SOS-versus-EAC turnout difference, and updates public denominator labels away from registered-voter assumptions. See `data/nd-2024-data-coverage-inventory.json` and `data/nd-2024-source-request-matrix.tsv` for source URLs, caveats, and parser fields.

## New Jersey Wave 15 Update

New Jersey currently uses official EAC 2024 V2 county/jurisdiction fallback rows at data/eac-2024-state-turnout/nj-2024-eac-turnout.csv, totaling 21 rows, 4,321,921 ballots cast, and 6,630,364 registered voters in the active NJ config. Wave 15 confirmed state-native New Jersey Department of State turnout leads: the official statewide 2024 General Election voter-turnout PDF reports 6,682,699 registered voters, 4,321,921 ballots cast, 26,600 rejected ballots, and 6,402 election districts, and the county registered-voters/ballots-cast PDFs expose municipal denominator rows. Keep EAC fallback active until a New Jersey DOE PDF parser and reconciliation report reviews the 52,335 registered-voter denominator difference, municipal joins, and denominator timing. See data/nj-2024-data-coverage-inventory.json and data/nj-2024-source-request-matrix.tsv for source URLs, caveats, and request fields.

## New Mexico Wave 15 Update
...
Rhode Island currently uses official EAC 2024 V2 city/town jurisdiction fallback turnout rows at `data/eac-2024-state-turnout/ri-2024-eac-turnout.csv`, totaling 39 rows, 522,164 ballots cast, and 792,075 registered voters. Wave 15 confirmed official Rhode Island Board of Elections result source leads on the RI.gov 2024 General Election data page: short-format ZIP, long-format ZIP, statewide JSON, and city/town JSON files. Those result artifacts are not a complete turnout replacement unless registered-voter denominator fields are confirmed in the data description or a separate official RI artifact. Keep EAC fallback active until denominator timing and ballots-cast semantics are reviewed. Follow-up fields are tracked in `data/ri-2024-data-coverage-inventory.json` and `data/ri-2024-source-request-matrix.tsv`.
