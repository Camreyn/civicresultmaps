...
Pennsylvania keeps loaded county turnout rows from the official DOS vote-history and voter-registration summary workbook at `data/pa-2024-voter-registration-vote-history-summary.xlsx`: 67 county rows, 7,074,630 vote-history ballots cast, and 9,175,132 registered voters. The native parser also processes `data/pa-2024-general-voter-registration-precinct.txt`: 9,186 non-placeholder rows across 67 counties and 9,175,133 registered voters, one voter higher than the county workbook total. All 9,186 rows join to the returns by county code plus precinct code. The raw municipality value differs for 7,651 rows because the returns redundantly append the two segmented registration breakdown fields; reconstructing that display value matches all rows. The 9,154 nonzero presidential review rows carry an exact reporting-unit identity and a warning-required presidential-votes/registration proxy; four zero-denominator rows are not drawable and 17 drawable ratios exceed 100 percent. These values remain review metrics, not `turnout_rows`, because no same-grain precinct vote-history or election-level ballots-cast source has been collected. See `data/pa-2024-data-coverage-inventory.json` for source URLs, caveats, and request fields.
## Mississippi Update
...
The official South Carolina Elections Database Voter Statistics toggle is also retained as a candidate source at `data/sc-2024-election-history/voter-statistics-7075.csv` (contest 7075, election 58, November 5, 2024 General). `scripts/normalize-sc-election-history-voter-statistics.mjs` hash-pins the 46 county and 2,446 precinct source rows, writes a 46-county candidate artifact, and records the exact request/export metadata plus a VREMS reconciliation. Election History reports 2,566,404 election-level ballots and 3,451,344 registered voters; active VREMS reports 2,553,185 participating voters and 3,851,930 printed-list registrants. The 13,219-ballot and 400,586-registration differences have no source-side timing/inactive reconciliation yet, so VREMS remains the active warning-required turnout package and the candidate is not loaded as native turnout.
...
## Maine Enrollment Denominator Normalization (2026-08-19)
...
- State-native leads: `scripts/normalize-me-2024-enrollment-leads.mjs` deterministically normalizes the Maine Secretary of State active and inactive registered/enrolled files for the November 5, 2024 General/Referendum Election into `data/me-2024-registered-enrolled-denominator-leads.csv` plus `data/me-2024-registered-enrolled-denominator-leads-summary.json`. It preserves the SOS county, municipality, W/P, CG, SS, SR, CC, and D/G/L/NL/R/U party fields without inferring undocumented source-label meanings.
- Counts and reconciliation: 741 active rows total 1,037,570; 741 inactive rows total 185,622; their 1,223,192 combined enrollment total is 276 below EAC's 1,223,468 registered voters. The reconciliation is statewide only; it does not force a municipality/ward-or-precinct join to EAC jurisdiction rows.
- Replacement condition: these registered/enrolled files have no ballots-cast, voter-history, or voter-participation field. Do not replace EAC turnout until official Maine ballots-cast or voter-participation rows at compatible grain are collected, parsed, and reconciled against certified totals.
