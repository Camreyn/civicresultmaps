...
- Local GIS validation: 7,439 reporting units, 22,317 three-bucket candidate rows, 7,907 polygon features, and 7,457 reviewed relationship records across all four elections, with four blocked geography versions and zero invalid database constraints

Expected native validation remains 17 county result rows, 17 county geometry features, 1,484,840 presidential votes, 1,057 local review rows from Clark/Washoe/Humboldt CVR precinct aggregates, 17 EAC fallback turnout rows, and 51 secondary historical baseline rows. The separate precinct-GIS plan retains 1,760 colorable units for 2012, 2,067 for 2016, 2,094 for 2020, and 1,518 for 2024. Public release remains blocked for every year: 2012 needs the election-date Washoe archive and an aggregate-rendering decision for five one-to-many units; 2016 needs official machine-readable row-level provenance or an explicit reviewed supplemental-source decision despite exact statewide reconciliation to the official LCB map; 2020 needs the exact Harvard Dataverse file-version-21 custom terms; and 2024 needs affirmative derivative-redistribution terms because the retained public-authoritative LCB ArcGIS metadata leaves `licenseInfo` and `copyrightText` empty. The delivery contract preserves all 208 reviewed 2024 no-data polygons without inventing result rows. Same-grain U.S. Senate advisory rows remain partial outside Clark, Washoe, and Humboldt; current advisory rows are public-interest screening inputs only, not findings.
