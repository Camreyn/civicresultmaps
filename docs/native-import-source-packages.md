...
Caveats: the 2024 President and U.S. House rows retain secondary CSV extraction lineage, but their 66 county values are exactly validated against the retained certified SOS certificate. Wave 24 retained official SD SOS archive ElectionID 684, map-data evidence, the official ResultsExport statewide XLSX workbook, official results FAQ text, and a source-request packet. Those official app/export artifacts agree at 428,728 President votes and 421,264 U.S. House votes, below the certificate-validated totals by 194 and 184 votes; the workbook labels itself 2024 Unofficial General Election Results. The former P0 certificate request is resolved; the request packet and audit-page context remain useful for the outstanding turnout, precinct/local, geometry, historical, and administration requests. Review rows are county-level President-versus-at-large-U.S.-House comparisons, not precinct scatter plots. EAC turnout remains active until state-native turnout/registration denominator rows are reconciled; the ElectionID 684 turnout archive is only a lead. No normalized audit, CVR availability, recount, incident, correction, or litigation rows are loaded.
