...
Expected validation: 67 certified county result rows, 67 county geometry features, 10,893,752 certified presidential candidate votes, 6,110,125 Trump votes, 4,683,038 Harris votes, 100,589 other votes, 5,618 precinct/location President-versus-U.S. Senate review rows, 67 EAC fallback turnout rows, and 201 official-source-derived historical county rows. The official 2024 precinct ZIP contains 67 county files, 537,897 source rows, 5,712 unique precinct/location rows, and 13,942,186 registered voters across those rows. `scripts/normalize-fl-historical-presidential-baseline.mjs` regenerates the 201 historical rows from retained official 2012/2016/2020 DOS precinct ZIPs and fails closed on 67 county rows and totals of 8,492,336; 9,498,093; and 11,090,844 candidate votes, respectively. Those historical totals are exact ZIP aggregates but have not been independently reconciled against a separately retained certified historical county or statewide canvass artifact.

Caveat: the precinct ZIP is valid for same-grain review rows and registration-denominator leads, but certified map totals remain anchored to the official county detail table. The ZIP presidential candidate total is 204 votes below the county detail table, and the ZIP Trump total is one vote higher. Historical ZIP registration is preserved only as dated source context; no compatible election-level ballots-cast semantics are available, so it does not replace EAC turnout. Historical county rows do not activate 2012 geometry or change the guarded 2016/2020/2024 release. Current advisory rows are public-interest screening inputs only, not findings.
## Minnesota
...
- Precinct registration source: `data/pa-2024-general-voter-registration-precinct.txt` (9,186 official DOS non-placeholder rows joined to returns by county code plus precinct code; the segmented breakdown fields verify all joined display identities)
...
- Precinct GIS collectors: `scripts/collect-pa-precinct-geometry.mjs` retains four deterministic canonical source packages. The reviewed partial 2016 package publishes 9,167 vote-free polygons and 8,014 exact result relationships; the reviewed partial 2020 package publishes 9,150 polygons and 6,805 relationships. `scripts/collect-pa-union-county-precinct-geometry.mjs` separately retains official Union County 2020 and 2024 candidates with 27 polygons and 27 reviewed relationships each. The canonical 2012 and statewide 2024 packages remain zero-feature availability diagnostics.
...
| Precinct registration rows | 9,186 |
| Drawable warning-required participation proxies | 9,150 |
| Turnout rows | 67 |

Caveats: the official readme says the election returns data was extracted January 10, 2025; the direct file name includes `20250129`. Actual turnout is county-level. The registration parser uses the stable county/precinct keys because 7,651 raw municipality names differ when the returns file redundantly appends the two registration breakdown fields; reconstructing that value matches all 9,186 registration rows. Local charts may use presidential votes divided by the December 23 registration snapshot only as an explicitly labeled, warning-required participation proxy, never as election-level ballots cast or SURE vote history. Four nonzero presidential rows have zero denominators and no drawable proxy percentage; 17 drawable ratios exceed 100 percent and remain disclosed. The reviewed partial 2016 and 2020 precinct packages are public at revision 27, but 1,158 official 2016 units / 782,683 votes and 2,360 official 2020 units / 1,545,703 votes remain excluded and unallocated; every 2020 Pike County unit is excluded. The official Union County 2020 candidate can account for two of the currently excluded units / 1,038 votes, and the 2024 candidate covers all 27 county units / 21,188 votes. The 2024 archive export lineage is dated September 5, 2024, but its feature metadata remains dated October 12, 2021, so the name match does not independently establish Election Day boundary effectiveness. Both candidates remain `delivery=null`, are absent from the registry and production, and require PASDA terms plus guarded release review; the 2024 candidate additionally requires affirmative effective-date evidence. The canonical 2012 and statewide 2024 packages remain zero-feature blocked diagnostics. The PA coverage inventory also records official 2024 RLA context and the loaded official 2012/2016/2020 historical bulk-return baselines.
...
- Voter Statistics candidate: `data/sc-2024-election-history/voter-statistics-7075.csv`, the official Election History contest 7075 export reached through the search page's Voter Statistics toggle (`voterStats=true`); it is hash-pinned and normalized by `scripts/normalize-sc-election-history-voter-statistics.mjs` to `data/sc-2024-election-history/voter-statistics-7075-county.csv`
- Voter Statistics reconciliation: the candidate has 46 county rows, 2,566,404 election-level ballots, and 3,451,344 registered voters; active VREMS has 46 rows, 2,553,185 participating voters, and 3,851,930 printed-list registrants. The exact request, raw SHA-256, and source-level deltas are in `data/sc-2024-election-history/voter-statistics-7075-request.json` and `data/sc-2024-election-history/voter-statistics-7075-vs-vrems-reconciliation.json`; VREMS remains active.
...
Caveats: review rows are President-versus-U.S.-House rather than a single statewide comparison contest. Precinct boundary geometry is not committed; the official SC VREMS precinct/polling-place export is a locator/reference path, not map-ready boundary geometry. The Election History Voter Statistics candidate has no active/inactive breakdown and does not reconcile to VREMS registration/participation totals, so it is not loaded as turnout. Official county historical baselines now include 2012, 2016, and 2020; the 2012 precinct-geometry candidate remains blocked. The retained historical source bytes and SHA-256 values are recorded in `data/sc-historical-presidential-baseline-summary.json`. A 2026-08-19 review proved the apparent 2016 drift was checkout line-ending conversion only: the repository blob, endpoint content, and all contest totals were identical. Byte-preservation attributes now keep the current 132,795-byte LF response stable (`cb2e118c354f87becf12883aafd40a7de83a1026be82516d13c6247b0122e42f`). Normalized audit/CVR/incident/recount/litigation rows remain future work.
...
- Historical registration/ballots-cast leads: `data/ga-historical-registration-turnout-leads.csv`, generated by `scripts/normalize-ga-historical-archive-registration.mjs` from hash-pinned official SOS 2012, 2016, and 2020 General Election ZIPs. It parses 159 nested county `summary.csv` files per year and fail-closes unless every county presidential tuple exactly matches the JSON baseline. These are historical context only; archive registration timing and compatibility with active EAC turnout semantics are not established.
...
- Current status: certified county rows, official-source local review, official active-voter turnout, and official 2012/2016/2020 county history are loaded, but SD remains in `sourceDiscoveryQueue` rather than `completedNativeStates`
- Current President source: `data/sd-2024-general-president-county.csv`, generated by `scripts/collect-sd-2024-county-results.mjs`; `scripts/verify_sd_2024_general_canvass.py` exactly validates its 66 county values against the retained official State Board of Canvassers certificate PDF
- Current comparison source: `data/sd-2024-general-us-house-county.csv`, generated by the same script and exactly validated against the certificate's 66 county U.S. House values
- Official validation source: `data/sd-2024-general-canvass-certificate.pdf`, a State Board of Canvassers certificate (801,624 bytes; SHA-256 `a9be018609c45e97c5b9b9c41d7f53dffc9c3390746486c115739e6d6d072c9c`) validated by `scripts/verify_sd_2024_general_canvass.py`; it verifies candidate results only and does not supply turnout/registration fields
- Review source: `data/sd-2024-general-president-us-house-precinct-review.csv`, generated by `scripts/collect-sd-2024-local-review.mjs` from the official ElectionID 684 President and U.S. House `Category=PREC` endpoints; it contains 691 same-key county-qualified reporting units across all 66 counties
- Comparison contest: at-large United States Representative at the same local reporting-unit grain
- Turnout source: the official `data/sd-2024-official-election-returns.pdf`, normalized to `data/sd-2024-official-active-voter-turnout.csv` by `scripts/normalize_sd_2024_official_turnout.py`; 66 counties report 436,478 ballots cast over 624,175 active voters as of November 5, 2024
- Historical baseline source: three hash-pinned South Dakota Secretary of State PDFs at `data/sd-2012-general-official-statewide-candidates-county.pdf`, `data/sd-2016-official-election-returns.pdf`, and `data/sd-2020-general-official-state-canvass.pdf`, normalized by `scripts/normalize_sd_historical_presidential_baseline.py` to 198 official county rows in `data/sd-historical-presidential-baseline.csv`; the 2012 source display name Shannon County is normalized to current Oglala Lakota County identity
- Certified-local publication audit: the four county-canvass bundles linked from the official 2024 Election History page contain county canvass certificate sections for 64 distinct counties, omit Buffalo and Stanley, and duplicate Brule. Local-result pages in the bundles expose visible precinct names but no ElectionID 684 `CountyID`/`StatePrecinctID`. The separate official polling-location PDF contains names and addresses but no polygons, feature IDs, CRS, or authoritative result-unit crosswalk.
...
- Official archive/export/request evidence: `data/sd-2024-official-results-archive-evidence.json`, `data/sd-2024-general-statewide-results.xlsx`, and `data/sd-2024-official-source-request-packet.json` identify official SD SOS archive ElectionID 684, candidate-list IDs 19833/19835, map RaceIDs 12665/11954, the official ResultsExport statewide XLSX workbook, official results FAQ text, public-records/county-auditor request targets, and 2024 General post-election audit-page context

Expected staging validation: 66 county result rows, 691 local review rows, 66 official active-voter turnout rows, 198 contextual historical baseline rows, and 66 county geometry features. President certified totals reconcile to 428,922 votes: 272,081 Trump, 146,859 Harris, and 9,982 Other. U.S. House certified totals reconcile to 421,448 votes: 303,630 Republican and 117,818 Democratic. The local ENR review rows total 428,728 President votes and 421,264 U.S. House votes.

Caveats: the 2024 President and U.S. House county rows retain secondary CSV extraction lineage, but every value is exactly validated against the retained certified SOS certificate. The 691 local review rows come from an official application labeled unofficial and are below the certificate by 194 President votes and 184 U.S. House votes; those county-level deltas are preserved but never allocated locally. Of the 691 units, 632 have four-digit identifiers and 59 are C-prefixed administrative units; none is assigned to precinct geometry. The published bundles contain county canvass certificate sections for only 64 counties, and their local-result pages cannot be safely joined to ENR units by display name alone. Turnout uses the dated official active-voter table, not the untimestamped ENR `Voters` field or the EAC total-registration denominator. Official historical baselines are resolved. The blocking request scope is the missing Buffalo and Stanley canvasses plus an authoritative ENR identity crosswalk and election-effective 2024 precinct geometry; normalized audit, CVR availability, recount, incident, correction, and litigation rows remain a supplemental follow-up. Advisory rows are screening inputs, not findings of fraud or misconduct.
