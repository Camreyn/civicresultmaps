...
import {
  ...
  sha256,
  validateWisconsinBlobPublicationEvidence,
...
} from "../../scripts/publish-wi-local-geography-status.mjs";
const deliveryOrigin = "https://example.public.blob.vercel-storage.com";
const blobReleaseCandidate = {
  id: "wi-local-reporting-gis-three-election-v1",
  path: ".etl/precinct-release-candidates/WI/example/release-candidate.json",
  sha256: "d".repeat(64),
};
const blobArtifacts = Array.from({ length: 219 }, (_, index) => {
  const year = [2016, 2020, 2024][Math.min(2, Math.floor(index / 73))];
  const kind = index % 73 === 72 ? "index" : "parent";
  return {
    kind,
    year,
    packageRelativePath: `delivery-assets/${year}/file-${index}.json`,
    publicUrl: `/data/geography/wi/${year}/local_reporting_unit/file-${index}.json`,
    pathname: `data/geography/wi/${year}/local_reporting_unit/file-${index}.json`,
    byteCount: 100 + index,
...
const evidence = {
  schemaVersion: 1,
  state: "WI",
  purpose: "wi-local-reporting-parent-scoped-immutable-geometry-publication",
  releaseCandidate: blobReleaseCandidate,
  authorizationId: "wi-blob-publication-1",
...
const rollbackTarget = {
  deploymentId: "dpl_wisconsin_previous",
...
const authorizationPlan = {
  id: "wi-local-database-publication-v1",
  releaseCandidate,
...
function publicationSummary() {
  return {
    path: ".etl/production-publication-receipts/WI/wi-local-publish.json",
    sha256: "a".repeat(64),
    activationId: "wi-public-camreyn",
...
test("Wisconsin Blob evidence requires all 219 immutable artifacts", () => {
  const result = validateWisconsinBlobPublicationEvidence(
    evidence,
    blobPlan,
    Date.parse("2026-08-13T00:01:00.000Z"),
  );
  assert.equal(result.assetCount, 219);
  assert.equal(result.deliveryOrigin, deliveryOrigin);
  const incomplete = { ...evidence, assetCount: 218 };
  assert.throws(
    () => validateWisconsinBlobPublicationEvidence(incomplete, blobPlan),
    /incomplete or incompatible/,
  );
});

test("Wisconsin public authorization pins production and rollback deployment trees", () => {
  const value = publicAuthorization();
  const checked = validateWisconsinPublicationAuthorization(value, {
    plan: authorizationPlan,
    planSha256,
    now: Date.parse("2026-08-13T01:01:00.000Z"),
  });
  assert.equal(checked.approvedBy, "Camreyn");
  assert.deepEqual(value.scopes, WISCONSIN_PUBLICATION_SCOPES);
  ...
  assert.throws(
    () => validateWisconsinPublicationAuthorization(sameTree, {
      plan: authorizationPlan,
      planSha256,
      now: Date.parse("2026-08-13T01:01:00.000Z"),
    }),
...
test("Wisconsin rollback authorization is bound to the exact publication receipt", () => {
  const receipt = publicationSummary();
  const value = buildWisconsinPublicRollbackAuthorizationTemplate(
    authorizationPlan,
    planSha256,
    receipt,
  );
  Object.assign(value, {
    ...
    rollbackId: "wi-rollback-camreyn",
    approvedBy: "Camreyn",
  ...
  const checked = validateWisconsinPublicRollbackAuthorization(value, {
    plan: authorizationPlan,
    planSha256,
    publicationReceipt: receipt,
    now: Date.parse("2026-08-13T01:20:00.000Z"),
  });
  assert.equal(checked.rollbackId, "wi-rollback-camreyn");
  assert.deepEqual(value.scopes, WISCONSIN_PUBLIC_ROLLBACK_SCOPES);

  const substituted = structuredClone(value);
  substituted.applicationRollback.target.deploymentId = "dpl_substituted";
  assert.throws(
    () => validateWisconsinPublicRollbackAuthorization(substituted, {
      plan: authorizationPlan,
      planSha256,
      publicationReceipt: receipt,
      now: Date.parse("2026-08-13T01:20:00.000Z"),
...
test("Wisconsin publication receipt reloads its exact original public authorization", () => {
  const root = mkdtempSync(path.join(tmpdir(), "crm-wi-publication-"));
  const authorizationPath =
    ".etl/production-authorizations/WI/wi-local-public-authorization.json";
  const receiptPath =
    ".etl/production-publication-receipts/WI/wi-local-publish-receipt.json";
  const authorization = publicAuthorization();
  const authorizationBytes = serializeWisconsinPublicationDocument(authorization);
  ...
  for (const [relative, bytes] of [
    ...
    [receiptPath, serializeWisconsinPublicationDocument(receipt)],
  ]) {
  ...
  const inspected = inspectWisconsinPublicationReceipt(root, {
  ...
  const substitutedPath =
    ".etl/production-publication-receipts/WI/wi-local-substituted-receipt.json";
  const substitutedBytes = serializeWisconsinPublicationDocument(substituted);
  writeFileSync(
...
test("Wisconsin Git verifier resolves both the live and rollback deployment trees", () => {
  const authorization = validateWisconsinPublicationAuthorization(publicAuthorization(), {
    plan: authorizationPlan,
    planSha256,
    now: Date.parse("2026-08-13T01:01:00.000Z"),
  });
  ...
  assert.equal(
    verifyWisconsinPublicationGitCandidate(process.cwd(), authorization, runner)
...
function rollbackTransactionFixture() {
  const manifests = [2016, 2020, 2024].map((year, index) => ({
    ...
    delivery: {
      ...
      featureCount: [3648, 3705, 3503][index],
      parentCount: 72,
      indexPath: `/data/geography/wi/${year}/local_reporting_unit/index.json`,
    },
  }));
  const plan = {
  ...
  const versions = manifests.map((manifest, index) => {
    ...
    return {
      ...
      year: manifest.year,
      status: "published",
      caveat: "Reviewed Wisconsin election-specific local reporting geometry is publicly authorized under activation "
  ...
  const authorization = {
    activationId: "wi-rollback-camreyn",
    rollbackId: "wi-rollback-camreyn",
...
function rollbackClient(fixture) {
  let revision = 41;
  let reason = "Wisconsin local reporting unit geometry publish wi-public-camreyn";
  ...
  const unsafe = async (statement, parameters = []) => {
    ...
    if (sql.startsWith("update reporting_unit_geometry_crosswalks")) {
      return Array.from({ length: 10_937 }, (_, id) => ({ id }));
    }
    ...
    if (sql.includes("from reporting_unit_geometry_crosswalks")) {
      assert.match(sql, /x\.match_method in \('reviewed_name','spatial_review'\)/);
      assert.match(sql, /x\.relationship_type='non_geographic'/);
      return [{ total: 10_937, exact: 10_937 }];
    }
    ...
    if (sql.includes("from result_rows rr")) return [{ total: 32_475 }];
    if (sql.includes("from pg_constraint")) return [{ count: 0 }];
    if (sql.includes("from public_data_revisions")) return [{ revision, reason }];
    throw new Error("Unexpected Wisconsin publication SQL: " + sql);
  };
  return { unsafe, get versionUpdateCount() { return versionUpdateCount; } };
}

test("Wisconsin rollback atomically blocks database delivery and preserves publish audit", async () => {
  const fixture = rollbackTransactionFixture();
  const client = rollbackClient(fixture);
  const result = await applyWisconsinGeographyPublicationTransaction(client, {
    mode: "rollback",
    plan: fixture.plan,
    planSha256,
  ...
  for (const [index, version] of fixture.versions.entries()) {
    ...
    assert.equal(version.metadata.publicActivation.activationId, "wi-public-camreyn");
    assert.equal(
...
test("Wisconsin publication atomically records the pinned rollback target", async () => {
  const fixture = rollbackTransactionFixture();
  for (const [index, version] of fixture.versions.entries()) {
...
test("Wisconsin rollback transaction rejects a substituted live rollback target", async () => {
  const fixture = rollbackTransactionFixture();
  ...
  await assert.rejects(
    () => applyWisconsinGeographyPublicationTransaction(client, {
      mode: "rollback",
      plan: fixture.plan,
      planSha256,
      authorization: fixture.authorization,
...
test("Wisconsin publication runner contains database-first rollback and read-only recovery guards", () => {
