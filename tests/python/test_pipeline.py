...
class PipelineTests(unittest.TestCase):
    ...
    def test_illinois_native_staging_parses_official_by_office_csvs(self):
        ...
        source_ids = {source["id"] for source in artifact["sources"]}
        self.assertIn("il-2024-by-office-turnout-source-review", source_ids)
        turnout_review = json.loads(Path("data/il-2024-turnout-source-review.json").read_text(encoding="utf-8"))
        self.assertEqual(turnout_review["decision"], "keep_eac_fallback")
        self.assertEqual(turnout_review["officialPresidentCsvTotals"]["registration"], 7210422)
        self.assertEqual(turnout_review["officialPresidentCsvTotals"]["zeroRegistrationPrecincts"], 2260)
        self.assertEqual(turnout_review["deltasVsEacFallback"]["contestTotalIncludingNonCandidateRows"], -14371)
    ...
    def test_indiana_enr_county_json_parser_builds_county_rows(self):
        ...
        self.assertEqual(len(artifact["sources"]), 13)
        ...
        audit_source = next(source for source in artifact["sources"] if source["id"] == "in-2024-vstop-general-audit-summary")
        self.assertEqual(audit_source["parser"], "indianaVstopAuditSummaryCsv")
        self.assertTrue(all(item["exists"] for item in audit_source["metadata"]["artifacts"]))
    ...
    def test_mississippi_election_recap_csv_parser_builds_county_rows(self):
        ...
        self.assertTrue(artifact["capabilities"]["historicalBaseline"])
        self.assertEqual(artifact["native"]["metrics"]["nativeHistoricalRows"], 246)
        self.assertEqual(artifact["native"]["metrics"]["nativeHistoricalYears"], [2012, 2016, 2020])
        adams_2012 = next(row for row in artifact["native"]["historicalRows"] if row["jurisdictionName"] == "Adams County" and row["electionYear"] == 2012)
        self.assertEqual(adams_2012["demVotes"], 9061)
        self.assertEqual(adams_2012["repVotes"], 6293)
        desoto_2020 = next(row for row in artifact["native"]["historicalRows"] if row["jurisdictionName"] == "DeSoto County" and row["electionYear"] == 2020)
        self.assertEqual(desoto_2020["demVotes"], 28265)
        self.assertEqual(desoto_2020["repVotes"], 46462)
    ...
    def test_new_york_county_president_csv_parser_builds_county_rows(self):
        ...
        self.assertEqual(artifact["native"]["metrics"]["nativeReviewRows"], 10408)
        self.assertEqual(artifact["native"]["metrics"]["nativeComparisonRows"], 10408)
        self.assertEqual(artifact["native"]["metrics"]["nativeComparisonContest"], "United States Senator")
    ...
    def test_florida_detail_html_parser_builds_county_rows(self):
        ...
        self.assertEqual(artifact["native"]["metrics"]["nativeReviewRows"], 5618)
        self.assertEqual(artifact["native"]["metrics"]["nativeComparisonRows"], 5618)
        self.assertEqual(artifact["native"]["metrics"]["nativeComparisonContest"], "United States Senator")
        ...
        self.assertFalse(artifact["native"]["metrics"]["nativeCountyDistributionAnalysis"])
        self.assertEqual(artifact["native"]["metrics"]["nativePrecinctZipFiles"], 67)
        self.assertEqual(artifact["native"]["metrics"]["nativePrecinctZipRows"], 537897)
        self.assertEqual(artifact["native"]["metrics"]["nativePrecinctZipPrecinctRows"], 5712)
        self.assertEqual(artifact["native"]["metrics"]["nativePrecinctZipRegisteredVoters"], 13942186)
        self.assertEqual(artifact["native"]["metrics"]["nativePrecinctZipPresidentCandidateTotalDelta"], -204)
        self.assertTrue(artifact["native"]["metrics"]["nativeExtractSenateVerificationSkipped"])
        self.assertTrue(any(row["coverageMode"] == "presidentVsSenatePrecinct" for row in artifact["native"]["reviewRows"]))
        alachua = next(row for row in artifact["native"]["reviewRows"] if row["county"] == "Alachua County" and row["localUnit"] == "1 - First Baptist Church of Waldo")
        self.assertEqual(alachua["comparisonDemVotes"], 357)
        self.assertEqual(alachua["comparisonRepVotes"], 700)
        self.assertEqual(alachua["registeredVoters"], 1324)
    ...
    def test_georgia_media_export_parser_builds_native_rows(self):
        ...
        ga_source_ids = {source["id"] for source in artifact["sources"]}
        self.assertIn("ga-2024-turnout-vote-method-source-review", ga_source_ids)
        ga_turnout_review = json.loads(Path("data/ga-2024-turnout-vote-method-source-review.json").read_text(encoding="utf-8"))
        self.assertEqual(ga_turnout_review["decision"], "keep_eac_fallback")
        self.assertEqual(ga_turnout_review["officialMediaExport"]["presidentCandidateVotes"], 5250047)
        self.assertEqual(ga_turnout_review["officialMediaExport"]["deltaPresidentVotesVsEacBallotsCast"], -47453)
    ...
    def test_idaho_county_president_csv_builds_results_with_county_review_rows(self):
        ...
        self.assertEqual(native["metrics"]["nativeBallotsCast"], 917469)
        self.assertEqual(native["metrics"]["nativeRegisteredVoters"], 1178750)
        self.assertEqual(native["metrics"]["nativeTurnoutWarningRows"], 0)
        sources = {source["id"]: source for source in artifact["sources"]}
        self.assertEqual(sources["id-2024-general-turnout-county-workbook"]["status"], "loaded")
        self.assertEqual(sources["id-2024-eac-turnout"]["status"], "candidate")
