...
class PipelineTests(unittest.TestCase):
    ...
    def test_iowa_clarity_json_parser_builds_county_rows(self):
        config = load_config("etl/state-configs/ia.json")
        report = validate_config(config)
        artifact = build_staging_artifact(config, report)

        self.assertTrue(report.passed)
        self.assertEqual(artifact["native"]["parser"], "nativeIowaClarityJson")
        ...
        self.assertEqual(artifact["native"]["metrics"]["nativeReviewRows"], 99)
        self.assertEqual(artifact["native"]["metrics"]["nativeComparisonRows"], 99)
        self.assertEqual(artifact["native"]["metrics"]["nativeComparisonContest"], "United States Representative")
        scott = next(row for row in artifact["native"]["reviewRows"] if row["county"] == "Scott County")
        self.assertEqual(scott["coverageMode"], "presidentVsUSHouse")
        self.assertEqual(scott["harris"], 42479)
        self.assertEqual(scott["trump"], 45976)
    def test_west_virginia_clarity_json_parser_builds_county_rows(self):
        config = load_config("etl/state-configs/wv.json")
        report = validate_config(config)
        artifact = build_staging_artifact(config, report)

        self.assertTrue(report.passed)
        self.assertEqual(artifact["native"]["parser"], "nativeWestVirginiaClarityJson")
        ...
        self.assertEqual(artifact["native"]["metrics"]["nativeReviewRows"], 55)
        self.assertEqual(artifact["native"]["metrics"]["nativeComparisonRows"], 55)
        self.assertEqual(artifact["native"]["metrics"]["nativeComparisonContest"], "United States Senator")
        kanawha = next(row for row in artifact["native"]["reviewRows"] if row["county"] == "Kanawha County")
        self.assertEqual(kanawha["coverageMode"], "presidentVsSenate")
        self.assertEqual(kanawha["harris"], 30231)
        self.assertEqual(kanawha["trump"], 43352)
    ...
    def test_new_york_county_president_csv_parser_builds_county_rows(self):
        ...
        self.assertEqual(artifact["native"]["metrics"]["nativeReviewRows"], 62)
        self.assertEqual(artifact["native"]["metrics"]["nativeComparisonRows"], 62)
        self.assertEqual(artifact["native"]["metrics"]["nativeComparisonContest"], "United States Senator")
        albany = next(row for row in artifact["native"]["reviewRows"] if row["county"] == "Albany County")
        self.assertEqual(albany["coverageMode"], "presidentVsSenate")
        self.assertEqual(albany["harris"], 92589)
        self.assertEqual(albany["trump"], 54560)
        self.assertEqual(albany["comparisonDemVotes"], 95231)
        self.assertEqual(albany["comparisonRepVotes"], 49505)
