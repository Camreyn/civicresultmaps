...
class KansasCoverageInventoryTests(unittest.TestCase):
    ...
    def test_kansas_config_registers_official_sources_and_inventory(self):
        ...
        self.assertEqual(config["expected"]["historicalBaselineRows"], 315)
        self.assertTrue(config["capabilities"]["historicalBaseline"])
        self.assertEqual(config["historicalBaselines"]["expected"]["rowCount"], 315)
    ...
    def test_kansas_inventory_keeps_loaded_and_missing_artifacts_distinct(self):
        ...
        self.assertEqual(artifacts["ks-historical-presidential-baseline"]["expectedCounts"]["historicalRows"], 315)
        self.assertEqual(artifacts["ks-historical-presidential-baseline"]["confidence"], "loaded_mixed_official_secondary_context")

        self.assertEqual(findings["stateNativeTurnout"]["status"], "not_loaded_eac_fallback_active")
        self.assertEqual(findings["historicalBaselines"]["status"], "loaded_mixed_official_secondary_context")
    ...
    def test_kansas_registries_are_aligned(self):
        ...
        self.assertIn("mixed official/secondary", " ".join(tier["availableFields"]))
        ...
        self.assertEqual(native_ks["expected"]["historicalBaselineRows"], 315)
        self.assertEqual(native_ks["artifacts"]["historicalBaseline"]["localFile"], "data/ks-historical-presidential-baseline.csv")
