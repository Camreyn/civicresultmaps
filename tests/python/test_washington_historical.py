...
class WashingtonHistoricalBaselineTests(unittest.TestCase):
    def test_washington_2020_historical_source_artifact_is_scripted_and_configured(self):
        ...
        self.assertEqual(raw_config["historicalBaselines"]["expected"]["rowCount"], 39)
        self.assertEqual(raw_config["expected"]["historicalBaselineRows"], 39)
        ...
        king = next(row for row in rows if row["jurisdiction_name"] == "King County")
        self.assertEqual(king["row_method"], "washingtonOfficialCountyCsvHistorical")
        ...
        self.assertEqual(sum(int(row["dem_votes"]) for row in rows), 2369612)
        self.assertEqual(sum(int(row["rep_votes"]) for row in rows), 1584651)
        self.assertEqual(sum(int(row["other_votes"]) for row in rows), 133368)
        self.assertEqual(sum(int(row["total_votes"]) for row in rows), 4087631)
